...
- Asset designer: resizing an object no longer shrinks the clearance boundary you drew. A traced, measured clearance keeps the size you gave it and the Inspector says so — "This clearance was kept at the size you drew it when the object was resized. Check that it still describes the space you need." — with **Mark clearance as reviewed** to clear the notice once you have. A clearance still waiting for its scale goes on scaling with the drawing, because it is in reference pixels rather than millimetres. The notice survives closing and reopening the asset: geometry holding one is saved as schema 4, which older builds refuse rather than dropping the flag. Marking a clearance reviewed is not a fit check and certifies nothing.

- Asset library: **Duplicate** an asset as a new definition with its own geometry, and see which plans place it before you do. **Used in plans** names each plan and how many copies it holds, says plainly when the list could not be read or when some notes were unreadable, and does not pretend an empty vault scan means nobody uses it. The copy is independent: plans that place the original keep the original.

- Asset designer: **Use in plan** takes the object you are designing into a plan — the one already open when exactly one is, and otherwise the one you pick — and arms placement with that asset on arrival. It is drawn only for an object that can actually be placed, so an unscaled or shapeless draft shows no button that could only refuse. **Open the asset library** goes the other way. Opening the asset library from the command palette is offered on desktop only, the same platform gate the plan editor and the designer already use.

- Asset designer: fade a reference sheet with **Reference opacity** in the View menu so you can trace over it, and **Remove reference** to take the sheet away without touching the calibration or the millimetres already measured from it. Removing a reference whose file has already been deleted succeeds — it is the one gesture that repairs that state.

- Asset designer: a **Reference** panel states, per group of coordinates, whether it is millimetres or still reference pixels, and whether the sheet is calibrated; a **Placement** panel sets the point a plan positions the object by (Centre, Back centre or a custom point) and the direction its front faces; and a **Clearance** panel generates a boundary from four numbers — in front, behind, left, right. The four-number helper is offered only for a rectangular outline with an axis-aligned front: for anything else the fields and the button are not drawn at all and the panel says why and what to do instead ("A four-sided helper needs a rectangular outline and a front pointing along an axis. Trace a boundary instead."), rather than offering a control that could only refuse. It says outright that the four numbers are your own allowances and not a standard.

- Asset designer: **Draw line** places an open polyline point by point, and **Draw rounded rectangle** draws a rectangle with rounded corners. A line has no inside, so solid and dashed set its pattern rather than filling it, and it has no resize or rotate handles — drag it to move it, or set its centre, size and rotation in the Inspector fields.

- Asset designer: **Arrange** groups parts and lays them out. Group and Ungroup, Bring group to front and Send group to back, six alignments and four distributions — even centres or even gaps, across or down — against either the selection bounds or one part you nominate to stay put, plus numeric move and scale and a **Repeat** form that adds a counted row of copies at a centre-to-centre distance or an edge gap. Each is one undoable change. An arrangement mixing parts still in reference pixels with measured ones is refused with its reason rather than silently scaled, and a locked part is named rather than moved.

- Asset designer: a **Parts** panel lists everything the object is made of — the reference sheet, the footprint, the clearance, each graphic and each group — and hides, shows, locks, unlocks, isolates and renames them. Its rows and the canvas share one selection, so choosing in either chooses in both, and **Show all parts** undoes an isolation.
...
- Plan editor: color anything on a plan — items, placed assets, paths, measurements, stairs, posts, beams, drafting marks, walls, doors, windows and rooms — from its right-click menu or Details, one at a time or several at once as one undo step. Choose a named preset or, in Details, any custom color from the system color picker. Fills and walls take a light tint under their hatch, lines and text are drawn in the color, and rooms get a see-through wash. Plan geometry holding a custom color, or a color on anything but an item or placed asset, is saved as schema 16, which older builds refuse; preset colors on items and placed assets still use schema 14.
...
- Plan editor: create a detail plan from a zone's context menu, open existing detail plans from the same menu, walk back up through the breadcrumb, and trace inside the parent zone's outline guide. Detail plan notes are saved as plan schema 9, which older builds refuse. The guide is captioned with its source, explained in the Inspector and framed by Fit, and a failed hierarchy read is reported in the Property tree.
...
- Asset designer: Set dimensions writes nothing when the numbers it offered are typed back, so a dialog opened and confirmed unchanged no longer costs an undo step that appears to do nothing. Undo, Redo, Choose a drawing, Start from preset and the replace-with-a-rectangle half of Set dimensions now wait their turn behind a gesture that is still being saved, instead of racing it: an undo clicked during a save undoes the gesture that just happened, and a background or preset chosen mid-save is no longer refused as a conflict with the user’s own earlier gesture.
