...
- Plan editor: assign one Item or placed asset a named preset color from its right-click menu or Details. Default clears the override; undo/redo, copy/paste and promotion preserve it. Color changes are Plan-only and require one selected member, leaving groups, library definitions and other placements unchanged. Colored geometry uses schema 14, which older builds refuse.
