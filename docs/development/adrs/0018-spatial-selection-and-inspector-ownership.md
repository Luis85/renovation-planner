...
**Amended 2026-09-12:** the Room/Area use of `OutlinePointsForm` is withdrawn, with its Inspector action, canvas `Edit shape` button and zone context-menu `Edit`. A zone's outline is edited only by dragging its canvas vertices; the form now serves linear and object elements alone.
