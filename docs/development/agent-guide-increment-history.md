# Renovation Planner — increment history

The recorded history of every design slice and increment: what landed, what was refused or
withdrawn, and the review-round findings behind the shape of each module. Extracted from
`CLAUDE.md` so it loads on demand rather than in every session; the rules that govern work
*today* stayed in `CLAUDE.md`.

Read this when you need to know why a module is shaped the way it is, what a slice decided
against, or what a review round already found — not to learn how to work in the repo.
...
- **Nothing draws this shape on a Plan.** A footprint, a clearance, an anchor and a facing
  ...
  `docs/user-experience/archive/asset-library-overview-DESIGN-SPEC.md`, which carried the identical
...
Tab rather than thirty. `docs/user-experience/archive/renovation-planner-home-DESIGN-SPEC.md` is the
...
# Gate and testing history

Extracted from `CLAUDE.md`'s *Definition of done* and *Testing* sections: the measurements,
post-mortems and defect catalogues behind those gates. The rules they produced stayed in
`CLAUDE.md`.

## Gate timing: the dated snapshots
...
## The machine-wide gate lock that was built and deleted
...
## `tests/**` type-checking: the ratchet
...
## The type-check ratchet, and the ten defects it found
...
## Why each .test-d.ts entry was worth a compiler
...
## The fake-too-thin rule: every recorded instance
...
## The planted-probe race: what came out of it
...
## Editorial provenance: what these passages used to say

Cut from `CLAUDE.md` in an overhaul pass. Each is the record of how a sentence there went
wrong, kept because the failure shapes are real; the rule each produced stayed in
`CLAUDE.md`.

### Settings-pane row count: the four ways it went wrong
...
rather than describing it. The persistence layer

### Slice-completion ledger: what each correction was
...
### Vue-root and view-type counts: why no number is stated
...
### Project list: slice attribution and the LAUNCHER correction note

The **Renovation project** view is a
singleton with
...
### The LAUNCHER paragraph, which corrected the one above it instead of rewriting it
...
one.** A project row NAVIGATES

### Plan editor: the "nothing on that canvas is editable" correction
...
### create-sample-project: the trigger that fired with nothing to notice it
...
left is the reason it was written for:


### I18N_LITERAL_BAN: how the last two call sites were added
...
### Coverage floors: the dated figures and the slice-16 anecdote
...
### Worktrees: how four accumulated, and the fallow file count
...
### vue-router: the six-versus-seven-destinations correction
...
### Bundle size: the full measured series
