...
| Next executable action | Recorded at the end of the session log below. **As of 2026-09-23 (session 17) it is L-40**: a harness fixture that can reach `ProjectWorkState.vue`'s schedule surface, so its **Show diagnostics report** button can be captured. L-42 is closed, and no other limitation is movable from this machine: L-38's residue needs a vault run to decide, and L-37, L-33's residue and L-36 are owner questions. |
...
|---|---|---|---|---|---|---|---|
| BP-00 | Reconcile baseline and ownership | P0 | None | Discovery | **Complete** | This session / unmapped | Discovery is done and the Finding reconciliation table below classifies the handoff's findings; session 17 corrected two of its rows, mobile evidence and optional export. Baseline gates: the Current state table's `Baseline full gates` row. No further action. History: the first session log (2026-09-16). |
...
| BP-02 | Durable incident detection and recovery | P0 | BP-01 | Safety hardening | **Slices 1 to 4 landed. Open: L-06, L-11, L-14, L-15, L-17 and L-18. L-13 is reclassified rather than closed, and its residue is L-14's affordance gap. L-16 is closed.** | This session / ADR-0019 requires a decision record for slice 2 | Slice 1 at `81f627b53..beda98597`. Slice 2 (ADR-0034, closing L-02) at `4599a388e..f05d5d62f`. Slice 3 at `60a748423..5580e53b5`. Slice 4 at `e6cdd914b..2546d88d8` (L-05), `36a4c92f7..4966cbe7b` (the write gate reads the vault's own record, closing L-01 for the Plan Editor) and `e7c24d91b..9d08aeed4` (L-06 narrowed by a pinned census). L-12 closed at `e118f61d4`. L-16, undo and redo landing while the vault is paused on both editor surfaces, closed at `38d5292f5` by `src/presentation/editor/tools/with-incident-gate.ts`. L-06 with L-11 is `05-owner-decisions.md` Q1, an owner question; L-14 is recorded as having no data-safety effect, L-15 as wrong emphasis in two strings, and L-17 and L-18 as accuracy findings. Next: none in this package until Q1 is answered. History: session logs 2–6. |
...
| BP-04 | Precise non-drag corner editing | P1 | BP-00; integrate after BP-03 | Interaction addition | **Slices A, A2 and B landed. The Acceptance criteria and Actions 1–6 are recorded as met. Open: the Deliverable's real-screenshot clause, which needs a vault run. The package is not marked closed.** | Session 9 discovery / session 10 slice A / session 11 slice A2 / **session 12 slice B** | Slice A at `6546f402c` and `152a3c18c`; slice A2 at `8bfd6dd6a`, `b6b6a1f27` and `e2d524a9b`; slice B at `1f2cf7cf8`, its user guide at `73af5eb95` and its fix round at `2cf585a40`. Action 1's specification is `docs/superpowers/specs/2026-09-21-bp04-numeric-corner-editing-spec.md` (`3c0e01d21`), cross-linked with the 2026-09-12 side-panels design's Amendment 1. The criterion-by-criterion call is in the session 12 log, and Action 1's closure in the session 13 log. Named tests: 10 covered / 2 partial / 0 absent, from session 14's re-run of the acceptance audit. Open limitations tied to this package: L-22, L-26, L-27 and L-32, and the owner copy question L-36; none of them is a closing item. Next: a vault run capturing the corner-editing states for the screenshot clause, since `harness-shots/` is gitignored and no capture travels with the repository, or an owner ruling that accepts the package without it (L-31). History: session logs 9–14. |
| BP-05 | Selection, transform, cancel and history | P1 | BP-03; coordinate BP-04 | Verification/polish | Not started under the plan. Much of its behaviour is already implemented and tested per feature in jsdom. | Unassigned / unmapped | Already in the tree: overlap priority and modifiers (`resolveSelectionTarget.test.ts`; ADR-0018 and ADR-0027), one Escape precedence function (`escapeRouting.ts`, `escapeRouting.test.ts`), one operation per history entry and no entry for a rejected command (`history.e2e.test.ts`, `commandHistory.test.ts`), a keyboard context menu on the ContextMenu key and Shift+F10 (`CanvasContextMenu.vue`, `contextMenuLifecycle.test.ts`, `nativeShellInputBoundaries.test.ts`), and paste across plans (`clipboard.test.ts`). Absent: the capability matrix, which is the Deliverable; a dense-overlap fixture; a case asserting that a hover leaves the selection unchanged; German interaction cases; a modifier change during a move or rotate drag; a unit test of `historyShortcut.ts`; and action-specific history labels, which `undoable-command.ts` carries no field for. **Owner question:** the Acceptance clause "rejected/no-op operations do not add history" contradicts `CommandHistory.runNow`, which puts a no-write gesture on the undo stack by design. `history.e2e.test.ts` pins that behaviour, and the Done PBI `docs/requirements/Undo and redo.md` records the clause's history half as NARROWED at its criterion 6. Either the clause is narrowed or the code changes (`05-owner-decisions.md` §8). Next, doable here: write the capability matrix from the existing tests and mark each empty cell. Needs a vault: the platform shortcuts on Windows and on macOS. |
| BP-06 | Empty-plan and reference journeys | P1 | BP-03 | Verification | Not started under the plan. The three starts and the image and PDF reference flow are built and covered in jsdom; none has been run in a vault. | Unassigned / unmapped | Already in the tree: `FloorStart.vue`'s three starts (`referenceWorkflow.e2e.test.ts` "offers three query-derived starts…"); prepare, measure and commit for PNG and PDF, with a reload through a fresh stack (`referenceWorkflow.e2e.test.ts`, `configurePlanReference.test.ts`); refused zero and invalid distances (`referenceSetup.test.ts`); a rescale of existing geometry that waits for explicit consent ("requires explicit acknowledgement before rescaling existing geometry" in `referenceWorkflow.e2e.test.ts`; ADR-0019, ADR-0020); missing, unreadable and invalid-page references (`background.test.ts`); and no write on cancel. Absent: a multi-page PDF fixture, since `grep -a -o "/Count [0-9]*"` on `tests/fixtures/editor-background-pdf-test.pdf` prints `/Count 1`; handling for a renamed reference source; a large-image bound; a known-distance example; and the short walkthrough the Deliverable names. Steps 4 and 5 of `docs/tests/cases/Empty States Walkthrough.md` expect a buttonless "No plan document yet" panel and predate `FloorStart`, which `PlanEditorRoot.vue` now draws for the no-background state. Next, doable here: a multi-page synthetic PDF in the fixture generator and one test decoding its page 2 through `pdfRaster`, then those two steps brought up to date. Needs a vault: the starts and the reference flow in Obsidian, Obsidian's own PDF.js, and a real rename or move of the source file. |
| BP-07 | Responsive, keyboard and accessibility | P1 | BP-04–BP-06 for final run | Verification/repair | Not started under the plan. Breakpoints, reflow with drafts and focus, and most keyboard alternatives are implemented and tested in jsdom; no screen reader, host zoom or community theme has been run. | Unassigned / unmapped | Already in the tree: the breakpoints `FULL_MIN_PX = 900` and `CONSTRAINED_MIN_PX = 400` (`layoutMode.test.ts`); reflow that keeps drafts, focus and selection (`persistentRegions.test.ts`, `responsiveShell.test.ts`); keyboard alternatives for corners (BP-04), nudging (`keyboardNudge.test.ts`), panel resizing (`panelResizer.test.ts`) and tree reordering (`propertyTreeReorder.test.ts`); axe-core scans of the real surfaces in the `tests/harness/accessibility*.test.ts` files, which switch off `color-contrast`, `color-contrast-enhanced` and `target-size` (`tests/harness/axeOptions.ts`); and a 200% CSS-zoom reflow pass in `scripts/editor-recovery-check.mjs`, whose own method string keeps native zoom separate. Absent: the interaction inventory (Action 3); keyboard-only panning of the plan canvas, since arrows nudge and Space arms a pointer-drag pan (`surface/keyDoors.ts`); and any contrast, focus-visibility or target-size check. Layout items against this package's acceptance: L-26, L-27 and L-32; L-27 and L-32 need a design decision rather than a patch. Next, doable here: the inventory assembled from the tests above, recording the keyboard-pan gap. Needs a vault, device or screen reader: a community theme, a named screen reader, host zoom, contrast and target size. |
| BP-08 | Representative performance and cleanup | P1 | BP-00; final integrated run | Benchmark | Not started under the plan. Cleanup across open and close is held in jsdom, and a mixed-scene browser driver exists that has not been run at the current tree. | Unassigned / unmapped | Already in the tree: `docs/tests/cases/Canvas performance.md` (room-heavy, harness rows dated 2026-09-13); the cases "stacks nothing across repeated open and close cycles" in `scene.test.ts` and in `planEditorView.test.ts`; and `scripts/editor-recovery-check.mjs`'s large-floor pass over `seedLarge()` in `tests/harness/planningRecoveryProbe.ts` (rooms, materials, assets and photos; frame gaps; stage and listener counts across close and reopen). That driver is not wired into `package.json`, was last changed 2026-09-07, and its recorded results in `pan-performance-diagnosis.md` are for older bundles and are not claimed for this tree. Absent: walls, openings and a reference image in that fixture, the three tiers, an environment record, and any measurement of a current bundle. **Owner or recorded decision needed:** the plan's targets (a plan open within 3 s, feedback within 100 ms, a p95 frame time of about 33 ms) disagree with the PBI `docs/requirements/Meet editor performance and cleanup budgets.md` (status New: initial render under 1.5 s, selection under 100 ms, Inspector under 200 ms), whose set the driver's `targets` object encodes; the plan requires a recorded decision for a changed target (`05-owner-decisions.md` §8). Next, doable here: extend `seedLarge()` with walls, openings and a reference image, and record one harness run at a named commit, labelled as harness numbers. Needs a vault: native numbers on the reference machine. |
| BP-09 | Desktop/mobile support boundary | P1 | BP-00; final BP-13 evidence | Native verification | Not started under the plan. The palette guards, the two canvas views' mobile refusals and the project view's read-only mode ship and are tested in jsdom; no device run exists; the Asset Library's write controls are not guarded on mobile (L-43). | Unassigned / unmapped | Already in the tree, from `git grep -n "Platform.isMobile" -- src`: `open-plan-editor`, `set-plan-background`, `open-asset-designer`, `create-sample-project` and `new-project` answer `false` on mobile; `PlanEditorView` and `AssetDesignerView` refuse in `sync()`; the project view passes `readOnly: Platform.isMobile`. Tests: `mobileReadOnly.test.ts`, `mobileDesktopOnly.test.ts`, `projectEntryBoundaries.test.ts`, `tests/plugin/*Commands.test.ts`. The requirement is `docs/requirements/Bound the mobile surface to what it can actually do.md` (status Active). **Open, and a P0 candidate under the plan's own rule: L-43.** The project view's enabled Assets door and the unguarded `open-asset-library` command reach an Asset Library whose create, edit and delete controls carry no mobile guard. Guarding the library or narrowing the mobile claim is an owner decision. Absent: a device run (`docs/tests/cases/Read projects on mobile.md` reads "Not yet run on a device"), a mobile case for the Work and schedule actions, and the tested-versus-untested matrix. Next, doable here: a jsdom case mounting the Asset Library with `Platform.isMobile = true` that asserts its write controls are refused, watched red on today's tree; which way it turns green waits on L-43's ruling. Needs a device: Actions 2 to 5 and that case's Runs table. |
...
| Mobile device evidence incomplete | Earlier review only | **Still true** | The mobile case's Runs table reads "Not yet run on a device". Other mobile evidence exists and is jsdom only: the requirement `docs/requirements/Bound the mobile surface to what it can actually do.md` (status Active) with its guards and tests (`mobileReadOnly.test.ts`, `mobileDesktopOnly.test.ts`, `projectEntryBoundaries.test.ts`, `tests/plugin/*Commands.test.ts`). That requirement lists the Asset Library's write controls as still open, recorded as L-43. | BP-09; L-43 |
...
| L-43 | **Limitation, NEW 2026-09-23 (session 17): an OWNER decision, deliberately not decided, and a P0 candidate under the plan's own rule.** On mobile the Asset Library can create, edit and delete assets, while the beta scope and `PRODUCT.md` read "mobile read-only" | Found by the session 17 BP-09 review lane; verified at source by the controller and by the register pass. Nothing has been run on a device | 2026-09-23 | `AssetLibraryRoot.vue`'s `.rp-al-create` button has no `disabled` binding. `git grep -n "Platform.isMobile"` finds no hit under `src/presentation/library/`, and because a grep cannot see a guard injected through deps, `assetLibraryViewDeps()` in `RenovationPlannerPlugin.ts` was read too: it passes no mobile or read-only member. `AssetLibraryCommandServices` (`AssetLibraryDeps.ts`) carries `createAsset`, `updateAsset`, `setAssetHeight` and `deleteAsset`. The `open-asset-library` command is a plain `callback`, where `open-asset-designer` (`assetDesignerCommands.ts`) answers `false` on `Platform.isMobile`. From the mobile project view, `.rp-view-aside__create-asset` is disabled on `context.readOnly` while `.rp-view-aside__open-library` has no such binding (`ViewRoot.vue`). `tests/presentation/views/projectEntryBoundaries.test.ts` "keeps the actual empty mobile launcher readable with only the available Library entry and no creation writes" sets `Platform.isMobile = true`, asserts that the Library door opens an `ASSET_LIBRARY_VIEW` leaf, and asserts nothing about what the opened library offers. The PBI `docs/requirements/Bound the mobile surface to what it can actually do.md` (status Active) lists "The Asset library's write controls" under Still open. This is a source reading: whether Obsidian mobile draws the library at all has not been observed. | Plan BP-09: "Discovery of unsafe mobile mutation becomes P0." Whether this is unsafe is the owner's call, between two remedies: guard the library on mobile, following the `readOnly: Platform.isMobile` the project view already passes (doable here in jsdom, test first and watched red); or narrow the beta scope's mobile claim (Current state's scope row, D-03 and `PRODUCT.md`) to say that the library writes. Either way a device run is needed before any mobile claim is published. As source reads today, BP-09's acceptance clause "mobile cannot bypass the intended write boundary through another entry point" is not met. Revisit when the owner rules. |
...
| G3 — support and first use | Not evaluated | BP-08–BP-11. BP-08 to BP-10 are not started under the plan and BP-11 is partial. L-43, the Asset Library writing on mobile while the beta scope reads mobile read-only, bears on this gate's device-scope claim and waits on an owner decision. |
