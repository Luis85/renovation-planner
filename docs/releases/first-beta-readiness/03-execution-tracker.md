...
| Next executable action | Recorded at the end of the session log below. **As of 2026-09-22 (session 16, continued) it is L-42 — the paste notice that glues a bare count to the sentence after it.** `clipboardActions.ts`'s `pastedNotice()` joins four parts with a space; three carry terminal punctuation and the `·`-joined summary rows do not, so a real `notifySuccess` toast raised by an ordinary paste renders `… Rooms: 2 · Walls: 4 Work, materials, costs and evidence stay with the original. …`. **Strongest candidate on five counts, and it inherits L-38's and L-41's whole argument.** It is MEASURED — found by an agent's unasked check and verified at source in both locales by the controller. It is a **LIVE product defect** in a shipped toast, not a harness artifact. **It mints no word and L-15 does not gate it**, because the remedy is punctuation in code — the property that made both predecessors movable where L-37, L-33's residue and L-36 are not. **Nothing in `tests/` asserts that notice's text**, so the watched-failing red is available and cheap. And it is **the last known member of the category**: an AST census over every `src/` file, tested first against a tree where it is known to fire, finds no fifth — so closing it closes the category rather than chipping at it. **Two things to settle first, neither a blocker.** The remedy is a **judgement about where the summary group ends** — a period after the `·` list, or a different joiner — rather than one character, so decide it against a rendered reading rather than by pattern. And there is a **second latent edge in the same function**: an empty summary makes the inner join the empty string, so the outer join leaves a **double space** after the first sentence; decide deliberately whether to close it in the same edit or record it, rather than meeting it afterwards. **Two alternatives, named as alternatives and not as the action.** (1) **L-38's residue** — the `--partial` marker orphaned on a second line at full-layout width, whose one-character remedy is measured (37.7px to 18.8px) and deliberately NOT taken because it wins by **0.72px**, inside the noise for a font the harness resolved to fallbacks; it becomes decidable the moment anyone runs that surface in a vault. (2) **L-40** — the third diagnostics button is reachable by no harness knob, so it is unphotographed though covered by unit cases at both altitudes; closing it is a harness fixture that can reach the schedule surface, which is its own increment. **L-37, L-33's residue and L-36 are NOT remainder tasks.** **BP-04 is unchanged and still closable against its Acceptance sentence with only the real-screenshot clause outstanding**, which needs a vault run. **G1 stays blocked on L-06, L-19 and L-21.** **The superseded L-41 entry follows.** Recorded at the end of the session log below. **As of 2026-09-21 (session 16) it is L-41 — the run-on sentence a screen reader announces.** `SelectionGuidance.vue` and `CanvasContextMenu.vue` both glue `editor.input.current-target` (`Current target: {target}`, carrying no terminal punctuation) to a full sentence with one bare space, and the first of those is a `role="status"`, visually hidden live region — so a screen reader hears `Current target: Bathroom Alt-click to select another overlapping item.` on every single-zone selection change. **Strongest candidate on the same five counts that carried L-38**, whose argument it inherits wholesale: it is MEASURED rather than suspected — driven live with a `MutationObserver`, not reasoned from source, and found twice independently, by the controller's unasked check and by the implementer's own census; it is a LIVE PRODUCT defect reproducible wherever a single zone is selected, not a harness artifact; **it mints no word and L-15 does not gate it**, because the remedy is punctuation in code exactly as L-38's was, which is the whole reason it is movable where L-37, L-33's residue and L-36 are not; it is two call sites and one character each; and it is the last member of the small category L-38 turned out to belong to. **Check first whether the two sites should share a helper or stay separate** — they are byte-identical expressions in two files, which is the case R-S15-8's measurement released for deduplication, but the SURFACES differ (a visually hidden live region against a `title` tooltip), so take that with a measurement rather than by pattern. **And know before starting what no instrument here can do: the live region is `rp-visually-hidden`, so no capture in this repository can show the fix** — the exact-string unit assertion is the only evidence there will be, which makes watching it fail first the whole of the verification. **Two alternatives, named as alternatives and not as the action.** (1) **L-38's residue** — the `--partial` `::after` marker now sits alone on a second line on three rows at the full-layout width; a one-character remedy is measured (37.7px to 18.8px) and deliberately NOT taken because it wins by **0.72px**, inside the noise for a font the harness resolved to fallbacks, so this becomes decidable the moment anyone runs the surface in a vault. (2) **L-40** — the third diagnostics button is still reachable by no harness knob, so it is unphotographed though covered by unit cases at both altitudes; closing it is a harness fixture that can reach the schedule surface, which is its own increment. **L-37, L-33's residue and L-36 are NOT remainder tasks**: L-37 needs a decision about whether an Obsidian `Notice` may carry an action at all, which is unanswerable outside a vault, and the other two need a minted both-locale sentence that L-15 blocks. **BP-04 is unchanged and still closable against its Acceptance sentence with only the real-screenshot clause outstanding**, which needs a vault run. **G1 stays blocked on L-06, L-19 and L-21.** **The superseded session 15 entry follows.** Recorded at the end of the session log below. **As of 2026-09-21 (session 15) it is L-38 — the Floor Inspector prints two bare numbers with one space between them.** `FloorInspector.vue`s `textFor` glues a formatted value that ENDS in a number to an `editor.inspector.partial` annotation that BEGINS with one, so a plan with refused zone notes reads `Rooms 1 2 could not be read` on three of its five stat rows. **Strongest candidate on five counts.** It is MEASURED rather than suspected — found by looking at a capture and confirmed at source in both locales. It is a LIVE PRODUCT defect reproducible in any vault where a zone note refuses, not a harness artifact: session 15`s L-35 prune made it SHARPER (two adjacent single digits where there were two `2`s) and could not have fixed it. **It mints no word and L-15 does not gate it** — the minimal remedy is a separator in `textFor`, which is code; changing the STRING would be a mint and is blocked, and that distinction is the whole reason this one is movable when L-33`s residue and L-36 are not. It is small, in one function, with the two locale strings already correct. And the state it appears in is now photographable, because `?unreadable=N` reaches it and two permanent captures draw it — so the fix can be SEEN rather than argued. **Check first whether all five stat rows want the same separator**: `Total area` and `Estimated cost` already read correctly because a unit or a word separates them, so a change keyed on the annotation rather than on the row may be the honest shape, and a change that reads well on `Rooms` may read badly on `Total area`. **Two alternatives, named as alternatives and not as the action.** (1) **L-39** — `npx eslint .` exits 1 in a worktree carrying gitignored session scratch, because flat config reads no `.gitignore`; **CI is unaffected** and the fix is one entry in the same global `ignores` block `.worktrees/**` already sits in. Nearly free, and the risk it removes is an agent learning to ignore a gate`s exit code. (2) **L-40** — the third new diagnostics button is reachable by no harness knob, so it is unphotographed though covered by unit cases at both altitudes; closing it is a harness fixture that can reach the schedule surface, which is its own increment. **L-37, L-33`s residue and L-36 are NOT remainder tasks**: L-37 needs a decision about whether an Obsidian `Notice` may carry an action at all, which is unanswerable outside a vault, and the other two need a minted both-locale sentence that L-15 blocks. **BP-04 is unchanged and still closable against its Acceptance sentence with only the real-screenshot clause outstanding**, which needs a vault run. **G1 stays blocked on L-06, L-19 and L-21.** **The superseded session 14 entry follows.** Recorded at the end of the session log below. **As of 2026-09-21 (session 14) it is L-34 — give the four remaining surfaces the door the Plan Editor now has.** `view.project.some-plans-unreadable` (two renderers), `zone.listing-incomplete`, `asset.listing-incomplete` and `view.asset-library.some-unreadable` all tell the user to open the diagnostics report and offer no way to reach it; the last has a button that opens a **note** instead. Strongest candidate on four counts: it is **MEASURED** rather than suspected; `b226b6c67` already built and independently reviewed the composition-root callback seam these would reuse, so the architecture question is settled rather than open; `command.show-diagnostics-report` exists in **both locales**, so **no German is minted** and L-15 does not gate it; and it closes the other half of an inversion session 14 only half-closed. Check first whether each surface`s own composition root can reach `RenovationPlannerPlugin.openDiagnosticsReport()` the same way — **one action, every input** means the fifth door calls the same public method, never a second composition beside it. **Two alternatives, named as alternatives and not as the action.** (1) **L-35** — the new `?unreadable` capture draws zones the strip says are not drawn AND distorts the Floor Inspector through the same fake; the "there is no picture to misread" mitigation is now spent because this session added two, and the round that found it recommends making `mountPlanEditorHarness` **refuse** a discarded knob combination loudly rather than pruning the fixture. (2) **L-33`s residue and L-36 are OWNER copy questions**, both needing a minted both-locale sentence that L-15 blocks, and explicitly NOT remainder tasks. **BP-04 is unchanged and still closable against its Acceptance sentence with only the real-screenshot clause outstanding**, which needs a vault run. **G1 stays blocked on L-06, L-19 and L-21.** **The superseded session 13 entry follows.** Recorded at the end of the session log below. **As of 2026-09-21 (session 13) it is L-33 — the one refusal sentence that answers three different causes.** `OutlinePointsForm.vue:150` renders `editor.resize.invalid` — *"Enter valid dimensions that can describe this room."* — for every whole-outline refusal: an invalid coordinate, a zero area, and **since `e9b982706` a self-crossing outline as well**. It is the strongest candidate on three counts: it is MEASURED rather than suspected, it is small, and **this session's own predicate made an existing copy defect worse**, which is a debt the next session inherits rather than discovers. It is a **copy decision rather than a substitution** — three distinct causes share one sentence, so the honest fix is either a code-to-copy mapping or a genuinely generic both-locale sentence — and it sits in the same instrument gap L-30 just closed one instance of. **Two alternatives, named as alternatives and not as the action.** (1) **The diagnostics-surface inversion, RE-COSTED this session as slice-shaped rather than cleanup-shaped** (R-S13-57): the `WarningAction` id and callback are mechanical, but `showDiagnosticsReport` lives in `src/plugin/` and `presentation/` may not reach it, so the composition root has to inject one more callback — a layer crossing with its own argument, not a tidy-up. (2) **L-23 remains an OWNER decision and is explicitly NOT a remainder task**; session 13's predicate deliberately left it open (R-S13-4). **BP-04's own state moved:** Action 1 is DONE at `3c0e01d21`, so **only the Deliverable's real-screenshot clause now stands between BP-04 and closed**, and that needs a vault run — owner-acceptance territory beside L-19 and L-21. **G1 is unchanged and stays blocked on L-06, L-19 and L-21.** **The superseded session 12 entry follows.** **As of 2026-09-20 (session 12) it is BP-04's Action 1 — the short interaction specification (L-31) — paired with a decision on the three geometry holes.** Action 1 is the one outstanding BP-04 item that can be done from here at all: it is a writing task, it needs no owner and no vault, and with it done the only thing left between BP-04 and closed is the Deliverable's real-screenshot clause, which needs a vault run and is therefore owner-acceptance territory alongside L-19 and L-21. **Beside it, and larger: L-23 is now MEASURED LIVE** — a vertex drag writes a zero-area Zone to the vault while BP-04's own typed dialog refuses to save the same shape — and it should be decided together with **L-29** (a self-intersecting straight outline is refused by nothing at all) and **L-22**, because all three are one geometry-validation question and two of them share a remedy site (`Zone.withGeometry`). **That decision is NOT a remainder-task**: the remedy is a behaviour change at a trust boundary, since a vault already holding a zero-area Zone would keep loading but refuse further edits. The cheap first step is to **drive L-29's gesture**, which is unmeasured where L-23's is not. **The superseded session 11 entry follows.** **As of 2026-09-20 (session 11) it is BP-04 slice B — the reach**: a context-menu entry and `SpatialInspectorActions.vue` wiring, the ONLY thing between BP-04 and closable, and **blocked on the OWNER rather than on work** — it needs an action label in both locales and L-15 forbids agent-minted German. Landing it also makes L-24 moot. If the owner is unavailable, the movable pieces are **L-23's measurement** (whether any live path reaches `MoveSpatialObject` with a degenerate outline — the cheap experiment that decides latent versus live) and **L-22**, which starts by exporting `validateCurvedBoundary` and needs a realistic-geometry case produced first. **The superseded session 10 entry follows.** As of 2026-09-20 (session 10) it is **BP-04 slice A2 — per-corner selection and highlight**, which is what BP-04's interaction contract and its action 3 actually ask for and what slice A deliberately did not build. It lands in **two files no budget table has ever named**: `src/presentation/editor/tools/render-state.ts`, which has no per-vertex field at all, and `src/presentation/editor/layers/InteractionLayer.vue`, whose `editableVertices` draws every handle uniformly. Slice A (2026-09-20, `6546f402c`) shipped the whole-outline numeric editor underneath it and is reachable only from a test. **Slice B — the reach — is NOT the next action and is blocked on copy**, not on work: its context-menu entry needs an action label, and **L-15 forbids agent-minted German**, so it needs the release owner. A2 needs no new string and is therefore the piece that can actually move. **G1 remains blocked on something that is NOT work:** a release-owner decision on what `onunload` owns (L-21), which is what five of BP-03 F3's six rows are blocked behind. BP-03's five numbered gaps are otherwise all closed or accepted. That decision and the two other open owner questions (L-06 with L-11 folded in, and L-19) are assembled for one sitting in [`05-owner-decisions.md`](./05-owner-decisions.md), which also records that the WORKED EXAMPLE behind ADR-0034's stated cost for the L-06 closing option does not hold: `ConstructionMaterialCommand` re-raises the stamp and is guarded on both doors, so that stamp already becomes a durable incident today. The DIRECTION of that cost is unchanged and no other example has been costed. |
...
| L-40 | **Limitation, NEW 2026-09-21 (session 15).** One of the three new diagnostics buttons is reachable by no harness knob and is drawn by nothing that re-runs | Found by session 15`s implementer as a STOP; verified four ways by the independent reviewer | 2026-09-21 | `ProjectWorkState.vue`s **Show diagnostics report** button cannot be photographed. Four independent locks: `mountHarness`s `section` is a two-value union admitting only `details` and `prices`; `page.ts` cannot emit `schedule`; `defaultRenovationProjectDeps` supplies no `work` member, so the whole `v-if` block never renders; and `navigate` forwards `section` only when it is `'prices'`. Reaching it is a fixture slice, not a knob. The sibling detail-state button IS captured (`project-detail-unreadable`, and at 460), and **that capture is what found the full-pane-width layout defect** the fix round then cleared — so the unphotographed surface is the one where that class of defect would go unseen. | The button is covered by unit cases at both altitudes, so it is not untested — it is **unphotographed**, and CLAUDE.md records ten defects the captures caught that no other gate could, every one a layout measurement jsdom cannot make. Closing it means a harness fixture that can reach the schedule surface, which is its own increment. Recorded rather than absorbed. |
...
| L-42 | **Limitation, NEW 2026-09-22 (session 16), and it is a LIVE user-facing toast.** A paste notice glues a bare count to the sentence after it | Found by the L-41 implementer's unasked check; verified at source in both locales by the controller; confirmed as the category's only remaining member by an AST census | 2026-09-22 | `editor/clipboard/clipboardActions.ts`'s `pastedNotice()` joins four parts with a space. Three carry terminal punctuation; the `·`-joined summary rows (`Rooms: {count}`, `Objects: {count}`, …) do not. So the notice renders `… Rooms: 2 · Walls: 4 Work, materials, costs and evidence stay with the original. …` — **a count followed directly by a word, which is L-41's exact shape** — in a real `notifySuccess` toast raised after an ordinary paste. German identical in structure. **Nothing in `tests/` asserts that notice's text.** A second, latent edge: an empty summary makes the inner join the empty string, so the outer join leaves a **double space** after the first sentence | **Not fixed, and the decision is recorded rather than deferred silently.** The remedy is a judgement about where the summary group ends — a period after the `·` list, or a different joiner — rather than one character, and it deserves its own watched red. **The remedy would be punctuation in code, so it mints no word and L-15 does not gate it**, exactly as L-38 and L-41 did not. **This is the last member of the category**: an AST census over all `src/` files (template literals, `+` concatenation, `.join` over tr-bearing arrays, adjacent SFC interpolations), **tested against the pre-fix tree first, where it finds both L-41 sites and nothing else**, returns 13 hits on the fixed tree — twelve verified benign against the locale tables, and this one. The census flags a SHAPE, so a member joined by a mechanism it does not model would still be missed |
...
| G2 — core journey | **UPDATED 2026-09-22 (session 16, continued). L-41 is CLOSED and the category it belongs to is now MEASURED closed at four.** `ee7d4b3ff` with `0f5e0e4f6`: both copies of the run-on moved into one pure function joining the two keys with `. ` — punctuation in code, **neither locale string touched, L-15 never engaged** — and `CanvasContextMenu`'s `title` computed, which existed only to feed its copy, is gone with it. CI run `35730335714` **success on all five jobs**. **The census changed the scope before any brief was written**: the job was SEVEN sites, not two, because five existing assertions rebuilt the glued expression to compare against it — the same shape as the `toContain('2')` that failed to catch L-38 — and the live region's single-selection text was asserted by **nothing**. **A review finding was REFUTED rather than fixed, exposing a FALSE RATIONALE**: the module justified its parameter by a watcher holding a value the store had moved past, and a probe of exactly that scenario shows Vue 3.5.41 coalescing two writes into one firing with the two values reference-identical — **reproduced independently by the controller**, so the code was right either way and the reason given for it was wrong. **Generation SIX of the wider-than-its-check overclaim landed one file from where the brief forbade it**, in the test docblock; six for six, every generation after the first introduced by the commit fixing the previous one; all three shapes deleted rather than corrected. **The category is closed at four by an AST census over every `src/` file, tested against the pre-fix tree first**, where it finds both L-41 sites and nothing else — a negative result worth more than a finding. Its one real remaining hit is **L-42**. **Two controller defects, both caught**: a duplicate implementer dispatched because a notification's status was read and its "may be interim" note was not, and a local gate diagnosed — by reading command lines, not counting processes — as starved by the **peer session's** three concurrent jobs rather than by this change; **nothing of the peer's was touched** and the remedy was the documented workflow, CI is the report. **No threshold, budget, ignore or assertion was weakened.** **BP-04 is unchanged.** G2 still waits on BP-05 to BP-07. **Nothing here has been run in an Obsidian vault, and no screen reader has ever been pointed at this branch.** The superseded session 16 text follows. **UPDATED 2026-09-21 (session 16). Two limitations closed, and the session's own value was in re-measuring claims rather than in the two lines it shipped.** **L-38 is CLOSED** (`13cd46975`): `FloorInspector.vue`'s `textFor` brackets the unreadable-count annotation instead of spacing it, so `1 2 could not be read` reads `1 (2 could not be read)` — punctuation in code, **no word minted in either locale**, L-15 never engaged. **L-39 is CLOSED** (`9db5fc7f8`): one entry in `eslint.config.mjs`'s global `ignores`, taking `eslint .` from exit 1 to exit 0, with scope proved over the whole linted SET (2290 to 2286 files, exactly the four removed) rather than by sampling, and `.oxlintrc.json` deliberately untouched because oxlint reads `.gitignore` and flat config does not. **A STOP fired on this tracker's own L-38 row before any work began**: it called `Estimated cost` a row that reads correctly because a word separates the parts, when that row is typed `Aggregate<never>` and can never reach the partial branch at all — **four rows, not five**. **Three reported claims were re-measured and two refuted**, one of them the controller's own: the vitest run WAS narrowed (403 files is exactly `tests/presentation/editor`, not a superset, so nothing outside it was exercised locally); NOT every separator costs the same (a comma fits the column, parentheses do not — the ruling stands on semantics, because a comma reads as a list); and `white-space: nowrap` DOES fit, contrary to the review, though the remedy is still not taken because the winning margin is 0.72px on a fallback font. **The captures were run and LOOKED AT** — and `plan-editor-unreadable-narrow.png` turns out not to show the Floor Inspector at all, because at 460 the Details overlay is closed. **A controller defect was made and caught**: `s14-rows.mjs` is a WRITER, not a checker, and re-appended three stale rows to this document; reverted by explicit path and the table re-verified identical. **L-41 is new** — two sites glue two strings into a run-on that a screen reader announces, found by a check nobody asked for and verified live. **One latent exposure recorded and deliberately not fixed**: `harness-shots/` is gitignored and absent from ESLint's `ignores`. **No threshold, budget, ignore or assertion was weakened.** **BP-04 is unchanged.** G2 still waits on BP-05 to BP-07. **No full `npm run check` was run locally, deliberately, and no CI run was cancelled.** **Nothing here has been run in an Obsidian vault.** The superseded session 15 text follows. **UPDATED 2026-09-21 (session 15). The diagnostics-surface inversion is now CLOSED AS NARROWED, and the narrowing is the finding.** L-34 framed four shipped strings as four surfaces; a read-only recon traced each and **two of them are not surfaces at all** — `zone.listing-incomplete` and `asset.listing-incomplete` are raised by `ListReassignmentTargets` and end at `new Notice(string, 0)`, a toast whose whole payload is a resolved string. They moved to **L-37** rather than being absorbed. The other two shipped their button (`cf6b39fac`, `c4c60f11c`): two required deps members injected by the composition root on the `planEditorDeps` seam, three controls, **no new component and no new locale string**, gated on the SOME arm because `all-plans-unreadable` names no report. **L-35 is CLOSED** (`780e562dc`), and session 14 had refused it: two of that refusal`s four cited couplings do not exist, and the real `findZonesByPlan` reads `structure` from the per-plan geometry sidecar — so keeping it whole under a prune is what a real vault does. **A costing is a hypothesis too**, and a refusal that goes unchallenged looks exactly like a settled one. **`npm run check` went RED at `cf6b39fac` on all four verify legs, at `analyze`**, and was found by the independent reviewer`s UNREQUESTED check on a commit already reported successful — the implementer`s report said four gates were run and listed three. **The controller`s proposed fix was then refuted by arithmetic**: wrapping inline moves a `v-if` to nesting 2 where it costs more, taking one file 17 → 18. Extracting the pair into one component both surfaces render with no `v-if` at the call site took both files 17 → 13. **No threshold, budget or ignore was touched.** **A FIFTH generation of the wider-than-its-check overclaim was found, in THREE homes**, one of them outside the commit under review — written by the commit that correctly deleted generation four. All three closed by deletion; no count was replaced with a newer count. **Four new limitations, none found by a gate: L-37** (a toast cannot carry an action), **L-38** (the Floor Inspector glues two bare numbers — a live product defect found by looking at a capture, which L-35 made sharper rather than better), **L-39** (`eslint .` exits 1 locally on gitignored scratch; CI unaffected) and **L-40** (one of the three new buttons is reachable by no harness knob, so it is unphotographed though not untested). **BP-04 is unchanged.** G2 still waits on BP-05 to BP-07. **No full `npm run check` was run locally this session, deliberately, and no CI run was cancelled.** **Nothing here has been run in an Obsidian vault.** The superseded session 14 text follows. **UPDATED 2026-09-21 (session 14). L-33 is CLOSED** (`b512f2db6`), and the fix was not the shape its row proposed: the code-to-copy mapping was costed and **refused on measurement** because it buys nothing user-visible, and what shipped is a one-line key swap to the both-locale `error.category.geometry` — **recorded as a deliberate departure from a declared fallback-tier convention, not as a clean win**, with the minted-sentence alternative surfaced as an owner question. Its measurement fired a STOP that **prevented a regression**: `editor.resize.invalid` has THREE render sites and is CORRECT at two, so editing the string would have broken the room width/depth dialog; `5f3895acd` now pins both correct sites, after a reviewer swapped the key at both and 179 tests stayed green. **The diagnostics-surface inversion is HALF closed** (`b226b6c67`): the Plan Editor`s `unreadable-zones` row has its door, injected as a callback from the composition root because `presentation/` may not reach `src/plugin/`, landing on the same public method the palette command and the settings row already call — **no locale string minted**. **L-32 stands unchanged.** **BP-04`s test figures are SETTLED by re-running the audit: 10 covered / 2 partial / 0 absent, confirmed on this tree**, moving that reading from a derivation nobody checked to a measurement — though item 3`s recorded *reason* is now stale while its verdict holds. **Three new limitations, and none of them found by a gate: L-34** (four more shipped surfaces name the diagnostics report and offer no way to reach it), **L-35** (the new `?unreadable` capture draws zones the strip says are not drawn and distorts the Floor Inspector through the same fake) and **L-36** (English describes one form`s two axis labels in two registers, an owner copy question left undecided). A lint rule`s **vocabulary** was widened rather than its copy bent (`63ecd9fd9`), after the controller`s own typo ruling was measured false — an uppercase `X` passes the marketplace sentence-case rule only because a social network is named X. **One German string was touched, by exactly two characters, under a narrow recorded ruling** that permits a case change to a standalone Latin axis letter and nothing wider. **FOUR generations of the same wider-than-its-check overclaim were found and closed this session**, the last three each introduced by the commit fixing the previous one, and the fourth closed by **deletion** rather than correction (`2ce9b424e`). G2 still waits on BP-05 to BP-07. **No full `npm run check` was run locally this session, deliberately — CI on the pull request is the report, and no CI run was cancelled.** **Nothing here has been run in an Obsidian vault.** The superseded session 13 text follows. **UPDATED 2026-09-21 (session 13).** **One of BP-04's two unreachable-from-here items is now DONE**: Action 1 has its artifact (`docs/superpowers/specs/2026-09-21-bp04-numeric-corner-editing-spec.md`, `3c0e01d21`), so L-31 is half closed and **the Deliverable's real-screenshot clause is the only BP-04 item left that cannot be done from here** — it needs a vault run. **L-29's WRITE side is NARROWED, not open and not closed**: all four editor write doors now refuse a self-crossing outline (`e9b982706`, `7c38db05c`, `0ad89aea3`, `74cfe8647`), with CI success at `7c38db05c` (run 35577104683) and `0ad89aea3` (run 35585566311) and `74cfe8647`'s run 35589022563 **still in flight when this row was written**. What L-29 does NOT cover: the asset designer's own two doors, SDD §26's "shown spatially" clause which nothing satisfies, and any detection, migration or diagnostic for crossing outlines already in vaults — so a Requirement figure derived from one is **still wrong and still silent**. **L-30 is CLOSED** (`78b851ab2`). **L-23 and L-22 are untouched and stand**, and L-23 was left open deliberately so that a predicate would not decide an owner question by accident. **Two new limitations were opened by LOOKING rather than by a gate: L-32** (no capture in this repository has ever shown a dialog's actions row) and **L-33** (one sentence answers three different whole-outline refusals, and this session's own predicate added the third). G2 still waits on BP-05 to BP-07. **No full `npm run check` was run locally this session, deliberately, under the parallel-work rule — CI on the pull request is the report.** **Nothing here has been run in an Obsidian vault.** The superseded session 12 text follows. Not evaluated, and **not advanced to a user** yet. **BP-04 is now reachable in the UI from two doors and all three slices are CI-green** (`1f2cf7cf8`, `73af5eb95`, `2cf585a40`), so the sentence this row carried for two sessions — "nothing in the UI opens either" — **is no longer true and L-24 is closed**. What G2 still waits on is BP-05–BP-07, plus BP-04's own two unreachable-from-here items (L-31: no interaction specification, and a real-screenshot clause needing a vault run). **Three geometry holes were measured this session and none is fixed** — L-23 is LIVE (a vertex drag writes a zero-area Zone to the vault), L-29 is new and refused by nothing at all, L-22 stands; together they are one geometry-validation decision with a trust-boundary cost. The superseded text follows. BP-04 slices A **and A2** are implemented and CI-green, and **nothing in the UI opens either** — the reach is slice B, which is blocked on the OWNER | BP-04–BP-07. BP-04's domain half ships already (`ReversibleMoveZoneCommand`), so the package is smaller than the plan assumed in that direction and LARGER in another: **that larger half is now BUILT** — session 11's slice A2 added the per-corner selection state BP-04's contract needs, in `tools/render-state.ts` and `layers/InteractionLayer.vue`, two files no budget table ever named. Slice A delivered the whole-outline numeric path (`6546f402c`, `152a3c18c`); **slice A2 the chosen-corner list and the canvas highlight (`8bfd6dd6a`, `b6b6a1f27`, `e2d524a9b`)**; slice B is blocked on OWNER COPY rather than on work (L-15), and it is the **only** thing left before BP-04 is closable. **A2's own limitations, found by capture and by nothing else: L-26** (the corner list is additive, so the dialog roughly doubled in height) and **L-27** (at a sidebar's width the modal covers the canvas, so the highlight cannot be seen at the one width BP-04's test case 12 is about). L-24 records that no gate can see slice A's unreachability in either direction, and L-25 that the German half of its reused title key has no instrument. L-22 and L-23 remain open, neither caused by BP-04 and neither fixed. **Nothing here has been run in an Obsidian vault.** |
