...
| L-38 | **Limitation, NEW 2026-09-21 (session 15), and it is a LIVE PRODUCT defect rather than a harness one.** The Floor Inspector glues two bare numbers together with one space | Found by the controller looking at a capture; confirmed at source and again after L-35 landed | 2026-09-21 | `FloorInspector.vue`s `textFor` glues the formatted value and the ${count} annotation with a single space, and `editor.inspector.partial` is `{count} could not be read` in EN (`{count} konnten nicht gelesen werden` in DE, identical shape). So an annotation that BEGINS with a number is glued to a value that ENDS with one. Three of the five stat rows collide — `Rooms`, `Areas` and `Planned changes` are bare counts; `Total area` and `Estimated cost` carry a unit or a word. **Reproduces in any vault where a zone note refuses; the harness fake neither caused it nor can fix it.** L-35 made it sharper rather than better: the row read `Rooms 2 2 could not be read` before the prune and `Rooms 1 2 could not be read` after, two adjacent single digits. | Visible on one panel, in a state that only occurs when notes refuse. **The minimal remedy is a separator in `textFor` — code, not copy — so it mints no word in either language and L-15 is NOT engaged.** Changing the string would be a mint and is blocked. Not scoped by session 15, which was working L-34 and L-35; recorded rather than absorbed so it is not re-discovered. |
| L-39 | **Limitation, NEW 2026-09-21 (session 15), and it is about a LOCAL instrument rather than the product or CI.** `npx eslint .` exits 1 in a worktree carrying session scratch | Found by session 15`s fix round while verifying gates | 2026-09-21 | `.superpowers/` is gitignored and holds `.mjs` helper scripts. **Flat config reads no `.gitignore`**, so `eslint .` walks into it and reports findings; `--max-warnings 0` then turns that into exit 1. **CI is unaffected** — `git ls-files .superpowers` returns 0, so the directory is never checked out. Excluding that one directory gives exit 0. **This is the identical trap `.worktrees/**` is already in `eslint.config.mjs`s global `ignores` for**, and that block`s own comment states the reason in as many words. | No product effect and no CI effect; the cost is entirely to a local gate`s credibility. **The real risk is not a phantom chase but the opposite** — an agent learning that this gate`s exit code can be ignored. The fix is one entry in the global `ignores`, and it is a gate-configuration change that was deliberately NOT absorbed into a round whose subject was a different red. |
