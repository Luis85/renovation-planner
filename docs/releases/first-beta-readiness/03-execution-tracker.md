...
| L-33 | **Limitation.** One wrong sentence answers THREE different whole-outline refusals, and **this session's own predicate made it worse** | Found 2026-09-21 by BP-04 Action 1's implementer, reported and deliberately not fixed | 2026-09-21 | `OutlinePointsForm.vue:150` renders `editor.resize.invalid` — *"Enter valid dimensions that can describe this room."* — for **every** whole-outline refusal. It says **"dimensions"** in a dialog that takes coordinates, **"room"** in a dialog that opens on a Garden, and **since `e9b982706` it is also what a self-crossing refusal reads** (R-S13-46). So L-29's predicate routed a third distinct cause into a sentence that was already wrong for the first two — which is exactly the kind of consequence a slice should report about itself. **Same instrument gap as L-30:** a correctly-spelled key carrying the wrong words, invisible to `I18N_LITERAL_BAN` (which passes a `tr(...)` call) and to `localeModuleSentenceCase` (which checks case, not meaning). | **Not fixed this session, deliberately, and unlike L-30 it is not a label swap.** Three distinct refusal causes — invalid coordinate, zero area, self-crossing — share one sentence, so the honest fix is either a code-to-copy mapping or a genuinely generic both-locale sentence: a **copy decision** rather than a substitution. Whether it needs an owner depends on which of those is taken; a generic both-locale sentence may be reachable by reuse the way L-30 was, and a per-code mapping mints copy. Recorded with its measurement and named as the strongest candidate for the next session's first action. Beside it, and smaller, is the coordinate-hint nit the same round measured: `editor.area.coordinates-hint` EN writes `X` beside a lowercase `y`, and the controller's own reading of `plan-editor-outline-narrow.png` (R-S13-61) sharpens that from "EN differs from DE" to **the only lowercase axis letter in a frame that shows an uppercase `Y` eleven times**. |
