...
| G2 — core journey | Not evaluated, and **not advanced to a user** yet. BP-04 slice A is implemented and gated-green, and **nothing in the UI opens it** — the reach is slice B | BP-04–BP-07. BP-04's domain half ships already (`ReversibleMoveZoneCommand`), so the package is smaller than the plan assumed in that direction and LARGER in another: no per-corner selection state exists anywhere, and BP-04's contract is per-corner — that is slice A2, the next action. Slice A delivered the whole-outline numeric path (`6546f402c`, `152a3c18c`); slice B is blocked on OWNER COPY rather than on work (L-15). L-24 records that no gate can see slice A's unreachability in either direction, and L-25 that the German half of its reused title key has no instrument. L-22 and L-23 remain open, neither caused by BP-04 and neither fixed. **Nothing here has been run in an Obsidian vault.** |
