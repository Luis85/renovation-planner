...
| Next executable action | Recorded at the end of the session log below. As of 2026-09-19 (session 9) it is **BP-04 slice A — one `createRoomEditAction` whose `accepts` covers Room AND Area, mounting the existing `OutlinePointsForm`, templated on `roomResizeAction.ts`** — which is now the next piece of WORK, because the package's domain half already ships (see the BP-04 register row). Its brief must carry the three corrections the independent review made to discovery, above all that **no per-corner selection state exists anywhere** (`render-state.ts` has no per-vertex field and `OutlinePointsForm.vue` renders every corner at once) while BP-04's contract is per-corner — the package's largest unbudgeted work. **G1 remains blocked on something that is NOT work:** a release-owner decision on what `onunload` owns (L-21), which is what five of BP-03 F3's six rows are blocked behind. BP-03's five numbered gaps are otherwise all closed or accepted. That decision and the two other open owner questions (L-06 with L-11 folded in, and L-19) are assembled for one sitting in [`05-owner-decisions.md`](./05-owner-decisions.md), which also records that the WORKED EXAMPLE behind ADR-0034's stated cost for the L-06 closing option does not hold: `ConstructionMaterialCommand` re-raises the stamp and is guarded on both doors, so that stamp already becomes a durable incident today. The DIRECTION of that cost is unchanged and no other example has been costed. |
...
| G2 — core journey | Not evaluated. **BP-04 is opened with discovery only — no production line changed** | BP-04–BP-07. BP-04's domain half turns out to ship already (`ReversibleMoveZoneCommand`), so the package is smaller than the plan assumed in that direction and LARGER in another: no per-corner selection state exists anywhere, and BP-04's contract is per-corner. Two code findings surfaced beside it, L-22 and L-23, neither caused by BP-04 and neither fixed. |
