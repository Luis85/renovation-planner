...
| L-41 | **Limitation, NEW 2026-09-21 (session 16), and it reaches a SCREEN READER.** Two shipped strings are glued into one run-on sentence at two sites, one of them a live region | Found by the controller's own unasked check during L-38 and independently rediscovered by the L-38 implementer's census; verified LIVE, not reasoned from source | 2026-09-21 | `SelectionGuidance.vue`'s `selectionGuidance()` and `CanvasContextMenu.vue`'s `targetGuidance` both join `editor.input.current-target` to `editor.input.overlap-cycle-guidance` with a bare space. The first is `Current target: {target}` (`Aktuelles Ziel: {target}`) and carries **no terminal punctuation**, so a full sentence is glued straight onto it. Driven live with a `MutationObserver` on `.rp-selection-guidance` across two room-row selections: `Current target: Bathroom Alt-click to select another overlapping item.` That element is `role="status"` and `rp-visually-hidden`, so **this is what a screen reader announces on every single-zone selection change**; the `CanvasContextMenu` copy reaches a `title` tooltip instead | Same family as L-38 and a DIFFERENT collision — a sentence run-on rather than two adjacent digits — at two sites rather than one, which is why it was recorded rather than absorbed into L-38's commit. **The remedy is punctuation in code, so no word is minted in either language and L-15 is NOT engaged** — the same property that made L-38 movable. **No capture in this repository can show it**, by construction: the live region is visually hidden. Not fixed. Whether a run-on in an announcement is worth an increment is the next session's or the owner's call, and this row deliberately does not make it |
...
### Session 16 — 2026-09-21 — L-38 and L-39 closed, and two claims refuted by re-measuring them
