...
| A26 | Mobile read-only, restored desktop tabs | F2/F5 | Promised data is readable; writes are guarded across entry points; unsupported editor tabs show a safe fallback; desktop remains usable afterward. The Asset Library's writes are guarded on mobile by owner decision (`03-execution-tracker.md` L-43), in code and in jsdom; no device has observed it. | Real device/Obsidian observations | BP-09 |
