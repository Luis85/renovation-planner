...
| Stale read-back | `ProjectStore.ts:244` `stale`, assembled into `writesBlocked` at `runtime.ts:607-609` | per-mount Pinia | **No — a remount silently clears the refusal**, then re-derives it from a fresh hydrate |
...
| F2 | 3 | `ProjectStore.ts:244` `stale` is per-mount Pinia, so a settings change clears an active write refusal and re-derives it only from a fresh hydrate | **UNVERIFIED whether the fresh hydrate reproduces it.** Needs an experiment, not a read |
