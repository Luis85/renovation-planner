...
Everything else in this backlog needs something to hang from. The domain model in §6 makes
Project the root, and §83 puts on it the things every downstream calculation reads: the
...
It is also the whole of what a first-time user sees. §93's onboarding and §94's empty states
land here, and §95's example project is how somebody decides in two minutes whether this tool
is for them.

The boundary worth naming: §12's project dashboard is *this* project's status and the way in
...
Derived from PRD §12 (Epic 1), with the settings model from §83 and the folder layout
from §36.
...
- The Project note carries the settings §83 assigns to project scope — currency, units, tax
  ...
  difference.
- Every folder path is configurable (§36) and every user-supplied path passes through
...
- Status, budget and date range are frontmatter a human can read and edit in any editor,
  because §44's portability requirement says the vault stays useful without the plugin.
- A Project note renamed, moved or edited outside the plugin is picked up rather than
  overwritten (§65).
- Opening a vault with no project shows something that says what to do next (§94), not an
  empty pane.
