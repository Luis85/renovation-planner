...
```bash
# The verifier is COMMITTED, at
# docs/reviews/2026-08-24-ux-layer-backlog-reconciliation/verify-dod.sh, and this plan points at
# it rather than carrying a second copy. It used to be written out here and run from a
# scratchpad, which meant a clean checkout could not run one of the gates the ledger advertises
# — the whole harness was a claim wider than its mechanism, which is the defect this pass exists
# to find. Two copies would rot the same way every duplicated count in this ledger has.
bash docs/reviews/2026-08-24-ux-layer-backlog-reconciliation/verify-dod.sh
