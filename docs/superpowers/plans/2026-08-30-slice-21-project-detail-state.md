...
| 12 — the `open-project-detail` command | ✅ complete — commit *"Add a palette command that goes to a project"*. No sha, for the reason Task 9's row gives. Two deviations from the brief, both stated in that commit's message: `planEntries` and `projectEntries` are ONE `entriesOfType(index, type)` in a new `src/plugin/indexEntries.ts` rather than two filters — the brief offered that as an alternative — and the modal's cases live in `tests/presentation/modals/projectSuggestModal.test.ts` because `tests/presentation/modals/` did not exist, the plan picker's identical four being in `tests/presentation/editor/units.test.ts`. Eight behavioural cases, not four: the brief's four plus an unrecovered-settings twin of the empty-vault case and three the brief's floor omitted. **The brief's third case did not discriminate as written** — see that commit's message and the report |
| 13 — the documents whose stated triggers this slice fires | ✅ complete — commit *"Record what slice 21 changed, and walk what no gate can see"*. No sha, for the reason Task 9's row gives. Three deviations, all stated in that commit's message: the harness reaches the detail state through `view.setState` rather than through `deps.projectId` (`mount` writes the view's own field over that member, so a bundle naming a project is silently ignored); `project-detail-narrow` is taken in the LIGHT scheme, so two shots hold both palettes — the status label measures 6.69:1 in light against 8.13:1 in dark; and the capture found two CSS defects, which are fixed in the same commit rather than reported |
