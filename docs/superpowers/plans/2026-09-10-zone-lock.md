...
Run: `npm run harness-shot -- --width=460` and open the `plan-editor` captures in `harness-shots/`. Confirm the lock button sits right of each Rooms and areas row without wrapping the area figure, in both schemes. This is a look, not a gate; note what you saw in the commit body.
