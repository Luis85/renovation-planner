...
| F02 composed vanity | **not-run — a written case exists, unwalked (AD15-R2)** | A whole-workflow fixture is a vault scenario, not a unit fixture — **and the instrument it asks for EXISTS**: `docs/tests/cases/Compose an asset from parts.md`, written and unwalked, so this takes U05's grade rather than `none`. Measured 2026-09-22: the string `vanity` appears nowhere in `src/`, `tests/` or `docs/tests/`. **A builder was REFUSED rather than overlooked** — with no consumer it is a dead export `npm run analyze` fails on, and with one that consumer re-drives `arrangeDetails.test.ts` and `groupEdits.test.ts` under a themed name, which is AD15-R1/T20's losing side verbatim. **AMENDED 2026-09-22 (wave 17, ruling AD18-R8): the fixture now EXISTS, and it is a preset rather than a builder.** The user authorized a `vanity` preset, which is wider than §4 row 2's *adopt as a test fixture* and is recorded as added scope. That does not overturn the refusal above — it removes both premises it rested on: a preset lives in `src/domain/asset/presets/sanitary.ts` with `ASSET_PRESETS`, `AssetPresetForm.vue` and `presetThumbnail` as real consumers, so it is no dead export, and it is driven by `presets.test.ts`'s existing `describe.each` rather than by a themed re-drive. So the fixture this row asks for is `ASSET_PRESETS.find((p) => p.id === 'vanity')`, and the *string `vanity` appears nowhere* measurement above is superseded. **The grade does not move**: what keeps this row `not-run` is that `Compose an asset from parts.md` is still unwalked, which is the deferred manual pass's to discharge and not this fixture's |
...
| F10 one definition in two projects, one frozen revision | **passed — regraded 2026-09-22 (wave 16)** | The two-project half is **asserted** since W16-A’s sibling card — `listPlansUsingAsset.test.ts` *"names the plans of EVERY project that places the asset, each row carrying its own project"*, driven over the real `ObsidianProjectRepository` and `ObsidianPlanRepository` with two saved projects, watched red against a truncated project walk that left the other seven cases green. **The frozen half is not owed**: the row says *"where supported"*, and `r1` row 3 settles that nothing in this product can be frozen or approved, so writing that test would be the defect AD15-R1/T20 already refused. **`passed` is about the fixture and the query and NOT about what a user can tell apart** — of every `src/` file importing `AssetPlanUsage`, the only one naming `projectId` is the query that produces it, so both usage panels draw plan name and placement count only and two plans both named `Kitchen` in different projects render identically. That is a recorded `src/` finding, not a gap in this row’s evidence |
...
| F12 25/250/1000 parts | **partial — regraded 2026-09-22 (wave 16)** | The fixture family exists at all three sizes since W16-A — `shapeWithParts` in `tests/helpers/assetShapes.ts`, driven by `tests/domain/asset/partFixtures.test.ts`, with part, vertex and curved-edge counts documented in prose and asserted against a hand-written literal table that shares no code with the builder, and every size accepted by the real `validateAssetShape` (checked at 1000 FIRST, in case the domain refused a shape that large: `validateDetails` iterates with no count cap and `validateAssetShape` never reads `details.length`). 25/250/1000 parts carry 92/917/3667 vertices and 32/332/1332 curved edges. **The measurement half is untouched and stays `not-run`**: §6’s conditions are a warmed renderer, recorded hardware and a leaf width, and there is no benchmark harness, no host and no compositor here. **What the family does NOT measure is recorded rather than implied** — overlap and occlusion, per-part path depth (four vertices against a traced outline’s dozens), groups, clearance and pending coordinates |
...
| U05 recover rather than lose work | **not-run — a written case now exists (wave 15)** | Needs a host. `docs/tests/cases/Recover an asset design rather than lose it.md` walks the five clauses that are not the two-leaf one (T12 owns that, and the two cases cross-reference rather than repeat). **The "two real leaves" in this row's old reason turned out not to be needed for any clause here** — the case works in one leaf throughout. **Its post-write-refresh clause is DISCHARGED BY A PROCEDURE rather than narrowed, and that reversal is the wave's main finding**: it first read as unwalkable, because every geometry write reads both resources the read-back reads, so any fault breaking the read has already refused the write — correct about every obvious fault, and falsified by an asymmetry. Nothing on the write path derives what the read path derives (`validateAssetShape` calls `dimensionsOf` zero times; `GetAssetDesign` derives it twice), so a schema-valid clearance whose SPAN is not representable is written happily and refused on the way back, which is `getAssetDesign.test.ts` "refuses a clearance whose span overflows rather than reporting Infinity" met from the vault side. Steps 12a–12d walk it, through **Remove reference** rather than a drag — after a failed refresh an ordinary gesture carries the stale version the store still holds and is refused, so the write lands only through a door supplying no expected version. **Nobody has executed that sequence**; the rows are written so that "the write did not land" is a recordable finding. Stays `not-run` until walked |
...
| T05 canonical values survive unit change and fractional editing | **partial — narrowed 2026-09-22 under AD15-R2** | The mm-canonical and fractional-input halves are asserted (`dto/assetGeometry.test.ts` "refuses a unit that is not mm", "keeps three decimals, which is what catches a YAML float coercion"; `decimalInput.test.ts`). **The display-unit half is unasserted because there is no display-unit feature** — `worldUnit.test.ts` states only that one world unit is one millimetre. The row assumes a capability the product does not have. **The display-unit half assumes a NAMED FUTURE increment, and the setting that looks like it is INERT.** Measured three ways and all three agree: `\.units\b` over `src/` reaches only `src/plugin/settings/` and the two locale tables that label the row; a destructured `{ units } =` has ZERO hits; and the `Units` type is declared in `settings.ts`, named in one COMMENT in `ProjectIndex.ts`, and referenced nowhere else. The display path is hard-coded in the other direction — `formatLength.ts` and `formatArea.ts` each build an `Intl.NumberFormat('en-US', …)` and both docblocks name *"the per-plan units PBI"* as the increment that would make that locale a variable. **`settings.units` binds a control, persists through `saveSettings` and is read by nothing**, which is recorded in AD15-R2 as out of this package's scope rather than acted on |
...
| T08 undo/redo cannot overtake write/read-back | **partial — narrowed 2026-09-22 under AD15-R2** | Asserted densely (`designerWriteChain.test.ts` "undo clicked while a second drag is still being written undoes THAT drag, not the one before it"; `editShape.test.ts`; `reversibleAssetDesignWindows.test.ts`). **Narrower in LAYER**: the row says session/fault injection and the faults are injected at fake ports rather than at a session boundary. AD15-R2: **the guarantee is at the PORT layer and this sentence says so.** There is no session boundary in this environment to inject at, so the row's stated layer is unreachable rather than skipped. No losing side was available |
...
| T12 two leaves, expected-version conflict | **partial — a written case now exists, unwalked (wave 15)** | Conflict behaviour asserted (`setAssetFootprint.test.ts` "refuses the second of two writes built from the same revision, rather than losing one"; `designerWriteChain.test.ts` "stays conditional on the design it was replayed on, so a peer write during the live replayed drag refuses it"; `designerSelectTool.test.ts` "moves a dragged detail in one write, conditional on the version the press read"). **CORRECTED 2026-09-21 (wave 15): this row cited `designerCrossLeaf.test.ts` for conflict behaviour and that file contains no conflict case at all** — its six cases across two describes are bus delivery, per-asset filtering, a closed leaf, listener disposal and repeated open/close, and its only two matches for `conflict|expected` sit inside a docblock about leaked Konva stages. Reached independently by the card, its reviewer and the integrator, each by listing every `it(` rather than trusting the file name. **True about a behaviour, wrong about a file** — the behaviour was asserted all along, elsewhere. **The row still demands a real host and there is still no real-host test anywhere in `tests/`** — every leaf here is a `FakeLeaf` that records asks rather than behaving. `docs/tests/cases/Two designers on one asset.md` is now the instrument, and its own step 1 asks the question the row assumes an answer to: **no control this plugin owns opens a second designer leaf on one asset**, so whether Obsidian will give a walker a pair at all is unverified. It stays `partial` until somebody walks it |
...
| T25 Escape, pointercancel, blur, outside release are safe | **partial — regraded 2026-09-22 under AD15-R2** | `designerCanvasGestureOwnership.test.ts` now fires REAL DOM events at the designer’s own mounted `EditorSurface`: a `pointercancel` on the canvas, a `blur` on the canvas, and a `blur` on the owner window — three independent doors, since `blur` does not bubble and the container handler and the `listenOnOwner` registration are reached separately. Each asserts the band is gone, the cleared selection is RESTORED, and the sidecar is unchanged; all three were watched red against the handler that carries them. A fourth case covers what the other three cannot: `cancelInterruptedGesture` is the only thing that clears `#gestureInFlight`, and `EditorSurface` early-returns on it, so an unheard cancellation locks the camera out indefinitely — the case asserts the wheel is refused DURING the sweep and zooms after it, and went red at `expected 0.1 not to be 0.1` with the camera stuck at `DEFAULT_ZOOM`. **Two reasons this is not `passed`.** The `keyDoors.ts` arm of `gestureInFlight()` is not driven — only the wheel door is. And *a release outside the leaf* turns out not to be an interruption at all: `onPointerDown` calls `setPointerCapture` on both arms, so that release is delivered back to the captured container and the gesture COMMITS. Nothing asserts that commit, and this row’s own wording assumed a door that cannot exist while capture is held — `dropMarquee`’s docblock made the same assumption and is corrected on the integration SHA. **This row’s opening sentence was also false when written**: `designerEscapeRouting.test.ts` already reached `cancel` through a real `keydown` at the canvas, so Escape was never the gap. **CORRECTED 2026-09-22 (AD15-R2): the two reasons above were FALSE when the triage read them.** Commit `ef1ba2dff` (2026-09-19, *"Share the designer sweep vocabulary, and close T25's two residual gaps"*) added both — *"lets the keyboard go again: the zoom key zooms once the gesture has been abandoned"* drives the `keyDoors.ts` arm and asserts refused-then-free in that order, and `describe('a sweep released outside the leaf')`'s *"commits the selection rather than abandoning it"* asserts the commit. **The matrix was edited three times after that commit — `896a5f6a9`, `8deda6182`, `4bc2ba5b7`, all 2026-09-21 — and none regraded this row**, because a row is re-read by whoever is editing the rows beside it and nobody was editing this one. **The row stays `partial` for a NARROWER and mechanical reason**: jsdom implements no pointer capture at all (`setPointerCapture` is `undefined` on an element there, which is why `EditorSurface` spells the call `?.()`), so the outside-release case drives the SHAPE capture produces and its own docblock claims no more. The remaining gap is the browser, not the suite |
| T26 click-after-drag does not clear or retarget | **partial — narrowed 2026-09-22 under AD15-R2** | Behaviour asserted (`designerSelectMarquee.test.ts` "keeps the whole set rather than collapsing it to the part pressed"; `designerSelectTool.test.ts` "writes nothing, and previews nothing, for a press that never travels past the click epsilon"). **Narrower in LAYER**: the row says browser and every case is jsdom. **The reason is mechanical rather than a scope complaint**, and AD15-R2 narrowed it because *"the row says browser and every case is jsdom"* invites the next reader to try harder in jsdom. **jsdom implements no pointer capture** — `setPointerCapture` is `undefined` on an element there, measured, and it is why `EditorSurface` spells the call `?.()` — so the browser semantics click-after-drag depends on cannot be produced in this suite under any amount of effort. The losing side: the in-app browser over `npm run harness` could demonstrate it and produces no repeatable gate |
| T27 form/note keystrokes not eaten by shortcuts | **partial — the note-editor half ruled STRUCTURAL 2026-09-22 under AD15-R2** | The designer's own boundary is asserted (`designerKeyboard.test.ts` "deletes nothing for a Backspace typed in the inspector, a sibling of the canvas region"; `designerKeys.test.ts`'s HANDLED/IGNORED pair). **The NOTE EDITOR half is unasserted for the designer** — the native-field sweeps that exist are plan-editor's, and no test anywhere puts a Markdown or CodeMirror editor beside this surface. **The note-editor half is STRUCTURAL and gets no test.** Measured 2026-09-22: `src/presentation/designer/` registers NINE key doors, every one a template binding on an element inside the designer's own tree; nothing it mounts registers a key listener above that subtree — the only `listenOnOwner` call in anything it composes is `EditorSurface.vue`'s `'window'`/`'blur'`, the other three `listenOnOwner` calls in `src/` are `'document'`/`'pointerdown'`, and both bare `addEventListener('keydown', …)` calls in `src/` bind a Plan Editor component's own root. So a keystroke in a Markdown or CodeMirror editor — a different leaf — has nothing above the designer's subtree to reach. §6's own preamble agrees from the requirement side: *"Only the latter validates … interaction with a note editor."* **The losing side was live rather than refused, and wave 18 (W18-A) TOOK it.** `designerOwnerListeners.test.ts` walks `src/presentation/designer/` — a real recursive walk, so a file added tomorrow is scanned with no edit — and refuses `addEventListener`, `listenOnOwner` and `registerDomEvent` through `parsedSource.ts`'s AST reader rather than through source text. It is deliberately **wider** than this row's wording, refusing an element-local listener too, because deciding owner from element by callee text fails the moment the target is a local; the case names say the wider thing. Its blind spots are named in its own header — an aliased or computed door name, an SFC `<template>` binding, runtime installation, and handler-property assignment, which is not a call at all. **The row stays `partial` regardless**, exactly as this cell already predicted: its layer is *Browser + real host*, and no structural check can watch a keystroke cross from a Markdown leaf |
...
| T32 theme change preserves visibility, no geometry change | **partial — the WHOLE geometry half closed 2026-09-22 (waves 16 and 17); visibility never closeable here** | **Three components now, not two.** GEOMETRY, closed for the committed design’s marks: `designerTheme.test.ts` *"moves the whole palette and not one drawn coordinate"* writes every distinct `THEME_TOKENS` variable onto `document.body`, fires the recorded `onThemeChange` listeners through the real seam rather than setting a palette by hand, and compares the drawn coordinates of seven node selectors either side — reading the Konva SCENE, never the input shape, with an anti-tautology guard in the same case that reddens when the listener fire is removed. GEOMETRY, **closed for the selection's and the gesture's too, by wave 17 (W17-C)**: `SELECTION_NODES` compares `.asset-selection-outline`, `.asset-selection-handle`, `.asset-rotate-stem`, `.rotation-handle-button` and `.rotation-handle-icon` across the same real `onThemeChange` seam with something SELECTED, and a second case flips **mid-gesture** with `.detail-preview` on the stage, held open by `held()` because `drag()` releases in the same tick and leaves nothing to flip against. Together the two lists name all ten of the names `DesignerCanvas.vue` draws, plus two that `RotateArrowIcon.vue` draws from outside `designer/`. The capture is **ten fields per node, not four** — an independent review found `coordinatesOf` compared `x`/`y`/`width`/`height` while the case name promised *not one of its marks*, and demonstrated a green run with `offsetX` mutated; `offsetX`, `offsetY`, `cornerRadius`, `rotation` and both scales were added and the red re-taken against the FIXED tree. **The gap was reachability, not behaviour**: `designerRig`'s `onThemeChange` was `() => () => undefined`, a source that never fires, so no case built on that rig could flip a theme at all. VISIBILITY/OCCLUSION, **not closeable here**: jsdom has no rendering engine, so whether a line reads against its ground is unobservable at any effort. The `.theme-light`/`.theme-dark` class half is deliberately undriven, because `resolveThemeTokens` reads no class — only `getComputedStyle` — so a class flip would be inert theatre |
...
| T34 Use in plan places the real definition, returns context | **partial — assigned to the deferred manual pass (AD15-R2)** | Asserted end to end at the seam (`assetDesignerUsePlan.test.ts` "carries the asset into the Plan Editor it continues into"; `editorArrivalAssetHandoff.test.ts` "arms the placement tool with the asset the origin names"; `assetPlacementInspector.test.ts` for canonical size). **The row says browser + real host and no case is either**. **Not a triage row.** `reports/MANUAL-PASS.md` already assigns it: *"Take an asset from the library into a plan — 19 human steps — discharges U01, **T34**"*. It was carried into session twelve's triage set by mistake and is marked here so the next reader does not carry it again |
...
| T42 compact panes keep actions and errors reachable | **partial — the DOM and focus half closed by wave 7 (T42), the appearance half open** | `designerResponsiveShell.test.ts` renders the real designer at 520, 900 and 1400 and asserts a primary action, a refused read’s failure headline and its retry, and the additive `role="status"` notice are each present, enabled, accessibly named and focusable — plus one mounted tree walked 520 → 900 → 1400 → 520 with the set of enabled named buttons identical at every step. Width reaches it through the plan editor’s own `clientWidthFor` and `resizeTo`, and both channels were watched red through a temporary width-driven `v-if`. **What reframes this row**: the designer reads NO width in JavaScript. Established by an import walk from `AssetDesignerView.ts` over 435 files with 0 unresolved, not by the directory-scoped grep — that walk finds `EditorSurface.vue`’s `ResizeObserver` (sizing the Konva stage, deciding no region) and `followPixelRatio.ts`’s `matchMedia`, which is a device-pixel-ratio query. So the honest guarantee is INVARIANCE, and in jsdom the three widths produce an identical tree; the file says so. **It must never be graded `passed`.** jsdom applies no container query, so `styles/designer-narrow.css`’s `@container rp-designer (width < 35rem)` block has zero effect in every case, and nothing here sees the compact LAYOUT. `npm run harness-shot -- --width=460` is the only instrument that could, and there is no pinned Chromium on this machine. One thing measured in review makes the file stronger than it claims: that narrow block declares only `flex-direction`, `flex`, `width` and border swaps and no `display`/`visibility` at all, so it hides nothing and the invariance would hold in a real browser too. **AD15-R2 (2026-09-22) promotes this row's own refusal to the decisions side** so it is findable from `contracts/DECISIONS.md` rather than only from here: it must never be graded `passed` from an environment with no pinned Chromium, and `npx playwright install chromium` is forbidden here because it emptied `node_modules` once. The losing side: capturing with `RP_CHROMIUM_EXECUTABLE` pointed at another browser on disk produces a picture somebody then reasons about as if it were the pinned browser's |

**This paragraph states no count without the command that produced it, because the previous two
versions of it both went stale and neither could be detected by reading.** The T-table is 42 rows;
re-derive it rather than quoting this sentence:

```bash
awk -F'|' '/^\| T[0-9]+/ {g=$3; gsub(/\*/,"",g); gsub(/^ +| +$/,"",g);
  print (g ~ /^passed/ ? "passed" : g ~ /^structural/ ? "structural" \
    : g ~ /^partial/ ? "partial" : g ~ /^not-run/ ? "not-run" \
    : g ~ /^none/ ? "none" : "OTHER: " g)}' \
  docs/tasks/asset-designer-expansion/reports/AD15-validation-matrix.md | sort | uniq -c
```

**On 2026-09-22 (wave 16) that printed 27 passed, 10 partial, 5 structural — 42.** The whole matrix,
with `T` widened to `[FTU]`, printed **37 passed, 12 partial, 5 structural, 5 not-run, 0 none and 1
refused — 60**.

**The history of this sentence is the reason it now carries its own instrument.** Its first version
read "18 passed, 22 partial, 1 structural, 1 none", written from an impression before the tally and
wrong in every figure. Its second, counted properly, read 21/20/1/0. Its third, re-counted
2026-09-19 after wave 7, read 22/19/1/0 — **and was still standing on 2026-09-22 against an actual
27/10/5**, because wave 14 re-graded eight rows and AD15-R1 three more, and nobody re-ran a count
kept in prose beside the rows they were editing. **Wave 16 did not cause that drift and did not
change a single T-table grade word**; it found it while re-measuring, which is the only way a
number nothing re-runs is ever found.

**Two things about the 2026-09-22 figures are worth more than the figures.** The `none` column is
empty for the first time — no row in this matrix is left with no instrument at all, F12 having been
the last one. And a naive grep for a bolded grade word answers **79** rather than 60, because it
