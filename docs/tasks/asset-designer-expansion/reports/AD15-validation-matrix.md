...
| F08 legacy v1/v2 files | **partial** | The v1 and v2 documents exist only as LITERALS inside `dto/assetGeometry.test.ts` and `assetGeometrySidecarDetails.test.ts`. There is **no checked-in legacy `.rpgeo` fixture** — `tests/vault/legacy-schema/` holds `Plans/` and `Zones/` only, and `migration/legacyFixture.test.ts` is about notes rather than asset geometry. A literal cannot catch a byte-level read defect the way a file can |
...
| U04 trace and calibrate deliberately | **not-run** | Needs a host, and a PDF page rendering is the one thing no fake here stands in for — production asks Obsidian for its own pdf.js and the suite runs a different copy. Steps: `Calibrate a sheet and reserve space.md` |
...
|---|---|---|
| T01 L-resize keeps topology; Replace explicit | **partial** | Scaling asserted at both layers (`assetDimensions.test.ts` "scales a calibrated L-shaped footprint instead of squaring it off"; `shapeEdits.test.ts`; `scaleSolve.test.ts`). **The EXPLICITNESS of replacement is unasserted** — presets warn ("warns that the current design will be replaced when there is one") and the clearance warns, and footprint replacement has no equivalent case |
...
| T20 groups reject dangling/duplicate/nested | **partial** | Dangling and duplicate asserted (`groupEdits.test.ts` "refuses a graphic the design has not got", "refuses the same graphic named twice"). **NESTED is asserted only as flat exclusivity** — "refuses a graphic that is already in another group". Nothing refuses a group naming another GROUP, and the schema may not express one; C06 asks for shallow groups, so this is likely structural, but it is unasserted either way |
| T21 group/ungroup does not move or reorder | **partial** | Domain asserted (`groupEdits.test.ts` "changes no coordinate, no order and none of the shape's other parts", "moves a noncontiguous group to the front as a block, keeping its internal order"). **The RENDERING half is missing**: `partRows.test.ts` is the Parts list read-model, and `layers.test.ts` has no group case, so nothing asserts the drawn z-order is unchanged by grouping |
...
| T25 Escape, pointercancel, blur, outside release are safe | **partial, and this is the sharpest gap in the table** | Every designer case calls `tool.cancel()` / `tool.abandonGesture()` **directly on the tool object**. **No designer test ever fires a real `pointercancel`, a window `blur`, or a release outside the leaf** — the mapping from those DOM events to `abandonGesture` survives only as a LABEL STRING in `designerSelectMarquee.test.ts` (`'pointercancel, blur or a release outside the leaf'`), which is prose in a test title and not an assertion. The real DOM-event cases exist for the plan editor only (`canvasGestureOwnership.test.ts` dispatches `pointercancel`; `canvasKeyboardGestures.test.ts` "disarms a held space bar when the WINDOW blurs"). `DesignerCanvas.vue` mounts the same `EditorSurface`, so the shared component IS covered — **through the plan editor's fixture, and nothing asserts the designer mounts it with the wiring intact** |
...
| T40 graphic editing does not change cost/quantity/work | **partial, and thin** | The only command-level isolation proof is `setAssetHeight.test.ts` "changes no requirement figure, no requirement revision, and reaches no catalogue subscriber" — and a height is a NOTE edit, not a graphic one. **No test takes a geometry, detail, group or repeat write and asserts the unit cost, waste, unit or any requirement figure is untouched.** `designerPartsPanel.test.ts`'s "says a part's name and nothing else — no count, no unit, no price" is presentation-only |
| T41 repeated mount/unmount releases resources | **partial** | Single-cycle release asserted (`designerCrossLeaf.test.ts` "leaves nothing subscribed once every leaf is closed"; `assetDesignerView.test.ts` "draws nothing when a CLOSED leaf is rebound, rather than resurrecting its tree"). **§6 asks for 50 cycles and monotonic growth and nothing runs more than one.** No observer-disconnect case for the designer either; the one that exists is the plan editor's `responsiveShell.test.ts` |
| T42 compact panes keep actions and errors reachable | **partial** | The designer's compact behaviour is asserted **only as CSS RULES** — `designerStyles.test.ts` reads stylesheet declarations ("stacks the inspector under the canvas below 35rem, in fixed shares of the body"). Nothing renders at 520, 900 or 1400 px and checks that a primary action and an error are reachable; the width-driven reflow tests are the plan editor's. The accessibility harness scans one default width |

**Counted down the Grade column after the table was finished, not before it: 21 passed, 20 partial,
1 structural, 0 none — 42.** The first version of this sentence read "18 passed, 22 partial, 1
...
T40 and T42 are the three to close first**: a gesture surface whose DOM-level cancellation is
