...
| T32 theme change preserves visibility, no geometry change | **partial — geometry half closed for the committed design 2026-09-22 (wave 16)** | **Three components now, not two.** GEOMETRY, closed for the committed design’s marks: `designerTheme.test.ts` *"moves the whole palette and not one drawn coordinate"* writes every distinct `THEME_TOKENS` variable onto `document.body`, fires the recorded `onThemeChange` listeners through the real seam rather than setting a palette by hand, and compares the drawn coordinates of seven node selectors either side — reading the Konva SCENE, never the input shape, with an anti-tautology guard in the same case that reddens when the listener fire is removed. GEOMETRY, **still open** for the selection’s and the gesture’s: `selectionMarks`, `RotateArrowIcon.vue` and `DesignerGestureLayer.vue` each take `tokens` AND emit coordinates, and this case selects nothing and draws no gesture, so none of their geometry is asserted across a flip — a mount that does both is a card, not a fix. VISIBILITY/OCCLUSION, **not closeable here**: jsdom has no rendering engine, so whether a line reads against its ground is unobservable at any effort. The `.theme-light`/`.theme-dark` class half is deliberately undriven, because `resolveThemeTokens` reads no class — only `getComputedStyle` — so a class flip would be inert theatre |
