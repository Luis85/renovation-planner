...
| 10 | Duplicate a whole project folder as a backup (copy-paste it elsewhere in the vault, keeping its contents unchanged) and reload the plugin | A `persistence.index.sidecar-duplicate` line appears in the developer console (`Ctrl+Shift+I`), naming a `planId` and two paths. No setting change needed to see it: this diagnostic logs at `warn`, and `console.warn` is emitted whenever the level is at or above the plugin's default floor (`'info'`) — the **Verbose logging** toggle only widens the floor to `'debug'` for lower-level lines, so it does not gate this one | This is the data-loss-shaped path ADR-0013's own "moves, backs up and deletes as one unit" property opens: two `.rpgeo` files now share both a basename and the plan id inside it, and the sidecar's own verification cannot tell them apart. **Record which sidecar the plan actually resolved to rather than asserting a winner** — it is last-writer-wins by scan order, and nothing promises an order |
