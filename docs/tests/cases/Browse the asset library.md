...
| 4 | `suite` | Compare a shelf heading, the inspector's Category dropdown and the New asset dialog's Category dropdown for one asset | All three name the category the same way | **Known to FAIL as shipped.** The shelf says `Furniture` (through `ASSET_CATEGORY_LABELS`), the inspector's `<select>` says `furniture` (raw key), and `NewAssetForm` says `Furniture`. Same for the unit, where the row prints the raw `m2` while `MEASUREMENT_UNIT_LABELS.m2` resolves to `Square metres`. (This said *rather than a translated `m²`* for one commit. `grep -rn "m²" src/presentation/library/ src/presentation/views/ src/presentation/i18n/` prints nothing, so no library site, no form site and no locale entry carries that glyph — the defect is the raw KEY beside a translated label, not a missing `m²`) |
...
| 18 | `suite` | Read the **Back to library** control's label at a sidebar width | It matches what the spec asks for | §7's table writes `‹ Back to library` and the shipped copy is `Back to library`. **§6.2 writes it WITHOUT the chevron** (`grep -rn "‹"` on the spec prints one line, §7's table at `:1564`), so the spec spells it two ways and the code matches one of them — this step is as likely to end in a correction to §7 as in a change to the copy, and that is why it is a step rather than a defect |
...
4. **Steps 2, 3, 4, 12 and 15 are expected to FAIL** against the build this case was written
   for. A run that finds them passing means somebody fixed them; a run that finds them failing
   has confirmed a finding rather than discovered one.
5. **Steps 7, 17 and 18 are expected to be OPEN rather than to pass or fail**, and each for its
   ...
   - **18** is a question about the SPEC as much as about the code: §7's table asks for a
     chevron, §6.2 does not, and the run decides which of the two is corrected.
