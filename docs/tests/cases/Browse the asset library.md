...
**Added 2026-09-23 (AD18-R18, second parity round). None of this has been seen in Obsidian.** A
...
| 20 | `obsidian` | Press **Grid** | The shelves are replaced by a tile grid: one tile per asset, each showing the SAME geometry mark the list row draws (at tile size), the asset's name, and its measured size in the SAME wording the list row uses (e.g. "800 × 400 mm") | The tile and the row share one rendering function (`markWords.ts`) for the mark and the size text, so the two surfaces cannot say two different things about one shape |
...
| 22 | `obsidian` | Look at the new sidebar down the left of the grid | It lists **All** plus every category this vault's assets actually use — an OPEN vocabulary, not a fixed list — each with its own icon (a grid for All, layered sheets for material, an armchair for furniture, a bath for fixture, a sprout for plant, a hammer for equipment, a brick wall for a building element, a pencil for custom, and a generic tag for anything else) | §1a's open vocabulary, reached in the Grid view for the first time; a category this vault has never used before does not need a code change to appear here |
| 23 | `obsidian` | Click a category in the sidebar | Both the grid AND the shelves (switch to List to check) narrow to that category alone; the search count and the empty state (if the category is now empty of matches) follow the SAME filter | One filter reaching both views (§10) — not a Grid-only affordance |
...
| 24a | `obsidian` | With the category sidebar SHOWING (a wide leaf, or the funnel pressed open), reach the empty state from step 24 and press **"Show all categories"** | Focus lands on the **"All"** category button (`aria-pressed="true"`) | `onClearFilter`'s `focusAfterSwap('.rp-al-category[aria-pressed="true"]', …)` (`AssetLibraryRoot.vue:164-168`) finds the pressed "All" button inside a sidebar that IS laid out |
| 24b | `obsidian` | Reach the same empty state with the sidebar NOT showing — below 35rem with the funnel closed — and press **"Show all categories"** | Focus lands on the **search field** instead | The same call's fallback: `focusWithin` (`shelfFocus.ts:177-183`) finds no laid-out `.rp-al-category` to focus and falls back to the search input — the one target guaranteed to exist in every layout. The sidebar renders in BOTH Grid and List (§10's filter reaches both views), so this branch is reached by width alone, never by which view is active |
...
| 26 | `obsidian` | At that narrow width, press the funnel | The sidebar opens as an overlay/toggle over the grid; `aria-expanded` on the button follows it | The same filter, reached by a second door rather than a duplicated one |
| 27 | `obsidian` | At a leaf already narrower than 35rem, select a tile | The category sidebar and the funnel button, already hidden by the container query, stay hidden with a tile selected — nothing about selecting a tile shows or re-hides either control | The hide rule is `@container rp-al (width < 35rem)` on the whole pane (`styles/asset-library-grid.css:324-335`, the container itself declared once at `styles/asset-library.css:48`) — selecting a tile never changes the pane's own width, so there is no separate selection-driven mechanism to check here |
| 28 | `obsidian` | Scroll to the end of the tile grid | A **"Create your own"** card sits there, with a pencil icon, a title and a hint, and a **New asset** button | AD18-R18's card, calling the EXISTING New asset door (`onCreateAsset`) rather than a second dialog |
| 29 | `obsidian` | Press that card's **New asset** button | The same dialog the toolbar's own **New asset** button opens, opens | One creation door, reached from a second place |
