...
| 12 | `browser` | At the 240px rail, read the *Used in* list for an asset a project holds a price override for | The project name and the override mark sit on one line, or the name wraps cleanly | **Fixed by Task A4** (commit `7099a030`, used-in row wraps): `.rp-al-used__row` gained `flex-wrap: wrap`, `.rp-al-used__name` is `flex: 1 1 auto; min-width: 0; overflow-wrap: normal` and the override mark is `white-space: nowrap; flex: 0 0 auto` — the name keeps whole words and the mark drops to its own line rather than sharing one. Measured (Task A6, `AL10-460-dark.png`): row heights `[52.09, 50.59, 35.5]`, no rows colliding — was 51.8px against 19.6px for the row below it, with the name breaking `Flat / renovation — / 1 / requirement( / s)` one character per line |
