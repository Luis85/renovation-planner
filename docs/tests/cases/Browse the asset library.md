...
| 27 | `obsidian` | At a WIDE leaf, select a tile so the inspector takes the pane at a narrow effective width | The category sidebar and the funnel button both disappear while the selection holds the pane narrow, exactly as the shelves' own inspector already does | The narrow behaviour is about the PANE'S width, not the window's — a selection narrowing the browsing column has to hide the same controls a literally-narrow leaf does |
