...
| 2026-09-13 | Before: the row above's build on 74973601 (vue-konva 3.4.0); after: the fix on the same base. Back to back on one harness server; headless pinned Chromium, 1600×1000, DPR 1 | before: pan 2.7–3.3 ms, zoom 6.4–6.8 ms; after: pan 3.2–3.5 ms, zoom 5.5–5.8 ms | after: pan 3.3–3.6 ms, zoom 6.9 ms (before not re-run; the row above reads 20.7 ms) | before: pan 3.4–3.7 ms, zoom 34.5–37.0 ms; after: pan 3.3–3.5 ms, zoom 7.6 ms | See the next row for the same change on vue-konva 4.0.1. |
| 2026-09-13 | Rebased onto 4ca4c7ea (vue-konva 4.0.1). Quiet: the commit adding this row alone. Loaded: before and after interleaved twice (before, after, before, after) while another session loaded the CPU, pan being the load control | quiet after: zoom 5.9 ms; loaded before: pan 6.0–7.1 ms, zoom 12.6–15.1 ms; loaded after: pan 5.1–6.5 ms, zoom 8.8–9.5 ms | quiet after: pan 3.1–3.6 ms, zoom 6.6–6.9 ms | quiet after: pan 3.6–4.1 ms, zoom 7.7–8.2 ms; loaded before: pan 6.0–6.5 ms, zoom 60.6–67.1 ms; loaded after: pan 5.9–8.2 ms, zoom 13.5–16.7 ms | vue-konva 4.0.1 keeps the three mechanisms below (a deep `config` watch per node, the Layer/Group `onUpdated` z-order walk, `on*` props bound as Konva events), so the cut carries over: under equal load, zoom at rooms=80 drops about fourfold, and 0 → 80 growth drops from about 50 ms to about 5 ms. Quiet, zoom at rooms=80 sits at §4's 8 ms trigger rather than clearly under it. The per-room zoom cost, profiled over 80 zoom notches at rooms=80 before the change: every room caption's `1 / zoom` scale travelled through its vue-konva `config`, so each notch re-rendered every `ZoneShape` and re-applied `scaleX,scaleY` to all 252 caption Texts (84 zones × 3; a `setAttrs` census counted 10 080 over 40 notches) through vue-konva's deep config watch and each group's `onUpdated` walk; and each room's automatic caption anchor (`labelAnchor`: a centroid and a containment test) was re-derived although it depends on the geometry alone. After: `ZoneLayer.vue` writes the counter-scale onto every `.zone-caption` node from one watch, and from the layer's Konva `add` event for a room mounted later; `ZoneShape.vue` caches the automatic anchor per geometry. Zoom at rooms=80 is back under §4's 8 ms trigger. The residual 0 → 80 growth (about 2 ms) is each `ZoneShape`'s props update on the changed `zoom` prop, which its `v-memo` then skips. Checked in the same browser: all 48 captions at rooms=12 sit at exactly `1 / zoom` after mount and after three wheel notches, and no Vue warning is logged. Pinned by `tests/presentation/editor/zoneZoomConfiguration.test.ts`. |
