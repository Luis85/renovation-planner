...
- **A slow write's own visual absence of feedback is not something a fake write can
  demonstrate.** `FormDescriptor.busy` makes every control inoperative for the DURATION of a
  real vault write; the suite drives it with a `dispatch` that resolves on the next microtask,
...
- **And the FOCUS consequence of how a control is made inoperative is invisible to every gate
  here.** Nothing may be `:disabled` while busy — Chromium moves focus off a disabled element
  to `<body>`, and `.rp-dialog` does not contain it, so the dialog's `Escape` and its Tab trap
  both go dead for the write window. jsdom implements no focus-loss-on-disable at all, so the
  suite can only pin the CONDITION (no `[disabled]` in the dialog while busy); steps 7a and 7b
  are where the behaviour itself is looked at.
...
| 2 | Click that button | A dialog opens, titled "New renovation project", with labelled Name, Status, Description, Start and Target completion controls, and Save/Cancel buttons | `DialogHost` mounting a real `FormDescriptor.component` for the first time in this plugin — every earlier `form`-kind caller was a test stub |
...
| 7a | Do step 7 again on a vault big enough for the write to be visible, and press `Escape` WHILE it is in flight | The dialog stays open and nothing is cancelled — and, if this vault binds a hotkey to `Escape`, that hotkey fires (see step 11's note on what "modal" means here) | The defect this step exists for: no control may be `:disabled` while busy, because Chromium blurs the disabled focused element to `<body>`, which `.rp-dialog` does not contain — so `DialogHost`'s `Escape` handler stopped receiving keys for the whole write window and the deliberate refusal became an accidental one. Nothing in jsdom models focus-loss-on-disable, so a live vault is the only instrument for the SYMPTOM; the suite pins the condition (`newProjectForm.test.ts`, `formBusy.test.ts`) |
| 7b | Still during a write in flight, press `Tab` several times | Focus cycles inside the dialog and never leaves it | The other half of the same defect: a dialog whose controls are all `:disabled` holds nothing the Tab trap can walk, so `Tab` walked straight out into the pane behind it |
| 8 | Once the write finishes | The dialog closes, and the new project appears as a row in the list — **without** reloading Obsidian or reopening the view | Task 8's `ProjectList` re-hydrating after `onCreateProject`'s own `store.hydrate` call, over the real `CreateProjectCommand` write Task 5 wired into this view landing in the vault |
| 9 | Click the list header's own "New project" button (the list is no longer empty, so this is a different button from step 1's) | The same "New renovation project" dialog opens | Task 8's list header shares Task 7's one handler rather than opening the form a second, independently-decided way |
...
| 15 | Switch Obsidian's language to German (Settings → General → Language) and reopen the New Project dialog, then repeat steps 4 and 5 | Every label, both buttons and every inline error render in German. Not the banner: no step in this case renders one — see "Deliberately NOT checked" | Every string this slice added is a `StringKey` resolved through `t`/`tr`; `de.ts` answers all of them, and nothing here has ever been rendered by a gate that reads German |
...
- **`FormBanner` itself. NO STEP IN THIS CASE RENDERS ONE**, and that is worth stating
  plainly rather than leaving a reader to discover it mid-walkthrough. Step 4 produces
  `project.empty-name`, which routes to the NAME FIELD; step 5's `project.target-before-start`
  routes to a field PAIR. The one refusal that would reach the banner is a vault write failure
  during submit, and the bullet below explains why that is not a step anyone can follow. So
  the banner's markup, its `role="alert"` and its German copy are all exercised by the unit
  suite alone (`routeError.test.ts` for the routing rule, `newProjectForm.test.ts` for the
  rendering) and by no human here. An earlier draft of this file claimed step 4 proved the
  banner path and step 15 told the runner to "force one — see step 4"; both were wrong in the
  same way, and a runner following them would have hunted for a banner, failed to find one,
  and reported working code as broken under this file's own footer rule.
- **The calibration form's `coincident-points` banner.** `KnownDistanceForm` is slice 7/15's,
  ...
  it renders no `FormBanner` at all, and nothing routes a calibration error through
  `routeError`. `docs/tasks/16`'s reconciliation records Definition of Done item 4 as unmet
