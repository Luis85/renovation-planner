...
| 81 | `obsidian` | Look at the canvas's bottom-left corner | A legend lists five rows, each with its own swatch, in this order: Clearance, Footprint, Details, Placement point, Front direction | The full row set, on a design that carries all five parts, in the order `legendRows.ts` pushes them (clearance before footprint) |
