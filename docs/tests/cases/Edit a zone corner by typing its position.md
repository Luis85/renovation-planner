...
| 9 | `judgement` | Read the submit button's own label before pressing it | Recorded, not asserted: it reads **Save / Speichern** — `dialog.form.submit`, the key `FormSubmitRow.vue` declares is "the one string every form's submit says". It read **Apply name** (`editor.rename.apply`) until L-30 was fixed on 2026-09-21; that key now labels the rename dialog and nothing else | A copy defect no gate can see — the key resolves, both locales are written, and sentence case holds |
