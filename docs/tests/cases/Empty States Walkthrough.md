...
| 8 | `suite` | With the draw tool active, draw and close a polygon, then set a plan background via `Set plan background` | With a background AND zones, no panel is drawn at all | The both-populated case — the normal render this slice must not disturb |
