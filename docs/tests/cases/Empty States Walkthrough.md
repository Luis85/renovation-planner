...
| 6 | Press the toolbar's Draw zone button | The panel DISAPPEARS while the tool is active, and the canvas is fully usable | The overlay yields to an active tool. A panel still floating there would leave the user in a mode they cannot reach the stage in |
| 7 | Press Escape (or switch back to Select) | The panel returns | The gate is the tool's state, not a one-way dismissal |
| 8 | With the draw tool active, draw and close a polygon, then set a plan background via `Set plan background` | With a background AND zones, no panel is drawn at all | The both-populated case — the normal render this slice must not disturb |
| 9 | Delete every zone on a plan that HAS a background | "No zones yet" appears over the canvas, WITH a "Draw a zone" button | The third state, and the only one carrying an action. Its copy must differ from step 5's — the same wording for both would tell a user with a background to import a plan |
| 10 | Press that "Draw a zone" button | The draw tool activates and the panel disappears in one click; drawing works immediately | The one wired hand-off in the slice. It sets the tool rather than dispatching a command, because a zone cannot be created with no geometry |
| 11 | Switch the vault to the other colour scheme (Settings → Appearance) and revisit any empty state | The panel is still clearly distinguishable from the page behind it, and no colour looks hard-coded | SDD §84. The build refuses a literal colour, but only a themed vault shows whether the Obsidian variables chosen were the right ones |
| 12 | Set Obsidian's language to German and reopen the Renovation project view | Every line of the panel is German | The registry holds `StringKey`s, never copy, and `de.ts` answers all seven of this slice's keys |
| 13 | Pan and zoom the canvas while an empty-state panel is showing | The camera responds normally; the panel does not swallow the gesture | `pointer-events: none` on the overlay with `auto` on its button alone. Setting it on the card would make the panel a pointer trap over a canvas the user is trying to draw on |
| 14 | Open the same plan in two tabs (split the leaf) while an empty state shows, then draw a zone in one | Both tabs agree — the panel clears in both once the plan has zones | The empty state is derived from store state that plan-change events refresh in every leaf, not from a one-off read |
