...
| 2 | `obsidian` | Install a community theme, then read the pane in **both** its light and dark schemes: the status word, the facts slot (`2 plans · EUR`), the foot line and the tick strip | Each is legible against the ground the theme gives it, and the reached cells of the strip are plainly distinguishable from the unreached ones | The one measurement the captures cannot make. Every contrast figure this surface has — `--text-muted` at 6.69:1 light and 8.13:1 dark, the strip's reached-vs-unreached at 6.93:1 light and 4.11:1 dark — is against **Obsidian's default palette**, and a theme replaces every one of those tokens. The strip is drawn from **two named Obsidian tokens** — `--text-normal` reached, `--text-faint` unreached — precisely so it follows the theme's text colour rather than a literal, and this is where that claim is checked. **NOT `currentColor`**, which the spec §6 said until the final review amended it and which `styles/project-list.css` refuses in writing: it resolves to one inherited colour and cannot produce two states, so a runner sent to verify it would have been verifying a mechanism this build does not use |
| 2a | `obsidian` | **AD18-R24, added 2026-09-25. Outside `MANUAL-PASS.md`'s own counting command — this case is not one of its seven, so this row moves no counted total.** With the theme from step 2 still active, look at the **Completed** group's `<summary>` chevron in both schemes, then click it to collapse and reopen the group once in each | The same small diagonal marker rotates from pointing right (closed) to pointing down (open) in both schemes, legible against `--text-muted`, exactly as before this round | AD18-R24 Task 3: `.rp-project-list__completed`'s chevron rules moved out of `styles/project-list.css` into the new shared `styles/disclosure-chevron.css`, combined with the asset designer's identical `.rp-designer-collapsible` rule ([[Compose an asset from parts]] step 26a is the other half). Every declaration value is byte-identical to what this file had, so this is a regression guard that the merge changed nothing to look at |
...
| 11 | `obsidian` | Restart Obsidian and resume; then remove the stored project or plan and reopen the overview | Restart re-resolves the saved target. A missing project shows recovery text; a missing plan offers its project. The stored target is retained; no substitute plan opens | Missing, unreadable and indexing states remain distinct |
...
| 14 | `suite` | Enter a query matching nothing | Clear filter restores the list and filter focus. New project opens a prefilled form; successful creation enters the returned project ID | Creation does not navigate by display name |
