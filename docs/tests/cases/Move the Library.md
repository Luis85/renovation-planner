...
| 12 | `obsidian` | Put the projects folder back. With the library at `Shared`, drag a whole project folder **into** `Shared` in the file explorer, then **reload the plugin — this is REQUIRED, not incidental; read the Why column before reporting a bug** — and open the Renovation project view | That project's row carries a marker with **both a triangular mark and the words "Overlaps the library folder"** — never one without the other | §83's third site, end to end **through the data path**, which is the half nothing has ever exercised: task 9 verified this marker by injecting the span into the DOM by hand, so `ListProjects` → `IndexLibraryOverlaps.overlapping` → `ProjectSummaryDto.libraryOverlap` → the `v-if` has never actually rendered it in anger. A mark AND a word is SDD §85's refusal of status carried by colour alone, and the mark is a `::before` drawn out of borders — a rule declaring nothing but `content: ''` satisfies "the stylesheet declares this class" and draws absolutely nothing, which is what the first draft of that block did. **Why the reload is REQUIRED**: the marker is derived per read from the Project INDEX, and a folder moved in the file explorer is never reported to that index — the plugin filters the vault's create, modify, delete and rename events to `TFile`, so the `TFolder` Obsidian reports for a folder move is dropped and the index keeps the note's old path. The row therefore updates at the next full index REBUILD (`onLayoutReady`, i.e. a reload — or a settings save), not as the drag lands. **Seeing no marker before reloading is a known limitation, not a bug**: that filter dates from design slice 4's persistence pipeline, and forwarding folder renames is a change to the vault-change pipeline every index consumer inherits rather than to this marker |
