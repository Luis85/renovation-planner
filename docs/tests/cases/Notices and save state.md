...
| 17a | `desktop` | **Stage an UNRECOVERED-write incident, which is not step 16's save error.** It needs a write that lands half-way AND a compensation that also fails, so the filesystem has to allow the write and refuse the undo: make the plugin's `Geometry/` folder unwritable while leaving the zone note's folder writable but its files UNDELETABLE (a Windows ACL denying Delete, or a Linux directory with the sticky bit and no write permission), then add a room. **If your platform cannot hold those two permissions apart, say so and stop here** — the node suite covers the stamp itself; 17b onward are what only a vault can answer | The status region reads **Save error** AND a persistent warning stands saying the geometry entry could not be written and the note could not be removed again. Further edits in that tab are refused | `ObsidianZoneRepository.compensateFailedSidecarWrite`'s `zone.sidecar-insert-uncompensated` arm — the one stamped by `markUncompensated`, which is what `withSaveStateTracking` turns into the incident. Step 16's single unwritable folder does NOT produce one: its compensation succeeds, so it stamps `zone.sidecar-insert-failed` and is an ordinary save error |
| 17b | `obsidian` | With that warning standing, restore both permissions, then open **Settings ▸ Renovation Planner** and change any setting — units will do | The warning is STILL there and that tab still refuses edits | The 2026-09-16 change: the incident belongs to the leaf, not to the mount. A settings save rebinds every open Plan Editor and used to lose the warning silently — a false all-clear over a half-written vault. Driven in jsdom by `tests/presentation/views/planEditorIncident.test.ts`; this row is the same claim in the real remount |
| 17c | `obsidian` | With the warning still standing, quit Obsidian fully and reopen it onto that same layout. Then look at `.obsidian/workspace.json` and find that leaf's `state` | **This step asks two questions rather than asserting an answer.** Record (i) whether `"unrecoveredWrite": true` is in the saved state at all, and (ii) whether the reopened tab still shows the warning and still refuses edits | **The one claim in this area no gate in this repository can reach.** SDD §14 and §102, the requirement and the issue note all rest on Obsidian handing the persisted state back to a restored leaf; `FakeLeaf` records asks rather than performing them, so the suite stops at what `getState` emits and what `setState` does with it. Question (i) is the narrower half: `PlanEditorView` never PUBLISHES its state the way `AssetLibraryView.publishViewState` does, so what Obsidian saved is whatever `getState()` answered at the last layout save — an absent key means the incident was raised after it, and that is a finding, not a pass or a fail |
| 17d | `obsidian` | Close that Plan editor tab, then run `Open plan editor` and pick the same plan | A NEW tab opens with no warning, and edits work again. Reopening the CLOSED TAB instead (Obsidian's undo-close gesture) is the other case: record whether the warning comes back with it | **The only way a user ends an incident**, and `docs/using-planning-recovery.md` now says so. Nothing clears the flag — this layer cannot tell a write that repaired the affected rows from any other write that landed — so the reset is a new leaf: `revealPlanEditor` sets `{ planId }` and nothing else on a leaf it creates. That it clears the warning is not evidence the vault was repaired, which is why the user document puts the inspection first |
