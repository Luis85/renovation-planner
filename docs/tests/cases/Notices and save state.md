...
| 21 | `desktop` | Raise three PERSISTENT warnings, then an error, and check the error gets through. Warnings: pick a background file Obsidian cannot render (`background.unsupported`), then make the plugin's `Geometry/` folder unwritable from outside Obsidian and move a zone, which raises both `cascade.*` warnings. With three standing, produce the ERROR: activate **Calibrate** and click **twice in the same place** | The error notice **appears**, taking the slot of the newest warning, and that warning returns when the error is dismissed. With a screen reader on, the error is **announced**. No warning is announced that you never see | **Design slice 17, and the only instrument for it.** `warning` and `error` have no auto-dismiss, so before this three standing warnings held every later error invisibly AND unannounced — `announce` rides `render`, and `render` runs only for a notice actually shown. `severity.ts` predicted both the exposure and this remedy before anything depended on it. The last clause covers the second defect a review bot found on the same mechanism: a free slot used to be handed to a held warning that the preemption then immediately hid, and `open` announces, so a screen reader heard a message that was never readable. **The error source is deliberately NOT Undo**, which an earlier draft of this step used and which cannot work: slice 17 routes an undo refusal to the save indicator and raises no notice at all, so that procedure exercised nothing. Coincident calibration clicks are a `Calculation` refusal at an `explicit-operation` origin — an error toast, with no filesystem setup of its own |
