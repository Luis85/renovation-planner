...
| 13a | `suite` | With a plan open, assign an asset to a zone, then assign **the same asset to the same zone again**. Watch the save-state region through both | The first shows *Saving* then **"Saved just now"**. The second shows *Saving* then settles back on EXACTLY what the first write already left — the same relative reading, not a fresh "just now" of its own — but if a **Save error** was standing before you started, the second assignment must LEAVE it standing | **A success that wrote nothing must not clear a save error**, which is the rule `SaveStateStore`'s header states and which this slice shipped broken. `AssignAssetCommand` answers `ok` from a READ when the link already exists, having saved nothing, and the tracker read that as a successful write. The command reports a `DispatchOutcome` now. Reproducing the standing error needs step 16's failure injection first, which is why this step is written as a conditional rather than a sequence — the node tests prove the settlement, and what a vault adds is that the second assignment really does take the idempotent path. **AMENDED 2026-09-23 (AD18-R19): the plain word "Saved" is no longer what a session that has just written sees** — a screen reader still hears only "Saved" (the relative phrase is `aria-hidden`), but the region's VISIBLE text is now the relative one. **A neutral second batch never re-stamps `savedAt`**: `settle()` only sets it `if (hasWriteInBatch.value)` (`save-state-store.ts:100-109`); the idempotent assignment goes through `resolveNeutral`, so the relative clock keeps counting from the FIRST write rather than resetting — a naive reading of "returns to Saved just now" would claim a second stamp that never happens |
| 14 | `suite` | Add a room — **Add ▸ Room**, drag a rectangle, press **Create room** — and watch that region | It shows **Saving**, then settles on **"Saved just now"** | The tracked dispatcher. `withSaveStateTracking` sits inside `wrapDispatcher` and outside the refresh decorator, so the settled text never appears while the canvas still shows the pre-command state. A fast write may make **Saving** hard to catch; the contract's own Open question 2 asks whether that state should be shown at all. **This row said "draw a zone" until 2026-09-05**, and that is not a wording change: the Add Room increment repointed Add ▸ Room to `'draw-room'`, so the polygon tool has no door in the Plan Editor at all and the write now happens at Create rather than on a closing vertex. Watch the region across the CREATE press. **AMENDED 2026-09-23 (AD18-R19): the settled text reads "Saved just now" now, not bare "Saved"** — a screen reader still hears only "Saved" |
| 15 | `suite` | Press Undo, then Redo | The region passes through **Saving** each way | An undo is a write like any other — slice 8's reversible delete writes a snapshot back through the repository. A decorator over `run` alone would leave the indicator reading the settled text through an in-flight undo |
...
| 17 | `desktop` | Make the folder writable again and move a zone | The region returns to **"Saved just now"** | The error clearing on the one thing entitled to clear it. **AMENDED 2026-09-23 (AD18-R19): it settles on the relative phrase, not bare "Saved"** |
| 18 | `obsidian` | Open two plans in two leaves (split the workspace), then add a room in one (Add ▸ Room, drag, **Create room**) | Only that leaf's indicator moves — to **"Saved just now"**; the other leaf's is untouched, reading whatever it read before (plain **Saved** if it has not written this session, or its own earlier relative reading if it has) | One `SaveStateStore` per view's own Pinia instance, which follows from `CommandHistory` being per-Plan. A plugin-global store would report one plan's write on both. The gesture is step 14's, for step 14's reason. **AMENDED 2026-09-23 (AD18-R19): "the other stays Saved" no longer describes every case** — it stays whatever it already read, since each leaf's `savedAt` is its own |
| 18a | `obsidian` | After step 14 or step 18, wait at least one minute with NO further edit, watching the region the whole time | The VISIBLE text advances on its own from "Saved just now" to **"Saved 1 min ago"**, with no action from you | The minute tick is ALIGNED TO THE SAVE rather than to the component's mount, so this is the first time it has run against a real wall clock rather than an injected fake one — a mount-aligned tick would have let a save landing just after it read "just now" for nearly two minutes |
| 18b | `obsidian` | With a screen reader running, repeat step 14 (or step 18) and then step 18a in the same sitting, listening the whole time | The screen reader announces **"Saved"** once, at the write — and says **nothing more** while the visible text later moves from "just now" to "1 min ago" on its own | AD18-R19's own constraint: a minute tick must not be announced. The relative phrase is wrapped `aria-hidden`, and only a plain, unchanging "Saved" sits inside the `role="status"` region's visible-to-assistive-tech content, so a tick that only changes the hidden text should be silent |
| 18c | `obsidian` | **Added 2026-09-24 (AD18-R21 Task 9).** After step 14 or 18, move the system clock forward to the next calendar day (or leave the leaf open across local midnight) without making another edit, then look at the region again | The VISIBLE text advances on its own from a relative reading (or a same-day "Saved at HH:MM") to **"Saved {date} at {time}"** — e.g. "Saved Sep 23 at 2:05 PM" — once the save is over an hour old AND from an earlier calendar day. A save still under an hour old at the moment midnight passes stays on its minutes-ago reading | The day check only applies past the existing hour-mark branch (the same "over an hour" threshold row 13a and step 14 already exercise), so a fresh save crossing midnight does not jump straight to a dated form — [[Design an Asset]] step 88c is the same fix seen on the designer's own header |
| 18d | `obsidian` | Repeat step 18c with Obsidian's language set to German | The dated form reads **"Am 23. Sept. um 14:05 gespeichert"** (day, then time, German word order) — not a literal translation of the English word order | `save-state.saved-on-date`'s two locale strings, added in both `en/saveStateRelative.ts` and `de/saveStateRelative.ts` |
