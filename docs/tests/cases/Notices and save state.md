...
| 21 | `desktop` | Raise three PERSISTENT warnings and check an error still gets through. With a plan open: pick a background file Obsidian cannot render as one (`background.unsupported`), then trigger the two cascade warnings — the simplest is step 16's unwritable `Geometry/` folder, which makes a zone move fail its stale-marker write. With three warnings standing, make an ERROR happen: with the folder still unwritable, press Undo | The error notice **appears**, taking the slot of the newest warning, and that warning returns when the error is dismissed. With a screen reader on, the error is **announced** | **Design slice 17, and the only instrument for it.** `warning` and `error` have no auto-dismiss, so before this three standing warnings held every later error invisibly AND unannounced — `announce` rides `render`, and `render` runs only for a notice actually shown. `severity.ts` predicted both the exposure and this remedy before anything depended on it. The announcement half matters more than the visible half and cannot be checked any other way: `tests/harness/obsidian.css` declares no `.notice` and no `.notice-container` rule at all, so neither `npm run harness` nor `npm run harness-shot` can draw a notice, and the live regions are appended to `document.body`, outside every axe scan in this repository |
| 22 | `obsidian` | With the Calibrate tool active, click **twice in the same place** on the canvas | The anchor marker comes off AND an error notice says the calibration could not be taken. Nothing prompts for a distance | It used to do the first half only: the tool refused a zero-length measurement before prompting — which is right, and stays — and then returned **silently**, discarding a point the user had placed with no reason given. Refusing before the prompt is deliberate: asking someone to measure a segment already known to be meaningless is worse than not asking |
