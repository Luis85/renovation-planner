...
| 7a | `obsidian` | **AD18-R23, added 2026-09-24.** At the default (non-narrowed) width, look at the left **Layers** panel's **Rooms** section, then click Terrace's own lock icon to lock it | Before clicking: every row's padlock (Kitchen's included) is open and nearly invisible at rest, appearing only on hover over its row or when focus reaches the icon itself (Tab); after clicking: Terrace's padlock closes, stays visible at rest with no hover or focus needed, and the padlock GLYPH ITSELF (not the room's name or area, which carry no lock-conditional styling) switches to the normal (not muted) text colour, unlike every still-unlocked row's open padlock | AD18-R23 Task 6, fix round 1: dropping `aria-pressed` off `ZoneLockToggle.vue` silently broke two shipped rules keyed on it — `styles/editor-side-panel.css`'s quiet lock (`.rp-editor-layers .rp-room-list__item > .rp-editor-inspector-lock[data-rp-locked='false']`, hidden until `:hover`/`:focus-within` on the row) and `styles/editor-inspector.css`'s locked emphasis (`.rp-room-list .rp-editor-inspector-lock[data-rp-locked='true'] { color: var(--text-normal); }`) — both scoped to `.rp-editor-inspector-lock`, the toggle BUTTON itself, per `task-6-report.md`'s own "the locked toggle's `--text-normal` emphasis". Both are re-keyed onto the toggle's own `data-rp-locked` attribute now, so the same visuals survive with no ARIA state announcing them |
| 7b | `desktop` | With a screen reader running, Tab onto Terrace's lock button (still locked from 7a), then onto Kitchen's | The screen reader announces "Unlock Terrace" and "Lock Kitchen" respectively, as plain buttons — neither announcement adds "pressed" or "not pressed" | AD18-R23 Task 6: the accessible name alone now carries the state ("Lock {name}"/"Unlock {name}"); `aria-pressed` used to double it and, for a locked zone, contradict it ("Unlock Kitchen, toggle button, pressed"). Press Terrace's toggle once more afterward to leave it unlocked, matching this case's own preconditions for the rest of the walk |
