...
| 37 | repair plus retry clears the notice, the button and the qualifier together | **Corrected citation (was OVERCLAIM)**: not *shows the stale notice…*, which heals via `reconcile()` alone and never presses Try again. The real discharge, closed in the Tasks 2 and 6 table below, is `assetDesignerRecoveryWalk.e2e.ts`'s *raises the notice with no press, and a press after the repair retires all three widgets at once* |
...
| 25 | no `Several Konva instances` line | Design an Asset's *keeps every shape across a plugin reload, which detaches the leaf and leaves the console clean* — checks two known substrings only; the general "no error at all" clause is closed separately in the Tasks 2 and 6 table below |
...
| 32 | it does not move for steps 3, 5 and 11, nor for step 19's sidecar half | same case, and *restores the sheet but not its scale…* — **OVERCLAIM on the step-5 sub-clause only, corrected in the Tasks 2 and 6 table below**; steps 3, 11 and 19's sidecar half are correctly cited here |
...
**Added 2026-09-26 (AD18 Tasks 2 and 6), after a clause audit (`.superpowers/sdd/audit/recover.md`).**
The B and D clauses the audit found unasserted, closed against a real vault by
`tests/e2e/assetDesignerRecoveryWalk.e2e.ts` and `tests/e2e/assetDesignerRecoveryWalkRestore.e2e.ts`,
and in the suite by `tests/presentation/designer/designerRecoveryScreenState.test.ts` and
`tests/application/queries/assetGeometryOrphanDiagnostics.test.ts`. Two corrections to the tables
above are folded in here too, per AD18-R27.

| Step | Clause | Discharged by |
| --- | --- | --- |
| 2 | same canvas, same selection through a read-only bit | **Left open, not closed.** `assetDesignerRecoveryWalk.e2e.ts`'s *draws the same canvas and selection through a read-only bit, and retires the early notice with no press* pins the picture, but no single-line `src/` mutation can falsify it — the attribute change reaches no plugin code at all. Recorded as a host pin, not a discharge; the row stays `obsidian` |
| 3 | no notice, no failure panel, every mode tool stays live | `designerRecoveryScreenState.test.ts` *recover.md step 3 — a refused write leaves everything else alone* › *draws no notice, no failure panel, and dims no mode tool* |
| 5 | the bowl springs back — the canvas, not only the file on disk | `designerRecoveryScreenState.test.ts` *recover.md step 5 — the bowl springs back* › *clears the preview and leaves the design store's own shape untouched* |
| 6 | the early notice clears with no press, within the plugin's own bound | `assetDesignerRecoveryWalk.e2e.ts` *draws the same canvas and selection through a read-only bit, and retires the early notice with no press* — `healed.pluginMs ≤ 2000`. **Left open**: whether the host raises the repair unprompted at all is recorded (13–18 ms and 1–5 ms on two runs here, 15 s with no reconcile once under load), not asserted; the row stays `obsidian` for that half |
| 7 | same bowl position, same selection, Inspector still shows the asset's fields | `designerRecoveryScreenState.test.ts` *recover.md step 7 — a kept-but-stale design keeps its position, its selection and its Inspector* › *keeps the footprint, the selection and the Inspector's own fields* |
| 8 | the notice arrives with no press, within the plugin's own bound | `assetDesignerRecoveryWalk.e2e.ts` *raises the notice with no press, and a press after the repair retires all three widgets at once* — `arrived.pluginMs ≤ 2000`. **Left open**: whether the host raises the write unprompted at all is recorded, not asserted; the row stays `obsidian` for that half |
| 21 | the second gesture persists and is drawn on reopen | `assetDesignerRecoveryWalkRestore.e2e.ts` *keeps both gestures across a tab closed by hand with no warning, and restores without ever drawing a failure panel* — bowl and tank both nudged before closing, revision 3 on disk, both parts' `centre-x` read back correctly after reopen |
| 21 | Redo is dimmed on reopen | same case — the Redo button's own `disabled` attribute, not only Undo's |
| 21 | nothing warned before the tab closed | same case — closed by the tab header's own close button rather than `leaf.detach()`, with an interruption recorder seeing no `.notice`, `.modal-container` or `.rp-dialog` |
| 23 | no flash of a failure panel | same case — a CDP-injected sampler watches from the page's own first script across the reload; `{ atStart: false, panelFrames: 0, panelMutations: 0 }`, with drawn frames after. **Record the scope**: this samples a renderer reload (`app:reload`), not a full process quit — a cold start is not sampled, though the row's own instruction stays a full quit for a human walk |
| 25 | no error is logged by the unload, in general, not only the two known substrings | `assetDesignerRecoveryWalkRestore.e2e.ts` *logs no error to the console while the plugin unloads with a designer open* — one known `console.error` injected as an instrument check before disable/enable, and exactly that one entry comes back |
| 26 | same shape | `assetDesignerRecoveryWalk.e2e.ts` *keeps the picture through a move, closes only its own tab once the note is gone, and leaves the orphan unspoken* — the full picture (Konva shapes by id and box) equal before and 2.5 s after the move |
| 26 | same Inspector | same case — the same picture's pressed rows and Inspector fields |
| 29 | no new tab opens, of any view type | same case — every leaf, of every type, equals the pre-close list minus only the closed one (not proxied through designer-leaf counting) |
| 29 | no other designer leaf is affected | same case — a second designer (Kept toilet) stays open, activates and draws its own name with no panel after the close |
| 30 | nothing told you | same case — an interruption recorder installed before the trash stays empty through the panel appearing |
| 30 | nothing offered to remove it | same case — the same recorder, read again just before the Close-this-tab click |
| 30 | record its size | same case — the `.rpgeo`'s byte size read before the delete and compared unchanged after, written to evidence as `orphan.json` |
| 30 | no diagnostic names it | `assetGeometryOrphanDiagnostics.test.ts` *recover.md step 30 — no diagnostic names the orphaned sidecar* › *leaves the orphan out of the real diagnostics report, though the file is still on disk* — the real `GetDiagnosticsSnapshotQuery` over a real repository stack, after a note deleted directly and the index rebuilt |
| 32 | revision does not move for step 5's validation refusal specifically | **Corrected citation (was OVERCLAIM above)**: not *counts one revision per landed write and undo…*, whose only validation refusal happens after step 11's stale notice is already up. The real discharge is `assetDesignerRecovery.e2e.ts`'s FIRST test, *refuses a write the OS forbids without a toast, and clears the error only on a write that lands* — `revision().toBe(1)` after the schema-99 nudge, checked before any stale notice exists in that flow |
| 37 | the notice, the button and the header's qualifier all clear together, and the canvas shows the repaired file | `assetDesignerRecoveryWalk.e2e.ts` *raises the notice with no press, and a press after the repair retires all three widgets at once* — one recorder reads all three together before and after the press (`['true,true,true', 'false,false,false']`), and the bowl's `centre-x` reads the repaired file's shifted value. **Also record**: this host heals the notice unprompted in about half a second (step 12's own mechanism), so the press is the door only while the host has not yet reconciled — the test holds the host's own watcher open to reach that state, which a human walk cannot do on demand |
