...
by construction rather than by omission: the palette's **Open asset designer**, the asset
library inspector's **Edit shape** and the Renovation project view's create-asset flow are all
bound to one `revealAssetDesigner`, which filters `getLeavesOfType` by the leaf's own persisted
`assetId` and reveals the first match without re-setting its view state. So a second leaf can
only come from **Obsidian's own tab split or a restored workspace layout**, and whether the
host offers that for a third-party `ItemView` is host behaviour this repository cannot settle.
`revealAssetDesigner.ts`'s docblock anticipates *"a vault reopened onto two Asset Designers"*
and nothing has ever seen one. **Step 1 is where that is found out.** If no gesture produces a
second leaf, steps 5 onward are unwalkable and the finding of this case is that sentence,
recorded in the Runs table — not a failed walk.
...
1. **Whether two leaves on one asset can exist at all.** This is the premise of row T12 and no
   code in this repository decides it. Step 1, with step 2 as the documented fallback.
