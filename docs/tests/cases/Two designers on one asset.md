...
Designers"* and nothing has ever seen one. **Step 1 is where that was found out.** It did not
fail: `twoDesigners.e2e.ts` and `twoDesignersMore.e2e.ts` both produce the pair, through Split
right and Split down, so nothing below is pared back to a set of survivors — every step is
walked (or now automated) as written.
...
## Why a human is the only instrument for one of these sixteen

Derived from the table below rather than remembered: of the eight rows that once carried a
verdict other than `suite` — 1, 2, 8, 10, 11, 14, 15 and 16 — **only step 10 still does.** The
other seven turned on the HOST, on real hardware, or on an eye, and each is now `e2e` (AD18-R28):
every clause of each is pinned by a real-Obsidian `npm run test:e2e` case, named under *Automated
in Obsidian* below, so nobody has to walk them by hand to find out. The reasoning that put all
eight here originally is kept below because it is what the automated cases had to reproduce, not
because it is still an open question:
...
1. **Whether two leaves on one asset can exist at all.** This is the premise of row T12 and no
   ...
   failed answer instead. **Closed**: `twoDesigners.e2e.ts`'s Split, `twoDesignersMore.e2e.ts`'s
   tab-menu walk and `twoDesignersDrag.e2e.ts`'s drag case answer it — Split right and Split down
   each produce a pair; Move to new window and the tab drag each move the one leaf instead of
   duplicating it.
2. **Whether Obsidian's own pointer capture survives a click in another leaf.** The conflict
   ...
   Chromium window is not a question any fake answers. Step 8, and it was the only `desktop` row
   here. **Closed, but not the way this reasoning expected**: `twoDesigners.e2e.ts` needs no
   second pointing device at all — the peer's write is fired by a timer inside the page while the
   driver holds one action chain, which is the only thing that can interleave with a held press.
3. **Whether anything at all tells the user their edit was refused.** The suite can assert that
   ...
   written as a judgement for that reason, and stays one — no instrument, human or automated,
   settles it.
4. **What a real `Notice` looks like, and that there is one at all for the sandwiched undo.**
   ...
   **Closed for the mechanism**: `twoDesigners.e2e.ts` shows the real `Notice` and its exact
   sentence; `reversibleAssetDesignWindows.test.ts` pins the content equality beneath it.
5. **Whether a write from outside the plugin reaches the same refusal.** `observed` is a digest
   ...
   held gesture needs it is a vault fact. Step 14. **Closed**: `twoDesigners.e2e.ts`'s
   external-rewrite case answers it — the same silent `Save error` badge, no toast, the external
   write's shape on disk.
6. **What is actually on disk afterwards.** Every version count in the suite is taken at an
   in-memory or probe store. Step 15 counts revisions in the real sidecar. **Closed**:
   `twoDesignersMore.e2e.ts` counts the real sidecar's `revision` across the whole walk in one
   vault.
7. **Whether a leaf closed and reopened rejoins the conversation.** `assetIdOf` reads the LEAF's
   ...
   matches — which only a real workspace restore exercises. Steps 2 and 16. **Closed**:
   `twoDesigners.e2e.ts` covers both — a real restart for step 2, a real close-and-reopen for
   step 16.
...
| --- | --- | --- | --- | --- |
| 1 | `e2e` | Open Asset K's designer. Then try, in order, every way Obsidian offers to duplicate that leaf: right-click the tab → **Split right**, **Split down**, **Move to new window**, and dragging the tab into a new split. **Record which of them produced a second Asset designer tab drawing Asset K, and which produced nothing or an empty leaf** | **Split right** and **Split down** each leave you with TWO leaves, both drawing Asset K's toilet — call them leaf A and leaf B. **Move to new window** and **dragging the tab into a new split do NOT**: `twoDesignersMore.e2e.ts` shows Move to new window moving the one leaf into a popout, and `twoDesignersDrag.e2e.ts` shows the tab drag moving it into the new split the same way — neither duplicates it. **CONTRARY, rewritten under AD18-R27**: this row used to leave the drag's outcome an open question ("record which of them produced a second… tab"); the build's answer is now pinned, and it is that only Split right and Split down produce the pair the rest of this case needs | The premise of matrix row T12. A second designer on one asset cannot be produced by any control this plugin owns: `revealAssetDesigner` filters `getLeavesOfType` by the leaf's persisted `assetId` and `revealCandidate` reveals the first match without calling `setViewState` on it. `revealAssetDesigner.ts`'s own docblock anticipates "a vault reopened onto two Asset Designers" — checked now by `twoDesigners.e2e.ts` *splits a designer into a second leaf on the same asset with Obsidian's own Split, and restores both*, `twoDesignersMore.e2e.ts` *duplicates a designer through the tab menu's Split right and Split down, and only moves it with Move to new window*, and `twoDesignersDrag.e2e.ts` *moves the one designer into a new split when its tab is dragged there, and never duplicates it* |
| 2 | `e2e` | With both leaves open, quit Obsidian and reopen the vault | Both leaves come back, both drawing Asset K. Neither is empty, neither shows a refusal, and neither has forgotten which asset it holds | **A pair PERSISTING, which is not the same question as a pair being obtainable** — this step presupposes what step 1 produced and cannot make a first one, so it is not step 1's fallback and nothing here is. `AssetDesignerView.getState` writes `{ assetId }` (with `''` as the sentinel for a leaf holding none) and `assetIdOf` reads the LEAF's state rather than `leaf.view`, deliberately, so a restored leaf whose view has been deferred still answers. `assetDesignerView.test.ts` "carries the open asset in its own view state, so a workspace restore reopens the same asset" drives the round trip against a fake leaf; a real restore is what this adds |
...
| 8 | `e2e` | **The step this case exists for.** Using one of the setups named in *The conflict window* above, press and hold a drag on the bowl in leaf B — moving it a clear distance, not a click — and, without releasing, make a write in leaf A. Then release in leaf B. **Record which setup you used and whether the held press survived at all** | **CONTRARY, rewritten under AD18-R27**: leaf B's header does NOT read Save error. `twoDesigners.e2e.ts`'s own comment states it plainly — leaf B's held drag is abandoned with no account at all: no write, no badge, no history entry, no toast — and its header reads the same plain **Saved** a leaf that never wrote would show. Leaf A's still reads **Saved just now**. Leaf B's canvas shows leaf A's change and NOT your drag. Nothing anywhere is paused, no dialog appears, and the asset's stored shape is leaf A's | C08's silent overwrite, at the one window a person can hold open. `DesignerSelectTool.begin` captures `design.geometryVersion` at the press and `commit` passes it as `expected`; the geometry store compares it inside its own per-asset queue, so read and compare are inseparable, and refuses with `asset-geometry.revision-conflict`. `designerWriteChain.test.ts` "stays conditional on the design it was replayed on, so a peer write during the live replayed drag refuses it" drives that ordering for a single leaf's own write chain and asserts the `save-error` state there; `setAssetFootprint.test.ts` "refuses the second of two writes built from the same revision, rather than losing one" asserts the stored shape is unchanged, because the refusal alone would be equally true of a build that wrote and then reported. `designerRefresh.test.ts` "answers false for writesBlocked, which this surface builds but never asks" discharges "nothing anywhere is paused" — this surface's `writesBlocked` is hard-wired `false`, so no gesture on it is ever paused, by construction. `designerRecoveryScreenState.test.ts` "opens no dialog (Two designers on one asset, step 8)" discharges "no dialog appears" over the same peer-mid-drag refusal. What only a host adds — and what it shows differently from the single-leaf vitest above — is two real leaves, two real Pinia instances, Obsidian's own pointer capture, and `twoDesigners.e2e.ts` "redraws the peer on a write, and DROPS a drag held across one — no write, no badge, no toast" as the pinning test for the header finding |
...
| 1 | dragging the tab into a new split | superseded — see `twoDesignersDrag.e2e.ts` below, added 2026-09-26 |
...
**Added 2026-09-26 (AD18 Task 4).** `tests/e2e/twoDesignersDrag.e2e.ts`. Step 1's last gesture —
dragging the tab into a new split — untried by the two files above because WebDriver's pointer
actions do not start an HTML5 drag in Chromium; this fires the `DragEvent` sequence a real one
produces (`dragstart`/`dragenter`/`dragover`/`drop`/`dragend`), which Obsidian's own tab-drag
handler reads regardless of `isTrusted`. What is NOT exercised is the operating system's own drag
loop that produces those events.

| Step | Clause | Discharged by |
| --- | --- | --- |
| 1 | dragging the tab into a new split | *moves the one designer into a new split when its tab is dragged there, and never duplicates it* — **CONTRARY**: the case's row used to claim this yields two leaves; the tab groups go from one to two but `leafStates` stays length 1, so the leaf was moved, not duplicated |
...
| — | — | **Superseded.** This row used to read "Not yet run in a vault" while the two dated runs above already recorded real `npm run test:e2e` passes — stale from the day the first of those runs landed. With steps 1, 2, 8, 11, 14, 15 and 16 now `e2e` (AD18-R28) and pinned by `twoDesigners.e2e.ts`, `twoDesignersMore.e2e.ts` and `twoDesignersDrag.e2e.ts`, there is nothing left in this case for a person to run in a vault except step 10's judgement, which has no Runs-table shape of its own — it belongs in the Outcome below. |

## Outcome

Whether a second designer leaf on one asset can exist in Obsidian at all, and which fault setup
produced the held-gesture conflict, are answered above by the Runs table and by
`twoDesigners.e2e.ts`, `twoDesignersMore.e2e.ts` and `twoDesignersDrag.e2e.ts` (AD18-R28). What
is still written here after a walk: what step 10's judgement answered, and anything only a live
vault showed that the automated cases above do not.
