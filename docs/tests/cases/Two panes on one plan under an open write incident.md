...
| 5 | `obsidian` | Open the **Asset designer** on any asset (from the asset library, or **Design an asset**) | The designer opens. **Its buttons are expected to look ENABLED — that is a recorded gap, not a failure.** What must hold is that no write lands: press **Edit dimensions**, commit a height, or drag a footprint vertex, and the design is unchanged. Record what the surface said | The designer's own gap being fixed without anyone noticing, and being met cold by a later reader as a new defect. BP-02 slice 4 gated the designer's TOOL FRAMEWORK (`EditorContext.writesBlocked`, which every registered tool consults) and did not disable its six control-bearing components; the drag a tool would have previewed is refused before it starts, and a button click is refused underneath |
...
| 7 | `obsidian` | Quit Obsidian entirely and reopen the vault, leaving both panes in the layout | Both restored panes are paused, and so is any third one you open. The incidents file is still on disk and unchanged | The durability half of ADR-0034 — the whole reason the record is a file rather than a flag. Also whether Obsidian restores two same-plan leaves at all, which nothing here can simulate |
...
- **An incident raised in another leaf while this pane is already open.** The registry notifies
  ...
  says is not reproducible by hand.
