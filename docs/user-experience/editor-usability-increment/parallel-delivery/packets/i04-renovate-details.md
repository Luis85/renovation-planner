...
**Status: not started.** Suggested model: `gpt-5.6-terra`, reasoning `high`. This is a bounded assignment, not a benchmark or a guarantee. Wave 1; prerequisites: I00 merged or explicitly dispositioned where conditional. Branch `codex/usability-i04-renovate-details`; worktree `.worktrees/usability-i04-renovate-details`.
...
Create the selected hybrid's Existing/Planned/Work hierarchy, primary eligible Add work and concise linked-content routes. Keep the selected identity and explicit Edit layout route. Reuse existing commands/target eligibility including room-less elements.
...
- New focused tests may be added only under `tests/presentation/editor/usability/i04-*.test.ts`.
- Write this packet's receipt at `docs/user-experience/editor-usability-increment/parallel-delivery/receipts/I04.md`.
...
- `src/presentation/editor/renovation/defaultRenovationContext.ts`
- `src/presentation/editor/planning/PlanningInspector.vue`

## Preserve

Do not modify costs, work schemas, source-of-truth ownership or Review behavior. Do not display zero where unknown.
...
```powershell
npm run check:fast -- tests/presentation/editor/renovationOverview.test.ts tests/presentation/editor/renovationRoutes.test.ts tests/presentation/editor/roomlessRecords.test.ts
```
...
```text
Implement only I04 (Focus Renovate on work and information) in the Renovation Planner repository.
Read docs/user-experience/editor-usability-increment/parallel-delivery/session-contract.md and packets/i04-renovate-details.md under that directory, plus the relevant hybrid screen contract. Follow repository AGENTS.md and .codex instructions if present.
Use model gpt-5.6-terra with high reasoning when available; do not silently change models.
Confirm the implementation request and that prerequisites I00 are merged/recorded before creating a fresh worktree from origin/main. Use branch codex/usability-i04-renovate-details under .worktrees/usability-i04-renovate-details; keep main clean.
Deliver this outcome: Create the selected hybrid's Existing/Planned/Work hierarchy, primary eligible Add work and concise linked-content routes. Keep the selected identity and explicit Edit layout route. Reuse existing commands/target eligibility including room-less elements.
Edit only this packet's owned paths and new tests/receipt. Preserve: Do not modify costs, work schemas, source-of-truth ownership or Review behavior. Do not display zero where unknown.
