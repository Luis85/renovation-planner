...
**Status: not started.** Suggested model: `gpt-5.6-terra`, reasoning `high`. This is a bounded assignment, not a benchmark or a guarantee. Wave 3; prerequisites: I00, I03 merged or explicitly dispositioned where conditional. Branch `codex/usability-i09-selection`; worktree `.worktrees/usability-i09-selection`.
...
First validate the current visible list/context route. If U4 adoption is recorded, expose a named chooser using current candidates; otherwise improve current guidance only and document the chooser disposition. Keep kind/context/current target and keyboard dismissal clear.

## Ownership

- `src/presentation/editor/selection/CanvasContextMenu.vue`
- `src/presentation/editor/shell/SelectionGuidance.vue`
- `styles/editor-context-menu.css`
- New focused tests may be added only under `tests/presentation/editor/usability/i09-*.test.ts`.
- Write this packet's receipt at `docs/user-experience/editor-usability-increment/parallel-delivery/receipts/I09.md`.
...
- `src/presentation/editor/selection/selection-store.ts`

## Preserve

Conditional scope is not permission to invent hit rules, nesting groups or a new selection model. Locked/hidden/handle rank behavior remains unchanged.
...
```powershell
npm run check:fast -- tests/presentation/editor/selection/resolveSelectionTarget.test.ts tests/presentation/editor/selection/menuKeyboard.test.ts tests/presentation/editor/selection/spatialSelection.test.ts
```
...
```text
Implement only I09 (Make overlapping targets understandable) in the Renovation Planner repository.
Read docs/user-experience/editor-usability-increment/parallel-delivery/session-contract.md and packets/i09-selection.md under that directory, plus the relevant hybrid screen contract. Follow repository AGENTS.md and .codex instructions if present.
Use model gpt-5.6-terra with high reasoning when available; do not silently change models.
Confirm the implementation request and that prerequisites I00, I03 are merged/recorded before creating a fresh worktree from origin/main. Use branch codex/usability-i09-selection under .worktrees/usability-i09-selection; keep main clean.
Deliver this outcome: First validate the current visible list/context route. If U4 adoption is recorded, expose a named chooser using current candidates; otherwise improve current guidance only and document the chooser disposition. Keep kind/context/current target and keyboard dismissal clear.
Edit only this packet's owned paths and new tests/receipt. Preserve: Conditional scope is not permission to invent hit rules, nesting groups or a new selection model. Locked/hidden/handle rank behavior remains unchanged.
