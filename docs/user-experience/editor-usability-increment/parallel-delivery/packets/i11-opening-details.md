...
**Status: not started.** Suggested model: `gpt-5.6-luna`, reasoning `high`. This is a bounded assignment, not a benchmark or a guarantee. Wave 4; prerequisites: I00, I03, I07 merged or explicitly dispositioned where conditional. Branch `codex/usability-i11-opening-details`; worktree `.worktrees/usability-i11-opening-details`.

## Visual reference

- [ux06-opening.png](../../mockups/images/ux06-opening.png)
...
Present Door/window/opening identity, real host context and current supported width/height/offset controls in a readable order. Keep hinge/swing secondary and explicit renovation route. Preserve room-less context.

## Ownership

- `src/presentation/editor/structure/StructureInspector.vue`
- `styles/editor-structure.css`
- New focused tests may be added only under `tests/presentation/editor/usability/i11-*.test.ts`.
- Write this packet's receipt at `docs/user-experience/editor-usability-increment/parallel-delivery/receipts/I11.md`.
...
No Door Rename invention, unattached openings, changed offset meaning or host reassignment logic. Verify exact current action names and retain validation.
...
```powershell
npm run check:fast -- tests/presentation/editor/structureSelection.test.ts tests/presentation/editor/openingMove.test.ts
```
...
```text
Implement only I11 (Clarify opening identity and host-relative properties) in the Renovation Planner repository.
Read docs/user-experience/editor-usability-increment/parallel-delivery/session-contract.md and packets/i11-opening-details.md under that directory, plus the relevant hybrid screen contract. Follow repository AGENTS.md and .codex instructions if present.
Use model gpt-5.6-luna with high reasoning when available; do not silently change models.
Confirm the implementation request and that prerequisites I00, I03, I07 are merged/recorded before creating a fresh worktree from origin/main. Use branch codex/usability-i11-opening-details under .worktrees/usability-i11-opening-details; keep main clean.
Deliver this outcome: Present Door/window/opening identity, real host context and current supported width/height/offset controls in a readable order. Keep hinge/swing secondary and explicit renovation route. Preserve room-less context.
Edit only this packet's owned paths and new tests/receipt. Preserve: No Door Rename invention, unattached openings, changed offset meaning or host reassignment logic. Verify exact current action names and retain validation.
