...
**Status: not started.** Suggested model: `gpt-5.6-terra`, reasoning `high`. This is a bounded assignment, not a benchmark or a guarantee. Wave 4; prerequisites: I00, I03, I09 merged or explicitly dispositioned where conditional. Branch `codex/usability-i12-clipboard-feedback`; worktree `.worktrees/usability-i12-clipboard-feedback`.

## Visual reference

- [ux07-copy-paste.png](../../mockups/images/ux07-copy-paste.png)
...
Add bounded presentation feedback over actual snapshot/result counts, target floor and excluded renovation content. Retain direct Paste and one undoable result. Reuse existing notice/Details components; request any shared composition update from integrator.

## Ownership

- `src/presentation/editor/clipboard/clipboardActions.ts`
- `src/presentation/editor/clipboard/editorClipboard.ts`
- New focused tests may be added only under `tests/presentation/editor/usability/i12-*.test.ts`.
- Write this packet's receipt at `docs/user-experience/editor-usability-increment/parallel-delivery/receipts/I12.md`.
...
Direct Paste and keyboard routing already exist: do not rebuild them. No formats, OS clipboard, dependency closure, new preview lifecycle, domain or history changes.
...
```powershell
npm run check:fast -- tests/presentation/editor/clipboard.test.ts tests/application/commands/pasteCommand.test.ts tests/presentation/editor/inputInteractions.test.ts
```
...
```text
Implement only I12 (Explain copied scope and direct Paste recovery) in the Renovation Planner repository.
Read docs/user-experience/editor-usability-increment/parallel-delivery/session-contract.md and packets/i12-clipboard-feedback.md under that directory, plus the relevant hybrid screen contract. Follow repository AGENTS.md and .codex instructions if present.
Use model gpt-5.6-terra with high reasoning when available; do not silently change models.
Confirm the implementation request and that prerequisites I00, I03, I09 are merged/recorded before creating a fresh worktree from origin/main. Use branch codex/usability-i12-clipboard-feedback under .worktrees/usability-i12-clipboard-feedback; keep main clean.
Deliver this outcome: Add bounded presentation feedback over actual snapshot/result counts, target floor and excluded renovation content. Retain direct Paste and one undoable result. Reuse existing notice/Details components; request any shared composition update from integrator.
Edit only this packet's owned paths and new tests/receipt. Preserve: Direct Paste and keyboard routing already exist: do not rebuild them. No formats, OS clipboard, dependency closure, new preview lifecycle, domain or history changes.
