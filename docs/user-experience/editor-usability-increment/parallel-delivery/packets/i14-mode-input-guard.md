# I14 — Prevent accidental geometry editing in Renovate

**Status: not started.** Suggested model: `gpt-5.6-terra`, reasoning `high`. This is a bounded assignment, not a benchmark or a guarantee. Wave 5; prerequisites: I00, I04, I08, I10, I11 merged or explicitly dispositioned where conditional. Branch `codex/usability-i14-mode-input-guard`; worktree `.worktrees/usability-i14-mode-input-guard`.
...
Hide geometry handles and guard pointer/keyboard mutation entrances in default Renovate while keeping selection/pan/navigation and eligible renovation actions. Edit layout enters Plan deliberately. Preserve existing draft confirmation, busy guard and Review-return snapshot.
...
- New focused tests may be added only under `tests/presentation/editor/usability/i14-*.test.ts`.
- Write this packet's receipt at `docs/user-experience/editor-usability-increment/parallel-delivery/receipts/I14.md`.
...
- `src/presentation/editor/escapeRouting.ts`

## Preserve

CSS-only hiding is insufficient. No source schemas/history rewrite. If guard must move into SelectTool/root, integrator owns that explicit extension and reruns all input boundaries.
...
```text
Implement only I14 (Prevent accidental geometry editing in Renovate) in the Renovation Planner repository.
Read docs/user-experience/editor-usability-increment/parallel-delivery/session-contract.md and packets/i14-mode-input-guard.md under that directory, plus the relevant hybrid screen contract. Follow repository AGENTS.md and .codex instructions if present.
Use model gpt-5.6-terra with high reasoning when available; do not silently change models.
Confirm the implementation request and that prerequisites I00, I04, I08, I10, I11 are merged/recorded before creating a fresh worktree from origin/main. Use branch codex/usability-i14-mode-input-guard under .worktrees/usability-i14-mode-input-guard; keep main clean.
Deliver this outcome: Hide geometry handles and guard pointer/keyboard mutation entrances in default Renovate while keeping selection/pan/navigation and eligible renovation actions. Edit layout enters Plan deliberately. Preserve existing draft confirmation, busy guard and Review-return snapshot.
Edit only this packet's owned paths and new tests/receipt. Preserve: CSS-only hiding is insufficient. No source schemas/history rewrite. If guard must move into SelectTool/root, integrator owns that explicit extension and reruns all input boundaries.
