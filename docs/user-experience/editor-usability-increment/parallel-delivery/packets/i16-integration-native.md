...
**Status: not started.** Suggested model: `gpt-5.6-terra`, reasoning `high`. This is a bounded assignment, not a benchmark or a guarantee. Wave 8; prerequisites: I15 merged or explicitly dispositioned where conditional. Branch `codex/usability-i16-integration-native`; worktree `.worktrees/usability-i16-integration-native`.
...
Reconcile only approved shared wiring/import/locale requests and remove superseded conflicting style declarations. Verify final integrated tree, CI and disposable native-vault journeys: reopen, PDF, two leaves, focus and recovery. Record any blocked human/AT checks honestly.
...
- New focused tests may be added only under `tests/presentation/editor/usability/i16-*.test.ts`.
- Write this packet's receipt at `docs/user-experience/editor-usability-increment/parallel-delivery/receipts/I16.md`.
...
- `package.json`
- `CLAUDE.md`
- `docs/tests/cases/Configure a reference plan.md`

## Preserve

No feature expansion or silent merging of unknown changes. Run npm run check only on final integration, after relevant subsets; never run concurrent heavy gates in one checkout.
...
```powershell
npm run check:fast -- tests/presentation/editor/shell.test.ts tests/presentation/editor/inputInteractions.test.ts tests/presentation/editor/clipboard.test.ts tests/presentation/editor/referenceWorkflow.e2e.test.ts
```
...
```text
Implement only I16 (Integrate final code and record native release evidence) in the Renovation Planner repository.
Read docs/user-experience/editor-usability-increment/parallel-delivery/session-contract.md and packets/i16-integration-native.md under that directory, plus the relevant hybrid screen contract. Follow repository AGENTS.md and .codex instructions if present.
Use model gpt-5.6-terra with high reasoning when available; do not silently change models.
Confirm the implementation request and that prerequisites I15 are merged/recorded before creating a fresh worktree from origin/main. Use branch codex/usability-i16-integration-native under .worktrees/usability-i16-integration-native; keep main clean.
Deliver this outcome: Reconcile only approved shared wiring/import/locale requests and remove superseded conflicting style declarations. Verify final integrated tree, CI and disposable native-vault journeys: reopen, PDF, two leaves, focus and recovery. Record any blocked human/AT checks honestly.
Edit only this packet's owned paths and new tests/receipt. Preserve: No feature expansion or silent merging of unknown changes. Run npm run check only on final integration, after relevant subsets; never run concurrent heavy gates in one checkout.
