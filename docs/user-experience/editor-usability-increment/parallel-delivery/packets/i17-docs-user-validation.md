# I17 — Align help and collect novice acceptance evidence

**Status: not started.** Suggested model: `gpt-5.6-luna`, reasoning `high`. This is a bounded assignment, not a benchmark or a guarantee. Wave 8; prerequisites: I15 merged or explicitly dispositioned where conditional. Branch `codex/usability-i17-docs-user-validation`; worktree `.worktrees/usability-i17-docs-user-validation`.
...
Update current visible routes in user help and relevant cases with dated amendments, then prepare scorecards/recruitment-ready protocol. Record actual user/AT results only when human sessions occur. Keep separate counts for first-use, mode comprehension and secondary tasks.

## Ownership

- `docs/using-plan-editor.md`
- New focused tests may be added only under `tests/presentation/editor/usability/i17-*.test.ts`.
- Write this packet's receipt at `docs/user-experience/editor-usability-increment/parallel-delivery/receipts/I17.md`.
...
- `docs/user-experience/editor-usability-increment/validation-plan.md`
- `docs/user-experience/editor-usability-increment/mockups/hybrid-screens.md`

## Preserve

No fabricated participant data or replacing native checks with docs. This model can prepare/synthesize records; a human-run study and product release decision remain required.
...
- Verify changed Markdown links, exact current UI wording and actual run evidence; no application test is claimed for prose-only edits.
...
```text
Implement only I17 (Align help and collect novice acceptance evidence) in the Renovation Planner repository.
Read docs/user-experience/editor-usability-increment/parallel-delivery/session-contract.md and packets/i17-docs-user-validation.md under that directory, plus the relevant hybrid screen contract. Follow repository AGENTS.md and .codex instructions if present.
Use model gpt-5.6-luna with high reasoning when available; do not silently change models.
Confirm the implementation request and that prerequisites I15 are merged/recorded before creating a fresh worktree from origin/main. Use branch codex/usability-i17-docs-user-validation under .worktrees/usability-i17-docs-user-validation; keep main clean.
Deliver this outcome: Update current visible routes in user help and relevant cases with dated amendments, then prepare scorecards/recruitment-ready protocol. Record actual user/AT results only when human sessions occur. Keep separate counts for first-use, mode comprehension and secondary tasks.
Edit only this packet's owned paths and new tests/receipt. Preserve: No fabricated participant data or replacing native checks with docs. This model can prepare/synthesize records; a human-run study and product release decision remain required.
