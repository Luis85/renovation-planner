...
The parent released the lease on 2026-09-15. Complete JSON/schema, local-link, image decode/hash and path validation now passes; [machine-readable results](parallel-delivery/validation-result.json) retain exact counts and hashes. The initial run correctly found five JPEG captures mislabeled PNG; filenames were corrected without byte changes. Production UI did not change, so Impeccable detector/full local production verification is not required for this documentation branch; its PR CI verifies the imported combined source. No universal docs-only test command exists in `package.json`.

## Pre-dispatch package gate

From the planning worktree run `python docs/user-experience/remplanner-adoption/parallel-delivery/validate.py` with `jsonschema` and Pillow available. This run installed jsonschema 4.26.0 and Pillow 12.3.0 in the isolated temporary `remplanner-doc-validation` directory, exposed through `PYTHONPATH`; no repository dependency or lockfile changed. It checks manifest structure and full Draft 2020-12 schema, dependency safety, local Markdown targets, existing ownership/test paths, forbidden absolute ownership paths, decoded image formats/dimensions and SHA-256. Missing validation dependencies fail the gate. Subsequent image hashes are compared to the committed evidence hash ledger.

Run `git diff --check`. Confirm main by read-only status, baseline ancestry and remote planning branch identity. Final wall/color receipts are reconciled; coding begins only after the plan PR is green. No dependency SHA may be guessed. Re-run package path checks after each stack placement.
