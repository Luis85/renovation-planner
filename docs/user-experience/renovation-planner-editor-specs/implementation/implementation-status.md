...
The finalization tree incorporates UI `adaeb782` (detail layouts `c080b0a9`, Object `aa9896db`), recovery `00adf082` (including `2c353329`), persistent shell/focus `cad06deb` and final recovery probe preparation `a4e902de`, over PR #88 `3c1c737a`. Generic spatial creation, numeric Room/Area editing, connected Work/Materials/Costs/Evidence navigation, mixed spatial deletion and persistent native controls are implemented. The Object presentation checkpoint is incorporated, including all eleven Add routes.

The pushed integration `24ee3177` passes production build/types, full lint and all 580 test files locally and in inspected Node 24/Windows CI logs. Its unchanged coverage floors fail: 98.48% statements / 96.80% branches / 98.42% functions / 99.28% lines. Separate full Fallow diagnostics find no dead code or clones but three cognitive-complexity findings. Spatial/modal focused regressions and cognitive-complexity refactors are implemented. The joined 89 focused cases pass across the initial run and corrected six-case rerun; types, scoped ESLint and whole Oxlint pass. Fresh global coverage is still required: Fallow now has zero cognitive findings but four estimated CRAP findings against the previous coverage artifact. UI M12 compact rows, M13 Work groups, contextual navigation and M14 photo-gallery/phase controls are now integrated locally through `54717024`, with owner browser evidence. The next combined gate and all-18 final reference captures remain pending. Full CI on `4dbc96dd` passed build/types/Oxlint, then found the joined renovation action factory at 102 lines versus the unchanged 100-line limit. The shared retry callback extraction and reviewed Renovate wall controls now pass full npm run lint and vue-tsc, plus 41 focused follow-up tests. The full gate and fresh global coverage remain pending. UI `408b443f` supplies preliminary overview and resilience evidence (four scenarios each); the repeated-rail product focus correction is separately owned by the recovery task. The local intermediate checkpoint includes E `37da445f` as `0fff8855`, modal `75084550`, UI `faeba386`, Room gesture controls, generic draft retry reporting and focused spatial guard regressions. The three new PR #90 projection/recovery review fixes and their final lint correction are integrated. Modal native-control checkpoint `75084550` is integrated as `253924fa`, with 72 focused tests, complete scoped static checks and four owner browser scenarios passing. UI `faeba386` adds shared-target highlight deduplication and the final modal driver; integrated HEAD before pending source edits is `9b2c32bb`. The M00 audit found hidden Renovate Room handles, primary actions and absent direct-action/dimension UI; those concrete requirements are being corrected before final acceptance. No complete gate or visual/live acceptance pass is claimed.

M10 Trade assignment/schedule and M13 quote comparison remain unresolved requirements: accepted ADR-0021 limits current responsibility to DIY/unassigned; ADR-0022 calls downstream features open within its bounded slice, which is not a full-goal deferral. No matching downstream implementations exist. User scope clarification is pending.
