...
The finalization tree joins UI through `408b443f`, recovery through `471cfc57`, modal controls `75084550`, and the root spatial/control continuation over PR #88 `3c1c737a`. All eleven Add routes, generic spatial objects, Room/Area precision, contextual Work/Materials/Costs/Evidence, compact material/cost layouts, gallery controls, persistent native focus, guarded drafts and exact history are implemented. Renovate now exposes Room and reviewed-wall handles plus Select/Add; native inline dimensions and direct-action popovers for M00/M07 remain under active UI implementation.

Verification is partial and explicit. The 89 joined spatial/modal cases have passing evidence across the initial run and corrected six-case rerun; the shared retry/M07 follow-up passes 41/41 cases. Full npm run lint and vue-tsc pass after correcting the combined 102-line renovation factory through a shared callback extraction. Owner recovery evidence adds 119/119 cases, and preliminary overview/resilience browser checks pass four scenarios each. The earlier complete coverage result on `24ee3177` failed the unchanged floors (98.48% statements / 96.80% branches / 98.42% functions / 99.28% lines); `4dbc96dd` CI stopped at the subsequently fixed full-lint finding. Complete CI on `d25805f7` then passed build/full lint and 590/591 test files, with one new German action-label vocabulary error (now corrected to the established “Baustoff” wording). Coverage improved to 99.05% statements / 97.54% branches / 99.15% functions / 99.50% lines; only the unchanged branch floor still fails. Fresh joined global coverage/health and the final all-18 reference/journey matrix remain pending. No complete gate or visual/live acceptance pass is claimed.
