...
Current acceptance is governed by the [completion matrix](completion-matrix.md) and [implementation status](implementation-status.md). M10 Trade/manual scheduling and M13 Supplier/Quote comparison are implemented in the integrated production source under ADR-0024; the older statements below about missing implementation or pending scope clarification describe their dated checkpoints. Full CI on `5557385f` passes build/types, lint and 8,001 tests on all four legs but fails the unchanged statement/branch coverage floors. The restarted browser run passes six four-scenario journeys and stops in German recovery at a resize/layout synchronization race; final nine-journey/eighteen-reference acceptance, the M14 caption correction and live-host acceptance remain open. See [current downstream evidence](downstream-planning-evidence.md).

## Historical checkpoint 63173897 and earlier scope audit
