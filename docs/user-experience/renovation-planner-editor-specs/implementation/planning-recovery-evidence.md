...
| Different failure classes | Validation/write/conflict messages remain distinct; unexpected hydration rejection becomes a read failure. Uncompensated operations keep a sticky warning — the LEAF's, carried in Obsidian's view state, so a settings rebind keeps it — even after a successful read. Existing conditional compensation tests remain authoritative; there is no general durable journal. |
