...
async function accessibility(page, scenario, out, state) {
	...
	const report = { violations: result.violations, incomplete: result.incomplete };
	await writeFile(`${out}/${scenario.name}-${state}-axe.json`, JSON.stringify(report, null, 2));
	assert.deepEqual(result.violations.map(item => ({ id: item.id, impact: item.impact, nodes: item.nodes.map(node => node.target) })), [], 'Object workflow has no automated WCAG violations');
