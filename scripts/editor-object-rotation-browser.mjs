...
export async function verifyObjectRotationPointer(page, scenario, out, id) {
	...
	assert.equal(await page.locator('.rp-wall-canvas-actions').count(), 0, 'wall controls do not cover rotation feedback');
	...
	assert.equal(await page.locator('[data-rp-canvas-detail]').count(), 0, 'selection has no redundant Add detail action');
