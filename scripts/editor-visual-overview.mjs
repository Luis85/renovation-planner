...
import { captureOverviewMatching } from './editor-matching-views.mjs';
...
async function journey(page, scenario, out) {
	...
	matchingViews.room = await captureOverviewMatching(page, scenario, out, 'room');
	...
	matchingViews.floor = await captureOverviewMatching(page, scenario, out, 'floor');
	...
	matchingViews.wall = await captureOverviewMatching(page, scenario, out, 'wall');
	if (scenario.width === 460) await page.keyboard.press('Escape');
	...
	return { theme, dimension, directActionsAccessibility, matchingViews, storage: 'production commands and repositories over FakeVault', state: 'committed reference, wall loop, Room, Existing/Planned/Work; overview, Floor and shared Work preview/apply', selection: 'keyboard additive wall membership retained after one shared command' };
}
