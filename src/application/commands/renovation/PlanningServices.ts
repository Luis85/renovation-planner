...
export function planningServices(deps: PlanningDeps): PlanningServices {
 return { read: id => readPlanning(deps, id), material(baseline, input, ledger) { const command = new MaterialCommand(deps, baseline, input, ledger); return { execute: () => command.execute(), undo: () => command.undo() }; } };
}
