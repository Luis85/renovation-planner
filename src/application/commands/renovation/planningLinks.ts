...
export function validateMaterialLinks(requirement: Requirement, baseline: PlanningBaseline, renovation = baseline.plan.entity.renovation ?? EMPTY_RENOVATION) {
	...
	if (source.workId && !renovation.work.some(item => item.id === source.workId && item.roomId === requirement.origin.zoneId)) return err(depthError());
...
export function validateDepthLinks(renovation: Renovation, baseline: PlanningBaseline) {
	...
	const records = new Map([...renovation.subjects, ...renovation.work, ...renovation.decisions, ...depth.costs].map(item => [item.id, item.roomId]));
	for (const material of baseline.materials) {
		records.set(material.entity.id, material.entity.origin.zoneId);
	...
	if (depth.evidence.some(item => item.recordId && records.get(item.recordId) !== item.roomId)) return err(depthError());
	return ok(undefined);
}
