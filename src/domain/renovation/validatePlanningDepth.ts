...
import { hasRoomContext, validSharedLinks, type RoomContext } from './SharedLinks';
...
export function validatePlanningDepth(value: PlanningDepth, renovation: Renovation) {
	...
	if (all.some(item => !item.id || !item.roomId || !item.targetId || (item.workId && !hasRoomContext(work.get(item.workId), item.roomId)))) return err(depthError());
	...
	const linked = new Map([...renovation.subjects, ...renovation.work, ...renovation.decisions, ...value.costs].map(item => [item.id, item]));
