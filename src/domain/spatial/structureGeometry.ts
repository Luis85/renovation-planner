...
export function scaleStructure(structure: Structure, factor: number): Structure {
	...
	return { ...structure,
		...(structure.elements ? { elements: structure.elements.map(element => ({ ...element, points: element.points.map(point), ...(element.stair ? { stair: { ...element.stair, width: element.stair.width * factor } } : {}), ...(element.width !== undefined ? { width: element.width * factor } : {}), ...(element.labelOffset ? { labelOffset: { dx: element.labelOffset.dx * factor, dy: element.labelOffset.dy * factor } } : {}) })) } : {}),
