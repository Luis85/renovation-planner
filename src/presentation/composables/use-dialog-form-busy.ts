 ...
 *
 * **What the returned refusal is for, and why it is not `:disabled`.** While a write is in
 * flight no control may be `disabled`, and that is a FOCUS rule rather than a styling
 * preference — `FormDialog.vue`'s docblock states it as an invariant of the framework.
 * Chromium moves focus to `<body>` when the element holding it is disabled, and `<body>` is
 * outside `.rp-dialog`, where `DialogHost` binds its `keydown` listener: disabling the focused
 * control takes `Escape` and the whole Tab trap out for exactly the window `busy` exists to
 * make `Escape` refuse DELIBERATELY. So controls stay focusable and are made INOPERATIVE
 * instead — `readonly` on a text input, `aria-disabled` on a button — and this is what makes
 * that refusal real.
 *
