 ...
 * that middle, because a long thin object facing across its short side would otherwise clip it.
 *
...
export function frontPreview(shape: AssetShape) {
	...
	const half = side / 2 + side * MARGIN;
	return {
		viewBox: `${middle.x - half} ${middle.y - half} ${2 * half} ${2 * half}`,
		footprint: presetPreview(shape).footprint,
		shaft: [middle.x, middle.y, base.x, base.y],
		head: [tip.x, tip.y, base.x - dy * barb, base.y + dx * barb, base.x + dy * barb, base.y - dx * barb],
	};
}
