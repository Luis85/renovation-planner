...
export interface ElementResizeGesture {
 ...
 */
export class ElementResize extends PreviewedDrag<ElementResizeGesture, ResizedGeometry> {
	constructor(private readonly deps: ElementResizeDeps) { super(); }
	 ...
	 */
	protected proposed(gesture: ElementResizeGesture, event: EditorPointerEvent): ResizedGeometry | null {
	...
	protected preview(gesture: ElementResizeGesture, next: ResizedGeometry): void {
		this.deps.previewResize?.(gesture.frame.element.id, next);
	}

	protected drop(gesture: ElementResizeGesture, next: ResizedGeometry): void {
		gesture.context.renderState.snapGuides = [];
