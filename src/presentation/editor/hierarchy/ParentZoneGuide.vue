...
/**
 * The parent zone's outline on a detail plan (ADR-0028): dashed, captioned with the zone and the
 * plan it comes from, never a hit candidate, never listed, never saved into this plan, and
 * derived from the parent on every hierarchy read, so recalibrating this plan does not move it.
 * Mounted inside the background layer, so it hides with the reference and adds no eighth layer
 * to §17's seven.
 */
...
import { guideOutline, guideSource } from './parentZoneGuide';
...
const source = computed(() => guideSource(hierarchy.value));
