...
/**
 * A Konva layer with nothing in it yet — a placeholder for layers whose content belongs to
 * slices that do not exist (currently `ConstructionLayer`).
 *
 * One component with a `layerId` rather than separate files for each placeholder layer:
 * there is nothing to say about any of them yet that is not said by their position in §17's
 * order, and duplicate files would create multiple places to forget when the transform or the
