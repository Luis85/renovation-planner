...
<template>
	<label>{{ tr('planning.catalogue') }}
		<select
			...
			name="asset"
			@change.capture="restoreInoperativeChoice($event, draft.assetId)"
		>
	...
	<label>{{ tr('planning.rule') }}
		<select
			...
			:aria-disabled="paused"
			name="rule"
			@change.capture="restoreInoperativeChoice($event, draft.source.rule)"
		><option
	...
	<label>{{ tr('planning.state') }}
		<select
			...
			:aria-disabled="paused"
			name="state"
			@change.capture="restoreInoperativeChoice($event, draft.source.state)"
