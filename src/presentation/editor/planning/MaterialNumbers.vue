...
<template>
	<dl class="rp-planning-totals">
		<dt>{{ tr('planning.needed') }}</dt><dd>{{ formatPlanningNumber(row.needed) }} {{ row.entity.unit }} · {{ tr(row.entity.quantity.override || row.source.rule === 'manual' ? 'planning.manual' : 'renovation.calculated') }}</dd><dt>{{ tr('planning.purchased') }}</dt><dd>{{ formatPlanningNumber(row.procurement?.purchased || '0') }}</dd><dt>{{ tr('planning.reserved') }}</dt><dd>{{ formatPlanningNumber(row.procurement?.reserved || '0') }}</dd><dt>{{ tr('planning.outstanding') }}</dt><dd>{{ formatPlanningNumber(row.outstanding) }} {{ row.entity.unit }}</dd><dt>{{ tr('planning.planned') }}</dt><dd>{{ formatPlanningMoney(row.cost) }}</dd>
	</dl>
