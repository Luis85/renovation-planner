...
export function useRenovationContextLabel(): (link: SpatialLink) => string {
	...
	const names = computed(() => new Map([...project.zones.values(), ...structureRecords(project.structure, project.plan?.id ?? '', project.plan?.spatialElements)].map(item => [item.id, item.name])));
	return link => {
