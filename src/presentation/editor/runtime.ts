...
function buildRuntime(context: PlanEditorContext): Omit<EditorRuntime, 'renovation' | keyof EditorFormActions> {
	...
	const { elementTask, elementActions, groupActions, rotationActions, toolBindings } = createSpatialEditing(context, { toolManager, returnToSelect, activeToolId, dispatcher: wrappedDispatcher, writesBlocked, refreshProjection, renderState, ledger, structureTask, wall: structureActions, openPlanNote: () => context.openPlanNote() });
	...
	return {
		...
		structureTask, structureActions, elementTask, elementActions, groupActions, rotationActions,
