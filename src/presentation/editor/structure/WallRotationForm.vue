...
const degrees = computed(() => parseRotationDegrees(text.value));
const proposal = computed(() => degrees.value === null ? null : rotateWallStructure(props.structure, props.wallId, degrees.value));
...
const invalid = computed(() => degrees.value === null ? tr('editor.rotation.invalid') : proposal.value && !proposal.value.ok ? spatialMessage(proposal.value.error) : null);
let alive = true;
...
<template>
	<form class="rp-dialog-form" data-rp-form="wall-rotation" @submit.prevent="submit" @keydown="nativeSubmitKey">
		<DraftRecovery v-if="blocked.value && !busy.value && !retired.value" :retry="retry" :open-source="openSource" />
		<p>{{ tr('editor.rotation.wall-hint') }}</p>
		<FieldError v-slot="{ inputId, aria }" :message="invalid">
			<label :for="inputId" class="rp-dialog-field">{{ tr('editor.rotation.degrees') }}
				<input :id="inputId" v-bind="aria" name="angle" type="text" inputmode="decimal" :value="text" :readonly="paused" @input="input">
			</label>
		</FieldError>
		<p v-if="reviewed && changed" role="status">{{ impact }}</p>
		<p v-if="reviewed && rooms.length">{{ tr('editor.structure.room-impact', { rooms: rooms.join(', ') }) }}</p>
		<p v-if="error" role="alert">{{ spatialMessage(error) }}</p>
		<p v-if="conflict || retired.value" role="status">{{ tr('editor.structure.conflict') }}</p>
		<button type="submit" class="rp-dialog-button" :aria-disabled="paused || !changed">{{ tr(reviewed ? 'editor.rotation.wall-apply' : 'editor.structure.preview') }}</button>
	</form>
</template>
