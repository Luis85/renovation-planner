...
export function structureCandidates(structure: Structure, shapeOf: ShapeLookup = NO_SHAPES, marks?: DraftingHitContext): SpatialObjectCandidate[] {
	...
	return [
		...
		...structure.walls.map(wall => ({ id: wall.id, kind: 'wall' as const, points: [wall.start, wall.end], bulges: [wall.bulge ?? 0, 0], width: wall.thickness, ...wallHits.get(wall.id) })),
		...structure.openings.map(opening => {
			...
			return { id: opening.id, kind: 'opening' as const, points: openingPoints(opening, structure.walls), bulges: [bulge, 0], width: host?.thickness, ...wallHits.get(opening.id) };
		}),
	];
}
