...
export function structureRecords(structure: Structure, planId: string, metadata: readonly SpatialElementMetadata[] = []): SpatialRecordDto[] {
	...
	return [
		...
		...structure.openings.map(opening => {
			...
			return { kind: 'opening' as const, id: opening.id, planId, name: tr(`editor.add.${opening.kind}.label`), zoneType: 'Opening', points: openingPoints(opening, structure.walls), bulges: [host ? Math.tan(Math.atan(host.bulge ?? 0) * opening.width / wallLength(host)) : 0, 0], areaMm2: 0 };
		}),
	];
}
