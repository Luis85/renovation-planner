...
export function createWallRotationActions(context: PlanEditorContext,
	...
	async function rotateWall(id: string, degrees = 0, original?: Wall): Promise<void> {
		...
		if (!alive || state.active.value || rotationBlocked.value || dialogs.current || !services || selection.selectedIds.length !== 1 || selection.selectedIds[0] !== id) return;
		state.active.value = true;
		try {
			const read = await services.read(context.planId as PlanId);
			if (!alive || epoch !== captured || rotationBlocked.value) return;
			...
			if (!sameGeometryDocument({ ...baseline.document, structure: project.structure, calibration: project.plan?.calibration ?? null }, baseline.document) || (original && !sameWall(original, wall))) {
			...
			await dialogs.openDialog({ kind: 'form', title: tr(id === wall.id ? 'editor.rotation.wall-title' : 'editor.rotation.host-title', { name }), component: markRaw(WallRotationForm), busy, props: {
				...
				preview: (value: Structure | null) => { state.preview.value = alive && epoch === captured && !rotationBlocked.value ? value : null; },
				dispatch: (next: Structure) => !alive || epoch !== captured || rotationBlocked.value ? Promise.resolve(err(staleWriteRefusal())) : runtime.dispatcher.run(services.command({ planId: context.planId as PlanId, baseline, structure: next, ledger })),
			} });
