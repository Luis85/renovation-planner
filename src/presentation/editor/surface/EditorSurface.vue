 ...
 * camera and the active tool. Lifted out of `PlanCanvas.vue` unchanged so that a second canvas
 * surface can mount the same doors; what is DRAWN inside it is whatever the default slot holds.
 *
 ...
 * stays free of zones and selections. `null` means there is nothing to frame.
 */
...
function gestureInFlight(): boolean {
	return toolManager.gestureInFlight || editor.dragState !== null;
}
...
function isGestureOwner(pointerId: number): boolean {
	if (toolManager.gestureInFlight) return toolGesturePointer === pointerId;
...
function onPointerDown(event: PointerEvent): void {
	...
	// swallowed release.
	if (toolManager.gestureInFlight && toolGesturePointer !== event.pointerId) {
		event.preventDefault();
		swallowedPointers.add(event.pointerId);
		return;
	}
...
function onPointerMove(event: PointerEvent): void {
	...
	if (activeToolId.value !== null) {
		...
		if (toolManager.gestureInFlight && toolGesturePointer !== event.pointerId) return;
		if (toolManager.gestureInFlight && (event.buttons & PRIMARY_BUTTON_BIT) === 0) {
			toolManager.pointerUp(pointerEventAt(event, at, 'primary'));
			return;
		}
...
function onPointerUp(event: PointerEvent): void {
	...
	if (activeToolId.value !== null) {
		...
		if (isPrimary(event)) toolManager.pointerUp(editorPointerEvent(event, stagePoint(event)));
		return;
	}
 ...
 * through `props.finishArea` (guarded — see that prop); a wall chain or a hosted opening through
 * the structure tool's own `finish`, which asks its task's `blocked` itself. Only a PLAIN Enter:
 * not a repeat, not a chord, not a keystroke an IME is still composing (`plainPress`).
 * `preventDefault` runs for every Enter while one of those tools is active, chorded or not, so
 * nothing beneath the canvas activates on it. Its own function beside `fitShortcut` for the
 * same reason that one is: `onKeyDown` was at its cognitive-complexity budget the day the merge
 * added this branch to it.
 */
function finishShortcut(event: KeyboardEvent): boolean {
	...
	if (tool === 'draw-area') props.finishArea();
	else toolManager.finishActiveTool();
	return true;
}
 ...
 * focusable inside it.
 *
 * The empty states are overlays INSIDE this element (`<slot name="overlay" />` below), and
