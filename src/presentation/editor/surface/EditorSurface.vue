...
/**
 * How fast a wheel notch zooms. Exponential, so the feel is the same at every scale — the
 * keyboard's `+`/`-` step (`./keyDoors.ts`) is a ratio for the same reason.
 */
...
/**
 * The keyboard doors — `./keyDoors.ts`, lifted out whole when this file reached its line
 * budget. What is handed over is every member a door reads and nothing else, so the boundary
 * is legible from this one call: `host` is the props, read at call time exactly as the doors
...
<template>
	...
	>
		...
		<!--
			...
			**The `.stop` modifiers are the pointer half of `isCanvasKey` (`./keyDoors.ts`)**, and they are
