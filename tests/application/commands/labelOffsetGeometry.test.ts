...
import { SpatialObjectGeometrySchemaV16 } from '../../../src/infrastructure/persistence/dto/planGeometry';
...
it('refuses an offset that is not a pair of numbers, and lets a zone version see an offset change', () => {
	...
	expect(SpatialObjectGeometrySchemaV16.safeParse({ ...entry, labelOffset: { dx: 'far', dy: 0 } }).success).toBe(false);
	expect(SpatialObjectGeometrySchemaV16.safeParse({ ...entry, labelOffset: { dx: 5, dy: 0 } }).success).toBe(true);
