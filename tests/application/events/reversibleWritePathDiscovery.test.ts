...
const DISPOSITIONS: Readonly<Record<string, Disposition>> = {
	'src/application/commands/zone/EditZoneDetails.ts::EditZoneDetailsCommand': rows(['execute', 'undo'], 'EditZoneDetails'),
 'src/application/commands/renovation/MaterialCommand.ts::MaterialCommand': rows(['execute', 'undo'], 'MaterialCommand'),
	...
	// The same shape again: a renovation write and its construction entries, composed, with no file of its own.
	'src/application/commands/renovation/ConstructionMaterialCommand.ts::ConstructionMaterialCommand': rows(['execute', 'undo'], 'ConstructionMaterialCommand'),
