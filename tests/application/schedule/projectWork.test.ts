...
describe('project Work over the existing floor registers', () => {
 ...
 it('lists Work once across shared rooms and floors, resolves blockers, and preserves missing-room and partial-read truth', async () => {
  const rig = await downstreamStack();
  try {
   const room = makeZone({ projectId: rig.plan.projectId, planId: rig.plan.id, name: 'Second room', zoneType: 'Room' }); expectOk(await rig.persistence.zones.save(room, 'absent'));
   const first = expectDefined(expectOk(await rig.persistence.plans.getById(rig.plan.id)), 'floor');
   ...
   expectOk(await rig.persistence.plans.save(expectOk(withPlanRenovation(first.entity, next)), first.version));
   const other = makePlan({ projectId: rig.plan.projectId, name: 'Upper floor' }); expectOk(await rig.persistence.plans.save(other, 'absent'));
   const read = expectOk(await readProjectWork(rig.persistence, rig.plan.projectId));
   ...
   vi.spyOn(rig.persistence.zones, 'listByProject').mockResolvedValueOnce(err(offline));
   const partial = expectOk(await readProjectWork(rig.persistence, rig.plan.projectId)); expect(partial.roomsIncomplete).toBe(true); expect(partial.rows[0].rooms.every(item => item.name === null)).toBe(true); expect(partial.rows).toHaveLength(2);
  } finally { rig.dispose(); }
 });
