...
export const CENSUS_TABLE: readonly CensusRow[] = [
	{ module: 'EditZoneDetails', direction: 'execute', mustPublish: 'ZoneDetailsChanged after a metadata write, including redo; nothing on refusal/no-op; editZoneDetails.test.ts' },
	{ module: 'EditZoneDetails', direction: 'undo', mustPublish: 'ZoneDetailsChanged after restoring metadata; editZoneDetails.test.ts' },
 ...
 { module: 'MaterialCommand', direction: 'undo', mustPublish: 'Requirement lifecycle, changed effective cost and PlanRenovationChanged after conditional inverse; planningFaults.test.ts' },
	...
	{ module: 'ConstructionMaterialCommand', direction: 'execute', mustPublish: 'Nothing of its own — the composed MaterialCommand and RenovationCommand events in step order: entry deletions, the record write, new entries; nothing on a referenced-entry refusal, which is checked before any write; constructionMaterial.test.ts' },
	{ module: 'ConstructionMaterialCommand', direction: 'undo', mustPublish: 'The composed commands\' own events in reverse step order; a refused step puts back the steps already moved; constructionMaterial.test.ts' },
