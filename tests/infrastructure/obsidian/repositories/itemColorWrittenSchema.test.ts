...
async function writtenVersion(structure: Structure, intended?: Structure) {
	const rig = await structureStack(), read = expectOk(await rig.geometry.read(rig.plan.id));
	expectOk(await rig.geometry.write(rig.plan.id, { ...read.document, structure, ...(intended ? { intended } : {}) }, read.version));
...
it('counts a colour on the intended structure alone: 16 for a wall, 14 for a preset on an item', async () => {
	const walls = WALL_LOOP_WITH_SIDES.walls.map((wall, index) => index === 0 ? { ...wall, color: 'rose' as const } : wall);
	expect(await writtenVersion(WALL_LOOP_WITH_SIDES, { ...WALL_LOOP_WITH_SIDES, walls })).toBe(16);
	expect(await writtenVersion(WALL_LOOP_WITH_SIDES, { ...WALL_LOOP_WITH_SIDES, elements: [{ ...item, color: 'blue' }] })).toBe(14);
});
