...
describe('reference appearance schema upgrade', () => {
	it('upgrades legacy metadata only in memory and preserves its absent appearance and calibration', () => {
		...
		expect(raw).toEqual(before); expect(latest).toEqual({ ...raw, 'schema-version': 7 });
		expect(migrations.migrateToLatest('plan', latest, 7)).toEqual(latest);
		...
		expect(migrations.lastApplied).toBe('plan: 6 -> 7');
	});
	...
	it('refuses malformed appearance and future versions instead of defaulting silently', () => {
		...
		expect(() => createMigrationRunner(MIGRATION_SET).migrateToLatest('plan', raw, 8)).toThrow(/newer than this build supports/);
