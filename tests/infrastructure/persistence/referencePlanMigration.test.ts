...
describe('reference appearance schema upgrade', () => {
	it('upgrades legacy metadata only in memory and preserves its absent appearance and calibration', () => {
		...
		expect(raw).toEqual(before); expect(latest).toEqual({ ...raw, 'schema-version': 8 });
		expect(migrations.migrateToLatest('plan', latest, 8)).toEqual(latest);
		...
		expect(migrations.lastApplied).toBe('plan: 7 -> 8');
	});
	...
	it('refuses malformed appearance and future versions instead of defaulting silently', () => {
		...
		expect(() => createMigrationRunner(MIGRATION_SET).migrateToLatest('plan', raw, 9)).toThrow(/newer than this build supports/);
