...
describe('reference appearance schema upgrade', () => {
	it('upgrades legacy metadata only in memory and preserves its absent appearance and calibration', () => {
		...
		expect(raw).toEqual(before); expect(latest).toEqual({ ...raw, 'schema-version': 11 });
		expect(migrations.migrateToLatest('plan', latest, 11)).toEqual(latest);
		...
		expect(migrations.lastApplied).toBe('plan: 10 -> 11');
	});
	...
	it('refuses malformed appearance and future versions instead of defaulting silently', () => {
		...
		expect(() => createMigrationRunner(MIGRATION_SET).migrateToLatest('plan', raw, 12)).toThrow(/newer than this build supports/);
