...
describe('reference appearance schema upgrade', () => {
	it('upgrades legacy metadata only in memory and preserves its absent appearance and calibration', () => {
		...
		expect(raw).toEqual(before); expect(latest).toEqual({ ...raw, 'schema-version': 4 });
		expect(migrations.migrateToLatest('plan', latest, 4)).toEqual(latest);
		...
		expect(migrations.lastApplied).toBe('plan: 3 -> 4');
	});
	...
	it('refuses malformed appearance and future versions instead of defaulting silently', () => {
		...
		expect(() => createMigrationRunner(MIGRATION_SET).migrateToLatest('plan', raw, 5)).toThrow(/newer than this build supports/);
