 ...
 *
 * `renovationProjectOpenPlan`/`renovationProjectOpenProject` — `RenovationProjectDeps.openPlan`
 * and `.openProject` bound to the real Obsidian-facing activations, pulled out of
 * `renovationProjectDeps` purely for `composition-root.ts`'s 400-line budget. Moving code does
 * not move its behaviour for free, so this file drives both directly rather than trusting that
 ...
 * faults" (that case predates the extraction and needs nothing new here); `openPlan`'s never
 * had a caller that drives a real fault through it, since `renovationProjectWiring.test.ts`
 * mocks `revealPlanEditor` outright to prove the WIRING. This file is the other half: the real
 * `revealPlanEditor`, faulted for real.
 */
...
import { renovationProjectOpenPlan } from '../../src/plugin/renovationProjectOpenSeams';
