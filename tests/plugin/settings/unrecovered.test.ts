...
describe('a fresh install, which is the opposite outcome', () => {
	it('loads the defaults, saves normally and renders its controls', async () => {
		...
		// Four definitions, three of them controls: units, the slice-4 project folder (the one
		// location field) and slice 11's verbose-logging toggle, plus slice 19's library folder
		// row — which is a name and a description and binds no control, because writing that
		// setting without moving the notes first strands the catalogue.
		expect(tab.getSettingDefinitions()).toHaveLength(4);
