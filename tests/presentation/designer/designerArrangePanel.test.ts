...
describe('what the block draws', () => {
	...
	it('offers the four distributions only at three, naming centres and gaps in the accessible name', () => {
		const { wrapper } = mountPanel();
		expect(names(wrapper)).toContain('distribute-centres-x');
		expect(names(wrapper)).toContain('distribute-gaps-y');
		expect(wrapper.find('[name="distribute-gaps-x"]').attributes('aria-label')).toBe(t('en', 'designer.arrange.distribute.gaps-x'));
	});
...
/**
 * AD18-R16 Task 10: both boards draw Align and Distribute as an icon row. `DesignerActionButton`'s
 * icon-only branch moves each button's label from its own text to `aria-label` — Obsidian's own
 * tooltip source (`DesignerToolButton.vue`'s convention, no `title`) — and draws the glyph the
 * mocked `setIcon` (`tests/helpers/obsidianIcons.ts`) records as `data-icon`.
 */
