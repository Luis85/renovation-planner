...
// The one calibration this suite has, rather than a third hand-spelled copy of its four fields.
...
describe('the guided trace checklist', () => {
	...
	/**
	 * The current step is the FIRST one not done. `assetDesign()` carries a sheet and a typed
	 * footprint but no calibration, so the step that is owed is the second one — which is also the
	 * case that proves "current" is not "the last done plus one".
	 */
	it('marks exactly one step current, and it is the first step not done', () => {
		const wrapper = checklist(assetDesign());
