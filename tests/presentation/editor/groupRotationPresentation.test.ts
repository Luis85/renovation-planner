...
it('uses the aggregate bounds centre for an explicit group and never treats disconnected members as a polygon', () => {
	...
	const controls = layoutRotationControls(group, pivot, 1); expect(controls).toHaveLength(4);
	for (const control of controls) expect(control.anchor.x === 0 || control.anchor.x === 9000 || control.anchor.y === 0 || control.anchor.y === 2000).toBe(true);
...
it('keeps hidden member geometry in the transform and refuses an empty group envelope', () => {
	...
	expect(layoutRotationControls({ ...group, points: [] }, pivot, 1)).toEqual([]);
});
