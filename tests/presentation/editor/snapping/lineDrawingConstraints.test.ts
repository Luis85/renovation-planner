...
it('matches the Zone constraint, placement and Shift release for walls and every line element', () => {
	...
	for (const id of ['draw-path', 'draw-fence', 'measure', 'place-stair', 'draw-arrow'] as const) {
		...
		const tool = new ElementTool(id, {
			draft: element, start: vi.fn<() => void>(), stop: vi.fn<() => void>(), finish: vi.fn<() => void>(), blocked: () => false,
			addPoint: point => { element.points.push(point); return true; },
			setPoints: points => { element.points = points.map(point => ({ ...point })); return true; },
		});
