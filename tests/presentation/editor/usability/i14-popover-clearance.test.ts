...
it.each([138.6875, 174.6875])('keeps Add detail above a wrapping taskbar when the canvas starts at %spx', async canvasTop => {
	...
	const popover = rig.wrapper.get<HTMLElement>('.rp-direct-actions').element;
	...
	const detail = rig.wrapper.get<HTMLButtonElement>('[data-rp-canvas-detail]').element;
	detail.focus();
