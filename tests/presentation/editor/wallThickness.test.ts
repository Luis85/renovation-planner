...
it('previews direct millimetre entry, commits once, undoes, redoes and reads the saved structure', async () => {
	...
	await rig.control.begin('wall-a'); rig.control.update('a', '0,237');
	expect(rig.runtime.structureActions.preview.value?.walls[0].thickness).toBe(312);
	...
	const after = { ...before, structure: { ...before.structure, walls: [{ ...before.structure?.walls[0], thickness: 312, sideExtents: { a: 237, b: 75 } }, ...expectDefined(before.structure, 'structure').walls.slice(1)] } };
...
it('clamps step previews, rejects invalid input and cancels without a write', async () => {
	...
	rig.control.increment('a', 1); expect(rig.control.total.value).toBe(160);
	rig.control.increment('a', -1); expect(rig.control.total.value).toBe(150);
	for (const text of ['bad', '', '-1', '1000.001']) {
		rig.control.update('a', text); rig.control.increment('a', 1); await rig.control.apply(); expect(rig.control.proposal.value).toBeNull();
	}
	rig.control.update('a', '0.001'); rig.control.increment('a', -1); expect(rig.control.values.value).toEqual({ a: 0, b: 75 });
	rig.control.update('a', '999.92'); rig.control.increment('a', 1); expect(rig.control.total.value).toBe(1_000_000);
	rig.control.close(); await rig.control.apply(); rig.control.update('a', '0.2'); expect(run).not.toHaveBeenCalled();
...
it('retires mode, selection and tool changes synchronously, including retained callbacks', async () => {
	...
	for (const mode of ['renovate', 'review'] as const) {
		await rig.control.begin('wall-a', 'adjust'); rig.control.increment('a', 1);
		const retained = rig.control.apply;
		rig.session.perspective = mode; expect(rig.control.target.value).toBeNull();
		await rig.control.begin('wall-a'); rig.control.increment('a', 1); await retained();
...
it('refuses stale and saving entry, discards delayed reads on mode round trips and disposal', async () => {
	...
	const pending = rig.control.begin('wall-a'); await rig.control.begin('wall-a'); rig.control.update('a', '0.2');
...
it('keeps ordinary refused writes retryable and refuses duplicate or busy updates', async () => {
	const rig = await setup(); await rig.control.begin('wall-a'); rig.control.update('a', '0.2');
	...
	const saving = rig.control.apply(); rig.control.update('a', '0.3'); await rig.control.apply(); expect(rig.control.text.value.a).toBe('0.2'); expect(run).toHaveBeenCalledTimes(2);
...
it('locks a conflicting preview and catches read/write faults', async () => {
	...
	await rig.control.begin('wall-a'); rig.control.update('a', '0.2');
...
it('offers keyboard focus, live feedback and Escape from the Details thickness route', async () => {
	...
	const input = rig.wrapper.get<HTMLInputElement>('[name="wall-side-a"]'); expect(document.activeElement).toBe(input.element);
	await input.setValue('0.183'); expect(rig.wrapper.get('.rp-wall-thickness-panel [role="status"]').text()).toContain('0.258');
...
it('steps an existing fractional millimetre thickness without quantizing it', async () => {
	...
	const structure = { ...rig.project.structure, walls: rig.project.structure.walls.map(wall => wall.id === 'wall-a' ? { ...wall, thickness: 150.25, sideExtents: { a: 75.125, b: 75.125 } } : wall) };
	expectOk(await rig.runtime.dispatcher.run(rig.services.command({ planId: rig.plan.id, baseline, structure, ledger: rig.runtime.structureTask.ledger })));
	await rig.control.begin('wall-a', 'adjust'); await rig.control.apply(); expect(rig.control.total.value).toBe(150.25);
	rig.control.increment('a', 1); expect(rig.control.total.value).toBe(160.25); await rig.control.apply();
...
it('validates the direct form, keeps modified Enter local, and saves only a valid Apply', async () => {
	...
	expect(expectOk(await rig.geometry.read(rig.plan.id)).document.structure?.walls[0].thickness).toBe(297);
});

it('refuses a real pending command when Plan retires before its version read completes', async () => {
	const rig = await setup(); await rig.control.begin('wall-a', 'adjust'); rig.control.update('a', '0.2');
	...
	expect(rig.control.target.value).toBe('wall-a'); expect(rig.control.total.value).toBe(275);
...
it('refreshes an externally changed wall before editing and refuses a vanished target', async () => {
	...
	const structure = { ...rig.project.structure, walls: rig.project.structure.walls.map(wall => wall.id === 'wall-a' ? { ...wall, thickness: 250, sideExtents: { a: 125, b: 125 } } : wall) };
	...
	await rig.control.begin('wall-a'); expect(rig.control.total.value).toBe(250); rig.control.close();
...
it('cancels on outside pointer input and restores canvas focus when the opener has disappeared', async () => {
	const rig = await setup(), opener = rig.wrapper.get<HTMLButtonElement>('[data-rp-action="wall-thickness"]');
	opener.element.focus(); await opener.trigger('click'); await settle(); rig.control.increment('a', 1);
...
it('highlights either face through focus and hover, steps only that face, and clears the cue on cancellation', async () => {
	const rig = await setup(), highlight = rig.runtime.structureActions.faceHighlight;
	await rig.control.begin('wall-a', 'adjust', 'b'); await settle();
	const panel = rig.wrapper.get('.rp-wall-thickness-panel'), a = panel.get('[data-wall-side="a"]'), b = panel.get('[data-wall-side="b"]');
	expect(document.activeElement).toBe(b.get('input').element); expect(highlight.target.value).toEqual({ wallId: 'wall-a', side: 'b' });
	await a.trigger('mouseenter'); expect(highlight.target.value?.side).toBe('a'); await a.trigger('mouseleave'); expect(highlight.target.value?.side).toBe('b');
	await b.trigger('focusout', { relatedTarget: b.get('button').element }); expect(highlight.target.value?.side).toBe('b');
	await b.trigger('focusout', { relatedTarget: rig.canvasEl }); expect(highlight.target.value).toBeNull(); await b.trigger('focusin');
	await b.get('button:last-child').trigger('click'); expect(rig.control.values.value).toEqual({ a: 75, b: 85 });
	await a.get('button:first-child').trigger('click'); expect(rig.control.values.value).toEqual({ a: 65, b: 85 });
	expect(rig.control.total.value).toBe(150); expect(rig.control.changed.value).toBe(true);
	await b.get('input').setValue('0'); await b.get('button:first-child').trigger('click'); expect(rig.control.values.value?.b).toBe(0);
	rig.control.focus(null); rig.control.hover(null); expect(highlight.target.value).toBeNull();
	rig.control.hover('a'); await settle(); expect(rig.stage.findOne('.wall-face-cue')).toBeTruthy(); rig.control.close(); await settle();
	expect(highlight.target.value).toBeNull(); expect(rig.stage.findOne('.wall-face-cue')).toBeUndefined();
	for (const [id, side] of [[null, 'a'], ['wall-b', 'a'], ['missing', 'b']] as const) highlight.show(id, side);
	expect(highlight.target.value).toBeNull();
});

it('offers the same face values and cues in the complete Details form and writes a same-total face change', async () => {
	const rig = await setup(), original = expectOk(await rig.geometry.read(rig.plan.id)).document;
	const pending = rig.runtime.structureActions.edit('wall-a'); await settleUntil(() => rig.dialogs.current !== null, 'details');
	const form = rig.wrapper.getComponent(StructureEditForm), a = form.get<HTMLInputElement>('[name="sideA"]'), b = form.get<HTMLInputElement>('[name="sideB"]');
	expect(a.element.value).toBe('0.075'); expect(b.element.value).toBe('0.075');
	await a.trigger('focus'); await b.trigger('mouseenter'); expect(rig.runtime.structureActions.faceHighlight.target.value?.side).toBe('b');
	await b.trigger('mouseleave'); expect(rig.runtime.structureActions.faceHighlight.target.value?.side).toBe('a');
	await a.setValue('0.1'); await b.setValue('0.05'); await form.trigger('submit'); await settle(); await form.trigger('submit'); await pending;
	const saved = expectOk(await rig.geometry.read(rig.plan.id)).document;
	expect(saved.structure?.walls[0]).toEqual({ ...original.structure?.walls[0], sideExtents: { a: 100, b: 50 } });
	expect(saved.structure?.openings).toEqual(original.structure?.openings); expect(rig.runtime.structureActions.faceHighlight.target.value).toBeNull();
	expect(rig.wrapper.text()).toContain('Side A'); expect(rig.wrapper.text()).toContain('Side B');
});

it('retires retained face callbacks when another dialog opens, and after disposal', async () => {
	const rig = await setup(); await rig.control.begin('wall-a'); rig.control.update('b', '0.125');
	const pending = rig.dialogs.openDialog({ kind: 'confirm', title: 'Another task', message: 'Continue?' });
	expect(rig.control.target.value).toBeNull(); rig.control.hover('b'); rig.control.focus('a'); await rig.control.apply();
	expect(rig.runtime.structureActions.faceHighlight.target.value).toBeNull();
	rig.dialogs.resolve('cancel'); await pending;
	const highlight = rig.runtime.structureActions.faceHighlight; rig.unmount(); highlight.show('wall-a', 'a'); rig.control.focus('b'); rig.control.hover('b');
	expect(highlight.target.value).toBeNull();
});

it('refuses retained Details callbacks after close and a pending face command after a mode round trip', async () => {
	const rig = await setup(), before = expectOk(await rig.geometry.read(rig.plan.id)).document;
	let pending = rig.runtime.structureActions.edit('wall-a'); await settle();
	const props = rig.wrapper.getComponent(StructureEditForm).props();
	rig.dialogs.resolve('cancel'); await pending;
	props.highlight?.('a'); props.preview(before.structure ?? null);
	expect(rig.runtime.structureActions.faceHighlight.target.value).toBeNull(); expect(rig.runtime.structureActions.preview.value).toBeNull();
	expect(await props.dispatch(expectDefined(before.structure, 'structure'))).toMatchObject({ ok: false });
	pending = rig.runtime.structureActions.edit('wall-a'); await settle();
	const form = rig.wrapper.getComponent(StructureEditForm); await form.get('[name="sideB"]').setValue('0.1'); await form.trigger('submit');
	const read = rig.geometry.read.bind(rig.geometry); let release: (() => void) | undefined;
	vi.spyOn(rig.geometry, 'read').mockImplementationOnce(async id => { await new Promise<void>(resolve => { release = resolve; }); return read(id); });
	await form.trigger('submit'); await settleUntil(() => release !== undefined, 'details command read');
	rig.session.perspective = 'review'; rig.session.perspective = 'plan'; release?.(); await settle();
	expect(expectOk(await rig.geometry.read(rig.plan.id)).document).toEqual(before); expect(rig.runtime.structureActions.preview.value).toBeNull();
	rig.dialogs.resolve('cancel'); await pending;
});

it('refuses an inside-curve collapse with an actionable error and allows the other face to recover', async () => {
	const rig = await setup(), before = expectOk(await rig.geometry.read(rig.plan.id));
	const wall = { ...rig.project.structure.walls[0], bulge: 0.5 };
	expectOk(await rig.geometry.write(rig.plan.id, { ...before.document, structure: { walls: [wall], openings: [], boundaries: [] } }, before.version)); await rig.runtime.refreshProjection();
	await rig.control.begin('wall-a', 'adjust'); rig.control.update('b', '2.5'); await settle();
	expect(rig.control.geometryIssue.value?.kind).toBe('curve-radius'); expect(rig.wrapper.get('.rp-wall-thickness-panel [role="alert"]').text()).toContain('curve');
	await rig.control.apply(); expect(rig.project.structure.walls[0].thickness).toBe(150);
	rig.control.update('b', '0.08'); rig.control.increment('a', 1); expect(rig.control.proposal.value?.walls[0].sideExtents).toEqual({ a: 85, b: 80 });
	rig.control.close();
});

it('names an unsupported curved junction and recovers when the connected network stays symmetric', async () => {
	const rig = await setup(), before = expectOk(await rig.geometry.read(rig.plan.id));
	const east = rig.project.structure.walls[0], west = { ...east, id: 'wall-west', start: { x: -4000, y: 0 }, end: east.start };
	const curve = { ...east, id: 'wall-curve', start: { x: -3000, y: -3000 }, end: east.start, bulge: 0.1, thickness: 600, sideExtents: { a: 300, b: 300 } };
	expectOk(await rig.geometry.write(rig.plan.id, { ...before.document, structure: { walls: [east, west, curve], openings: [], boundaries: [] } }, before.version)); await rig.runtime.refreshProjection();
	await rig.control.begin('wall-a'); rig.control.update('a', '0.08'); await settle();
	expect(rig.control.geometryIssue.value).toEqual({ wallId: curve.id, kind: 'curved-junction' });
	expect(rig.wrapper.get('.rp-wall-thickness-panel [role="alert"]').text()).toContain('three-way junction');
	await rig.control.apply(); expect(rig.project.structure.walls[0].thickness).toBe(150);
	rig.control.update('a', '0.075'); expect(rig.control.proposal.value).not.toBeNull(); expect(rig.control.changed.value).toBe(false);
});
