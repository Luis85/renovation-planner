...
export default defineConfig(({ mode }) => ({
	...
	build: {
		 ...
		 */
		outDir: 'dist',
		// The SYNTAX floor, which is not `tsconfig.json`'s `lib`: that is ES2021 plus
		// ES2023.Array and says which APIs exist. A method call is not downlevelled and the
		// build polyfills nothing, so the two move independently — see CLAUDE.md's Gotchas.
		target: 'es2020',
